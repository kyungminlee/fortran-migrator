program test_zhptrs
    ...
    use target_lapack,   only: target_name, target_eps, target_zhptrf, target_zhptrs
    use ref_quad_lapack, only: zhptrf, zhptrs
    implicit none
    ...
    call report_init('zhptrs', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_hermitian_matrix_quad(n, A0, seed = 81001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 81011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call zhptrs('U', n, nrhs, AP_ref, ipiv_ref, B_ref, n, info)
        call target_zhptrs('U', n, nrhs, AP_got, ipiv_got, B_got, n, info)
