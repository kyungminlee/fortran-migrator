...
class TargetMode:
    ...
    c_mpi_sum_real: Optional[str] = None      # 'MPI_SUM' / 'MPI_MM_SUM'
    c_mpi_sum_complex: Optional[str] = None   # 'MPI_SUM' / 'MPI_WW_SUM'
    c_mpi_max_real: Optional[str] = None      # 'MPI_MAX' / 'MPI_MM_AMX'
    c_mpi_max_complex: Optional[str] = None   # 'MPI_MAX' / 'MPI_WW_AMX'
    c_mpi_min_real: Optional[str] = None      # 'MPI_MIN' / 'MPI_MM_AMN'
    c_mpi_min_complex: Optional[str] = None   # 'MPI_MIN' / 'MPI_WW_AMN'
...
def load_target(name_or_path: str, project_root: Path | None = None) -> TargetMode:
    ...
    Resolution order:
      ...
      2. Look for codegen/targets/{name_or_path}.yaml relative to project_root.
      3. If name_or_path is a bare integer (e.g. "16"), try codegen/targets/kind{n}.yaml.
    """
    if project_root is None:
        # Default: repo root is two levels up from this file
        # (codegen/migrator/target_mode.py → repo root)
    ...
    # 2. Named lookup
    targets_dir = project_root / 'codegen' / 'targets'
