...
/*************************************************************************/
int METIS_MUMPS_PartGraphKway(idx_t *nvtxs, idx_t *ncon, idx_t *xadj, idx_t *adjncy, 
...
void InitKWayPartitioning(ctrl_t *ctrl, graph_t *graph)
{
  ...
  METIS_MUMPS_SetDefaultOptions(options);
  ...
  switch (ctrl->objtype) {
    ...
    case METIS_OBJTYPE_VOL:
      ...
      status = METIS_MUMPS_PartGraphRecursive(&graph->nvtxs, &graph->ncon, 
