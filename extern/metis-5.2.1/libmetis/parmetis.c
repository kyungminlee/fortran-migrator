...
/*! This function is the entry point for the node ND code for ParMETIS.
    The difference between this routine and the standard METIS_MUMPS_NodeND are
    the following
...
*/
/*************************************************************************/
int METIS_MUMPS_NodeNDP(idx_t nvtxs, idx_t *xadj, idx_t *adjncy, idx_t *vwgt,
...
*/
/**************************************************************************/
int METIS_MUMPS_ComputeVertexSeparator(idx_t *nvtxs, idx_t *xadj, idx_t *adjncy, 
...
/*************************************************************************/
int METIS_MUMPS_NodeRefine(idx_t nvtxs, idx_t *xadj, idx_t *vwgt, idx_t *adjncy, 
...
*/
/**************************************************************************/
int METIS_MUMPS_CacheFriendlyReordering(idx_t nvtxs, idx_t *xadj, idx_t *adjncy, 
           idx_t *part, idx_t *old2new) 
{
  ...
  cot = iincset(nvtxs, 0, imalloc(nvtxs, "METIS_MUMPS_CacheFriendlyReordering: cor"));

  /* This array will function like pos + touched of the CC method */
  pos = iincset(nvtxs, 0, imalloc(nvtxs, "METIS_MUMPS_CacheFriendlyReordering: pos"));
...
//  printf("lastlevel: %d\n", (int)-lastlevel);
  ...
  levels = ikvmalloc(nvtxs, "METIS_MUMPS_CacheFriendlyReordering: levels");
  maxdegree++;
  for (i=0; i<nvtxs; i++) {
    levels[i].val = i;
  ...
  pwgts  = ismalloc(nparts+1, 0, "METIS_MUMPS_CacheFriendlyReordering: pwgts");
