...
**************************************************************************/
int METIS_MUMPS_PartMeshNodal(idx_t *ne, idx_t *nn, idx_t *eptr, idx_t *eind, 
          ...
          idx_t *options, idx_t *objval, idx_t *epart, idx_t *npart)
{
  ...
  /* get the nodal graph */
  rstatus = METIS_MUMPS_MeshToNodal(ne, nn, eptr, eind, &pnumflag, &xadj, &adjncy);
  if (rstatus != METIS_OK)
    raise(SIGERR);

  /* partition the graph */
  if (ptype == METIS_PTYPE_KWAY) 
    rstatus = METIS_MUMPS_PartGraphKway(nn, &ncon, xadj, adjncy, vwgt, vsize, NULL, 
                  nparts, tpwgts, NULL, options, objval, npart);
  else 
    rstatus = METIS_MUMPS_PartGraphRecursive(nn, &ncon, xadj, adjncy, vwgt, vsize, NULL, 
                  nparts, tpwgts, NULL, options, objval, npart);
...
SIGTHROW:
  ...
  METIS_MUMPS_Free(xadj);
  METIS_MUMPS_Free(adjncy);
...
**************************************************************************/
int METIS_MUMPS_PartMeshDual(idx_t *ne, idx_t *nn, idx_t *eptr, idx_t *eind, 
          ...
          idx_t *npart) 
{
  ...
  /* get the dual graph */
  rstatus = METIS_MUMPS_MeshToDual(ne, nn, eptr, eind, ncommon, &pnumflag, &xadj, &adjncy);
  if (rstatus != METIS_OK)
    raise(SIGERR);

  /* partition the graph */
  if (ptype == METIS_PTYPE_KWAY) 
    rstatus = METIS_MUMPS_PartGraphKway(ne, &ncon, xadj, adjncy, vwgt, vsize, NULL, 
                  nparts, tpwgts, NULL, options, objval, epart);
  else 
    rstatus = METIS_MUMPS_PartGraphRecursive(ne, &ncon, xadj, adjncy, vwgt, vsize, NULL, 
                  nparts, tpwgts, NULL, options, objval, epart);
  ...
  nptr = ismalloc(*nn+1, 0, "METIS_MUMPS_PartMeshDual: nptr");
  nind = imalloc(eptr[*ne], "METIS_MUMPS_PartMeshDual: nind");
