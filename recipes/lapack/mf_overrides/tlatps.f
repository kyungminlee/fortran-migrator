*> \brief \b TLATPS solves a triangular system of equations with the matrix held in packed storage.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download TLATPS + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/tlatps.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/tlatps.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/tlatps.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE TLATPS( UPLO, TRANS, DIAG, NORMIN, N, AP, X, SCALE,
*                          CNORM, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> TLATPS solves one of the triangular systems
*>
...
*> TTPSV is called. If the matrix A is singular (A(j,j) = 0 for some j),
...
*> \param[in] AP
*> \verbatim
*>          AP is TYPE(real64x2) array, dimension (N*(N+1)/2)
...
*> \verbatim
*>
*>  A rough bound on x is computed; if that is less than overflow, TTPSV
...
*>                                   1<=i< j
*>
*>  Since |x(j)| <= M(j), we use the Level 2 BLAS routine TTPSV if the
...
*>                      1<=i<=j
*>
*>  and we can safely call TTPSV if 1/M(n) and 1/G(n) are both greater
...
*     .. External Subroutines ..
      EXTERNAL           TAXPY, TSCAL, TTPSV, XERBLA
*     ..
*     .. Intrinsic Functions ..
*     ..
...
*     Level 2 BLAS routine TTPSV can be used.
*
      J = ITAMAX( N, X, 1 )
      XMAX = ABS( X( J ) )
      XBND = XMAX
...
*
         CALL TTPSV( UPLO, TRANS, DIAG, N, AP, X, 1 )
      ELSE
*
...
*
                     CALL TAXPY( J-1, -X( J )*TSCFAC, AP( IP-J+1 ), 1,
     $                           X,
     $                           1 )
                     I = ITAMAX( J-1, X, 1 )
                     XMAX = ABS( X( I ) )
...
*                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j)
*
                     CALL TAXPY( N-J, -X( J )*TSCFAC, AP( IP+1 ), 1,
     $                           X( J+1 ), 1 )
...
*
                  IF( UPPER ) THEN
                     SUMJ = TDOT( J-1, AP( IP-J+1 ), 1, X, 1 )
                  ELSE IF( J.LT.N ) THEN
                     SUMJ = TDOT( N-J, AP( IP+1 ), 1, X( J+1 ), 1 )
                  END IF
               ELSE
*
