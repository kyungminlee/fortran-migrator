*> \brief \b VLATRS solves a triangular system of equations with the scale factor set to prevent overflow.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download VLATRS + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/vlatrs.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/vlatrs.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/vlatrs.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE VLATRS( UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X, SCALE,
*                          CNORM, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> VLATRS solves one of the triangular systems
*>
...
*> VTRSV is called. If the matrix A is singular (A(j,j) = 0 for some j),
...
*> \param[in] A
*> \verbatim
*>          A is TYPE(cmplx64x2) array, dimension (LDA,N)
...
*> \verbatim
*>
*>  A rough bound on x is computed; if that is less than overflow, VTRSV
...
*>                                   1<=i< j
*>
*>  Since |x(j)| <= M(j), we use the Level 2 BLAS routine VTRSV if the
...
*>                      1<=i<=j
*>
*>  and we can safely call VTRSV if 1/M(n) and 1/G(n) are both greater
*>  than max(underflow, 1/overflow).
*> \endverbatim
*>
*  =====================================================================
      SUBROUTINE VLATRS( UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X,
     $                   SCALE,
     $                   CNORM, INFO )
...
*     .. External Subroutines ..
      EXTERNAL           TSCAL, XERBLA, VAXPY, VTSCAL, VTRSV
*     ..
*     .. Intrinsic Functions ..
...
*           not +/- Infinity, use this value as TSCFAC.
            TMAX = DD_ZERO
            IF( UPPER ) THEN
*
*              A is upper triangular.
*
               DO J = 2, N
                  DO I = 1, J - 1
                     TMAX = MAX( TMAX, ABS( real64x2( A( I, J ) ) ),
     $                           ABS( AIMAG(A ( I, J ) ) ) )
                  END DO
               END DO
            ELSE
...
*     Level 2 BLAS routine VTRSV can be used.
*
      XMAX = DD_ZERO
      DO 30 J = 1, N
...
*
                     CALL VAXPY( J-1, -X( J )*TSCFAC, A( 1, J ), 1, X,
     $                           1 )
                     I = IVAMAX( J-1, X, 1 )
                     XMAX = CABS1( X( I ) )
                  END IF
...
*                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j)
*
                     CALL VAXPY( N-J, -X( J )*TSCFAC, A( J+1, J ), 1,
     $                           X( J+1 ), 1 )
...
*
                  IF( UPPER ) THEN
                     CSUMJ = VDOTU( J-1, A( 1, J ), 1, X, 1 )
                  ELSE IF( J.LT.N ) THEN
                     CSUMJ = VDOTU( N-J, A( J+1, J ), 1, X( J+1 ),
     $                              1 )
                  END IF
               ELSE
*
...
*
                  IF( UPPER ) THEN
                     CSUMJ = VDOTC( J-1, A( 1, J ), 1, X, 1 )
                  ELSE IF( J.LT.N ) THEN
                     CSUMJ = VDOTC( N-J, A( J+1, J ), 1, X( J+1 ),
     $                              1 )
                  END IF
               ELSE
*
