*> \brief \b TLATBS solves a triangular banded system of equations.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download TLATBS + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/tlatbs.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/tlatbs.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/tlatbs.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE TLATBS( UPLO, TRANS, DIAG, NORMIN, N, KD, AB, LDAB, X,
*                          SCALE, CNORM, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> TLATBS solves one of the triangular systems
*>
...
*> overflow, the Level 2 BLAS routine TTBSV is called.  If the matrix A
...
*> \param[in] AB
*> \verbatim
*>          AB is TYPE(real64x2) array, dimension (LDAB,N)
...
*> \verbatim
*>
*>  A rough bound on x is computed; if that is less than overflow, TTBSV
...
*>                                   1<=i< j
*>
*>  Since |x(j)| <= M(j), we use the Level 2 BLAS routine TTBSV if the
...
*>                      1<=i<=j
*>
*>  and we can safely call TTBSV if 1/M(n) and 1/G(n) are both greater
*>  than max(underflow, 1/overflow).
*> \endverbatim
*>
*  =====================================================================
      SUBROUTINE TLATBS( UPLO, TRANS, DIAG, NORMIN, N, KD, AB, LDAB,
     $                   X,
     $                   SCALE, CNORM, INFO )
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ITAMAX
      TYPE(real64x2)   TASUM, TDOT, TLAMCH
      EXTERNAL           LSAME, ITAMAX, TASUM, TDOT, TLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           TAXPY, TSCAL, TTBSV, XERBLA
*     ..
*     .. Intrinsic Functions ..
*     ..
...
*
            DO 10 J = 1, N
               JLEN = MIN( KD, J-1 )
               CNORM( J ) = TASUM( JLEN, AB( KD+1-JLEN, J ), 1 )
   10       CONTINUE
         ELSE
*
...
*     Level 2 BLAS routine TTBSV can be used.
*
      J = ITAMAX( N, X, 1 )
      XMAX = ABS( X( J ) )
      XBND = XMAX
...
*
         CALL TTBSV( UPLO, TRANS, DIAG, N, KD, AB, LDAB, X, 1 )
      ELSE
*
...
*
                  JLEN = MIN( KD, N-J )
                  IF( JLEN.GT.0 )
     $               CALL TAXPY( JLEN, -X( J )*TSCFAC, AB( 2, J ), 1,
     $                           X( J+1 ), 1 )
...
*
                  IF( UPPER ) THEN
                     JLEN = MIN( KD, J-1 )
                     SUMJ = TDOT( JLEN, AB( KD+1-JLEN, J ), 1,
     $                      X( J-JLEN ), 1 )
                  ELSE
                     JLEN = MIN( KD, N-J )
                     IF( JLEN.GT.0 )
     $                  SUMJ = TDOT( JLEN, AB( 2, J ), 1, X( J+1 ),
     $                               1 )
                  END IF
               ELSE
*
