      RECURSIVE SUBROUTINE PTLAQR1( WANTT, WANTZ, N, ILO, IHI, A,
     $                              DESCA, WR, WI, ILOZ, IHIZ, Z,
     $                              DESCZ, WORK, LWORK, IWORK,
     $                              ILWORK, INFO )
      USE multifloats, only: abs, int, max, min, mod, real64x2, sign,
     +sqrt, dd_half, dd_one, dd_zero, operator(+), operator(-),
...
*     .. Array Arguments ..
      ...
      TYPE(real64x2)   A( * ), WI( * ), WORK( * ), WR( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PTLAQR1 is an auxiliary routine used to find the Schur decomposition
*    and or eigenvalues of a matrix already in Hessenberg form from
*    cols ILO to IHI.
*
*  This is a modified version of PTLAHQR from ScaLAPACK version 1.7.3.
...
*      subdiagonal elements by PTLACONSB) is abandoned.
...
*    o For some reason, the original version of PTLAHQR sometimes did
...
*          ILO = 1). PTLAQR1 works primarily with the Hessenberg
*          submatrix in rows and columns ILO to IHI, but applies
*          transformations to all of H if WANTT is .TRUE..
*          1 <= ILO <= max(1,IHI); IHI <= N.
*
*  A       (global input/output) TYPE(real64x2) array, dimension
*          (DESCA(LLD_),*)
...
*
*  WR      (global replicated output) TYPE(real64x2) array,
*                                                         dimension (N)
*  WI      (global replicated output) TYPE(real64x2) array,
*                                                         dimension (N)
...
*
*  Z       (global input/output) TYPE(real64x2) array.
*          If WANTZ is .TRUE., on entry Z must contain the current
*          matrix Z of transformations accumulated by PTHSEQR, and on
...
*
*  WORK    (local output) TYPE(real64x2) array of size LWORK
*
...
*          = 0: successful exit
*          > 0: PTLAQR1 failed to compute all the eigenvalues ILO to IHI
...
*  Subroutines:
*       This routine calls:
*           PTLAWIL   -> Given the shift, get the transformation
*           TLASORTE  -> Pair up eigenvalues so that reals are paired.
*           PTLACP3   -> Parallel array to local replicated array copy &
*                        back.
*           TLAREF    -> Row/column reflector applier. Core routine here.
*           PTLASMSUB -> Finds negligible subdiagonal elements.
*
...
*     .. Parameters ..
     ...
     +                   RSRC_ = 7,  CSRC_ = 8,  LLD_ = 9 )
      TYPE(real64x2)   CONST
      ! Converted to assignments below: PARAMETER          ( CONST = real64x2(limbs=[1.50D0, 0.0_8]) )
      INTEGER            IBLK, LDS
...
*     .. Local Scalars ..
     ...
     $                   DBLK, NIBBLE, ND, NS, LTOP, LWKOPT, S1, S2, S3
      TYPE(real64x2)   AVE, DISC, H00, H10, H11, H12, H21, H22, H33,
...
*     .. Local Arrays ..
     ...
     $                   KP2ROW( IBLK ), KROW( IBLK ), LOCALK2( IBLK )
      TYPE(real64x2)   SMALLA( 6, 6, IBLK ), VCSAVE( 3 )
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC, ILAENV
      TYPE(real64x2)   PTLAMCH
      EXTERNAL           ILCM, NUMROC, ILAENV, PTLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, TCOPY, TGEBR2D, TGEBS2D,
     $                   TGERV2D, TGESD2D, TGSUM2D, TLAHQR, TLAREF,
     $                   TLARFG, TLASORTE, IGAMN2D, INFOG1L, INFOG2L,
     $                   PTLABAD, PTLACP3, PTLASMSUB,
     $                   PTLAWIL, PXERBLA, TLANV2, PTLAQR2, PTLAQR4
*     ..
*     .. Intrinsic Functions ..
*     ..
*     .. Executable Statements ..
*
      CONST = real64x2(limbs=[1.50D0, 0.0_8])
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PTLAQR1', -INFO )
         WORK( 1 ) = real64x2( LWKOPT )
         RETURN
      END IF
*
...
*     call TLAHQR directly.
*
      IF( NH .LE. LDS ) THEN
         CALL PTLAQR4( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,
...
*
      UNFL = PTLAMCH( CONTXT, 'SAFE MINIMUM' )
      OVFL = DD_ONE / UNFL
      CALL PTLABAD( CONTXT, UNFL, OVFL )
      ULP = PTLAMCH( CONTXT, 'PRECISION' )
      SMLNUM = UNFL*( NH / ULP )
*
...
*
         CALL PTLASMSUB( A, DESCA, I, L, K, SMLNUM, WORK( IRBUF+1 ),
     $                   LWORK-IRBUF )
         L = K
*
         IF( L.GT.ILO ) THEN
...
*
     ...
     $                    IROW, ICOL, ITMP1, ITMP2 )
            IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) THEN
               A( ( ICOL-1 )*LDA+IROW ) = DD_ZERO
            END IF
            WORK( ISUB+L-1 ) = DD_ZERO
         END IF
*
...
*           Exceptional shift.
*
            CALL PTLACP3( 2*JBLK, I-2*JBLK+1, A, DESCA, WORK( S1+1 ),
     $                    LDS, -1, -1, 0 )
            DO 20 II = 2*JBLK, 2, -1
               WORK( S1+II+(II-1)*LDS ) = CONST*(
     $              ABS( WORK( S1+II+(II-1)*LDS ) )+
     $              ABS( WORK( S1+II+(II-2)*LDS ) ) )
               WORK( S1+II+(II-2)*LDS ) = DD_ZERO
               WORK( S1+II-1+(II-1)*LDS ) = DD_ZERO
   20       CONTINUE
...
*
            IF( AED ) THEN
               DBLK = ILAENV( 13, 'TLAQR0', 'SV', N, L, I, 4*LDS*LDS )
               DBLK = MAX( 2*JBLK, DBLK ) + 1
               DBLK = MIN( NH, LDS, DBLK )
               CALL PTLAQR2( WANTT, WANTZ, N, L, I, DBLK, A, DESCA,
...
*
               NIBBLE = ILAENV( 14, 'TLAQR0', 'SV', N, L, I, 4*LDS*LDS )
               NIBBLE = MAX( 0, NIBBLE )
               I = I - ND
               DBLK = DBLK - ND
...
*               IF( ND.GE.0 .AND. ND+DBLK.GE.64 ) THEN
               IF( .FALSE. ) THEN
                  CALL TLASET( 'L', DBLK-1, DBLK-1, DD_ZERO, DD_ZERO,
     $                         WORK( S1+2 ), LDS )
                  WORK( IRBUF+1 ) = WORK( S1+1 )
                  WORK( ICBUF+1 ) = DD_ZERO
*
...
*
                  II = 2
   22             CONTINUE
                     IF( WORK( ICBUF+II ) .EQ. DD_ZERO ) THEN
               ...
               ELSE
                  CALL TLAHQR( .FALSE., .FALSE., DBLK, 1, DBLK,
     $                         WORK( S1+1 ), LDS, WORK( IRBUF+1 ),
     $                         WORK( ICBUF+1 ), 1, DBLK, Z, LDZ, IERR )
               END IF
            ELSE
               DBLK = 2*JBLK
               CALL PTLACP3( DBLK, I-DBLK+1, A, DESCA, WORK( S1+1 ),
     $                       LDS, -1, -1, 0 )
               CALL TLAHQR( .FALSE., .FALSE., DBLK, 1, DBLK,
     $                      WORK( S1+1 ), LDS, WORK( IRBUF+1 ),
     $                      WORK( ICBUF+1 ), 1, DBLK, Z, LDZ, IERR )
...
*
     ...
     $               WORK( S1+DBLK+(DBLK-2)*LDS )
            IF( ( JBLK.GT.1 ) .AND. ( ITS.GT.30 ) ) THEN
               ...
               DISC = ( H33-H44 )*DD_HALF
               DISC = DISC*DISC + H43H34
               IF( DISC.GT.DD_ZERO ) THEN
*
*                 Real roots: Use Wilkinson's shift twice
*
                  DISC = SQRT( DISC )
                  AVE = DD_HALF*( H33+H44 )
                  IF( ABS( H33 )-ABS( H44 ).GT.DD_ZERO ) THEN
                     H33 = H33*H44 - H43H34
...
*           PTLACONSB is the routine that does this.
*
*         CALL PTLACONSB( A, DESCA, I, L, M, H44, H33, H43H34,
...
*    $      GO TO 80
*
*        In principle PTLACONSB needs to check all shifts to decide
...
*
*            CALL TLASORTE( S1( 2*( JBLK-NBULGE )+1,
*     $                     2*( JBLK-NBULGE )+1 ), 3*IBLK, 2*NBULGE,
*     $                     WORK( IRBUF+1 ), IERR )
            CALL TLASORTE( WORK(S1+DBLK-2*NBULGE+1+(DBLK-2*NBULGE)*LDS),
...
*
         DO 31 ITMP1 = 1, 3
            VCSAVE(ITMP1) = DD_ZERO
   31    CONTINUE
         ITMP1 = ISTARTROW
         ITMP2 = ISTARTCOL
         CALL PTLAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33, H43H34,
     $                 VCSAVE )
         V1SAVE = VCSAVE( 1 )
         V2SAVE = VCSAVE( 2 )
         V3SAVE = VCSAVE( 3 )
   ...
   40       CONTINUE
     ...
     $                    *WORK(S1+DBLK-2*IBULGE+(DBLK-2*IBULGE-2)*LDS)
                  ITMP1 = ISTARTROW
                  ITMP2 = ISTARTCOL
                  CALL PTLAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33,
     $                          H43H34, VCSAVE )
                  V1SAVE = VCSAVE( 1 )
                  V2SAVE = VCSAVE( 2 )
                  V3SAVE = VCSAVE( 3 )
...
*
     ...
     $                             MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                     CALL PTLACP3( MIN( 6, N-K+2 ), K-1, A, DESCA,
...
*
     ...
     $                             MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                     CALL PTLACP3( MIN( 6, N-K+3 ), K-2, A, DESCA,
     $                             SMALLA( 1, 1, KI ), 6, ITMP1, ITMP2,
     $                             0 )
                  END IF
               END IF
*
*           TLAHQR used to have a single row application and a single
...
*
     ...
     $             ( ISTART.LT.MIN( I-1, ISTOP+1 ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL TCOPY( NR, SMALLA( 2, 1, KI ), 1, VCSAVE, 1 )
                  ELSE
                     VCSAVE( 1 ) = V1SAVE
                     VCSAVE( 2 ) = V2SAVE
                     VCSAVE( 3 ) = V3SAVE
                  END IF
                  CALL TLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1, T1COPY )
                  IF( K.GT.M ) THEN
                     SMALLA( 2, 1, KI ) = VCSAVE( 1 )
                     SMALLA( 3, 1, KI ) = DD_ZERO
                     IF( K.LT.I-1 )
     $                  SMALLA( 4, 1, KI ) = DD_ZERO
                  ELSE IF( M.GT.L ) THEN
                     SMALLA( 2, 1, KI ) = -SMALLA( 2, 1, KI )
                  END IF
                  V2 = VCSAVE( 2 )
                  T2 = T1COPY*V2
                  WORK( VECSIDX+( K-1 )*3+1 ) = VCSAVE( 2 )
                  WORK( VECSIDX+( K-1 )*3+2 ) = VCSAVE( 3 )
...
*
     ...
     $             ( ISTART.LE.MIN( I, ISTOP ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL TCOPY( NR, SMALLA( 3, 2, KI ), 1, VCSAVE, 1 )
                  ELSE
                     VCSAVE( 1 ) = V1SAVE
                     VCSAVE( 2 ) = V2SAVE
                     VCSAVE( 3 ) = V3SAVE
                  ...
                  CALL TLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1, T1COPY )
                  IF( K.GT.M ) THEN
                     SMALLA( 3, 2, KI ) = VCSAVE( 1 )
                     SMALLA( 4, 2, KI ) = DD_ZERO
                     IF( K.LT.I-1 )
     $                  SMALLA( 5, 2, KI ) = DD_ZERO
*
...
*
                  ...
                  ICOL1 = LOCALK2( KI )
                  IF( ISTART.GT.M ) THEN
                     VCSAVE( 1 ) = SMALLA( 4, 3, KI )
                     VCSAVE( 2 ) = SMALLA( 5, 3, KI )
                     VCSAVE( 3 ) = SMALLA( 6, 3, KI )
                     NR = MIN( 3, I-ISTART+1 )
                     CALL TLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1,
     $                            T1COPY )
                     A( ( ICOL1-2 )*LDA+IROW1 ) = VCSAVE( 1 )
                     A( ( ICOL1-2 )*LDA+IROW1+1 ) = DD_ZERO
                     IF( ISTART.LT.I-1 ) THEN
                        A( ( ICOL1-2 )*LDA+IROW1+2 ) = DD_ZERO
                     END IF
                  ELSE
                     IF( M.GT.L ) THEN
...
*
                     ...
                     IF( K.GT.M ) THEN
                        IF( MOD( K-1, HBL ).EQ.0 ) THEN
                           ...
                           VCSAVE( 3 ) = SMALLA( 6, 3, KI )
                        ELSE
                           VCSAVE( 1 ) = A( ( ICOL1-2 )*LDA+IROW1 )
                           VCSAVE( 2 ) = A( ( ICOL1-2 )*LDA+IROW1+1 )
                           IF( NR.EQ.3 ) THEN
                              VCSAVE( 3 ) = A( ( ICOL1-2 )*LDA+IROW1+2 )
                           END IF
                        END IF
                     ELSE
                        VCSAVE( 1 ) = V1SAVE
...
*
     ...
     $                ( NPROW.GT.1 ) .AND. ( ISTART.LE.ISTOP ) ) THEN
                     ...
                     IF( MYROW.NE.JJ ) THEN
                        CALL TGEBR2D( CONTXT, 'COL', ' ',
     $                                3*( ISTOP-ISTART+1 ), 1,
     $                                WORK( VECSIDX+( ISTART-1 )*3+1 ),
     $                                3*( ISTOP-ISTART+1 ), JJ, MYCOL )
                     ELSE
                        CALL TGEBS2D( CONTXT, 'COL', ' ',
     $                                3*( ISTOP-ISTART+1 ), 1,
     $                                WORK( VECSIDX+( ISTART-1 )*3+1 ),
     $                                3*( ISTOP-ISTART+1 ) )
                     END IF
...
*
                        T2 = T1*V2
                        T3 = T1*V3
                        CALL TLAREF( 'Col', A, LDA, .FALSE., Z, LDZ,
     ...
     $                      HBL ).LT.HBL-2 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL TLAREF( 'Row', A, LDA, .FALSE., Z, LDZ,
...
*
     ...
     $                          ITMP1, ITMP2 )
                  ...
                  II = KROW( KI )
                  CALL TLAREF( 'Row', A, LDA, WANTZ, Z, LDZ, .TRUE., II,
...
*
     ...
     $                      ( NPROW.EQ.1 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL TLAREF( 'Row', A, LDA, WANTZ, Z, LDZ,
     $                                  .FALSE., IROW1, IROW1, ISTART,
     ...
     $                      ( NPROW.GT.1 ) ) THEN
                           IF( IROW1.EQ.IROW2 ) THEN
                              CALL TGESD2D( CONTXT, 1, ICOL2-ICOL1+1,
     $                                      A( ( ICOL1-1 )*LDA+IROW2 ),
     $                                      LDA, UP, MYCOL )
                           END IF
                        END IF
...
*
     ...
     $                      ( NPROW.GT.1 ) ) THEN
                           IF( IROW1.NE.IROW2 ) THEN
                              CALL TGERV2D( CONTXT, 1, ICOL2-ICOL1+1,
     $                                      WORK( IRBUF+1 ), 1, DOWN,
     $                                      MYCOL )
                              T2 = T1*V2
                              T3 = T1*V3
...
*
     ...
     $                 IROW1, IROW2 )
                  ...
                  ELSE
                     ITMP2 = -1
                  END IF
                  CALL TLAREF( 'Col', A, LDA, WANTZ, Z, LDZ, .TRUE.,
...
*
     ...
     $                   ( NPCOL.GT.1 ) ) THEN
                        IF( ICOL1.EQ.ICOL2 ) THEN
                           CALL TGESD2D( CONTXT, IROW2-IROW1+1, 1,
     $                                   A( ( ICOL1-1 )*LDA+IROW1 ),
     $                                   LDA, MYROW, LEFT )
                           CALL TGERV2D( CONTXT, IROW2-IROW1+1, 1,
     $                                   A( ( ICOL1-1 )*LDA+IROW1 ),
     $                                   LDA, MYROW, LEFT )
                        ELSE
                           CALL TGERV2D( CONTXT, IROW2-IROW1+1, 1,
...
*
  ...
  340                      CONTINUE
                        ELSE
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL TGERV2D( CONTXT, 1, LIHIH-LILOH+1,
     $                                      WORK( IRBUF+1 ), 1, UP,
     $                                      MYCOL )
     ...
     $                              LDA+ITMP1 ) - SUM*T2
  350                         CONTINUE
                              CALL TGESD2D( CONTXT, 1, LIHIH-LILOH+1,
     $                                      WORK( IRBUF+1 ), 1, UP,
     $                                      MYCOL )
                           END IF
                        END IF
...
*                 LOCAL A(LILOZ:LIHIZ,LOCALK2:LOCALK2+2)
  ...
  360                      CONTINUE
                        ELSE
                           ITMP1 = LOCALK2( KI )
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL TGERV2D( CONTXT, LIHIH-LILOH+1, 1,
     $                                      WORK( ICBUF+1 ),
  ...
  370                         CONTINUE
                              CALL TGESD2D( CONTXT, LIHIH-LILOH+1, 1,
     $                                      WORK( ICBUF+1 ),
     $                                      LIHIH-LILOH+1, MYROW, LEFT )
                           END IF
                        END IF
...
*                       IF WE ACTUALLY OWN COLUMN K
                              IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                                 CALL TGERV2D( CONTXT, LIHIZ-LILOZ+1, 1,
     $                                         WORK( ICBUF+1 ), LDZ,
     $                                         MYROW, LEFT )
                                 ITMP1 = ( ITMP1-1 )*LDZ
  ...
  390                            CONTINUE
                                 CALL TGESD2D( CONTXT, LIHIZ-LILOZ+1, 1,
     $                                         WORK( ICBUF+1 ), LDZ,
     $                                         MYROW, LEFT )
                                 LOCALK2( KI ) = LOCALK2( KI ) + 1
                              END IF
...
*
         CALL PTELGET( 'All', ' ', H11, A, L, L, DESCA )
         CALL PTELGET( 'All', ' ', H21, A, I, L, DESCA )
         CALL PTELGET( 'All', ' ', H12, A, L, I, DESCA )
         CALL PTELGET( 'All', ' ', H22, A, I, I, DESCA )
         CALL TLANV2( H11, H12, H21, H22, WR( L ), WI( L ), WR( I ),
     $                WI( I ), CS, SN )
         CALL PTELSET( A, L, L, DESCA, H11 )
         CALL PTELSET( A, I, L, DESCA, H21 )
         CALL PTELSET( A, L, I, DESCA, H12 )
         CALL PTELSET( A, I, I, DESCA, H22 )
*
*        Transform H to the standard Schur form
*
         IF( WANTT ) THEN
            IF(I .LT. N) CALL PTROT( N-I, A, L, I+1, DESCA, DESCA( M_ ),
     ...
     $                               LWORK-VECSIDX, IERR )
         ...
         ELSE
            LTOP = I1
         END IF
         IF (L .GT. LTOP) CALL PTROT( L-LTOP, A, LTOP, L, DESCA, 1, A,
     $                                LTOP, I, DESCA, 1, CS, SN,
     $                                WORK( VECSIDX+1 ), LWORK-VECSIDX,
     $                                IERR )
         IF( WANTZ ) THEN
            CALL PTROT( IHIZ-ILOZ+1, Z, ILOZ, L, DESCZ, 1, Z, ILOZ, I,
...
*
         NH = I - L + 1
         IF( NH .LE. LDS ) THEN
            CALL PTLAQR4( WANTT, WANTZ, N, L, I, A, DESCA, WR, WI,
     $                    ILOZ, IHIZ, Z, DESCZ, WORK( S1+1 ), NH,
     $                    WORK( S2+1 ), NH, WORK( S3+1 ), 4*LDS*LDS,
     $                    INFO )
            IF( INFO.NE.0 ) THEN
...
*
      IF( NUM.GT.1 ) THEN
         CALL TGSUM2D( CONTXT, 'All', ' ', IHI-ILO+1, 1, WR(ILO), N,
     $        -1, -1 )
         CALL TGSUM2D( CONTXT, 'All', ' ', IHI-ILO+1, 1, WI(ILO), N,
     $        -1, -1 )
      END IF
*
      WORK( 1 ) = real64x2( LWKOPT )
