      SUBROUTINE PVLAHQR( WANTT, WANTZ, N, ILO, IHI, A, DESCA, W, ILOZ,
     $                    IHIZ, Z, DESCZ, WORK, LWORK, IWORK, ILWORK,
     $                    INFO )
      USE multifloats, only: abs, aimag, cmplx64x2, conjg, max, min,
     +mod, real64x2, operator(+), operator(-), operator(*), operator(/),
...
*     .. Array Arguments ..
      ...
      TYPE(cmplx64x2)            A( * ), W( * ), WORK( * ), Z( * )
*     ..
*
*  Purpose
*  =======
*
*  PVLAHQR is an auxiliary routine used to find the Schur decomposition
...
*          ILO = 1). PVLAHQR works primarily with the Hessenberg
...
*          = 0: successful exit
*          > 0: PVLAHQR failed to compute all the eigenvalues ILO to IHI
*               in a total of 30*(IHI-ILO+1) iterations; if INFO = i,
*               elements i+1:ihi of W contains those eigenvalues
*               which have been successfully computed.
*
*  Logic:
*       This algorithm is very similar to TLAHQR.  Unlike TLAHQR,
...
*           VLAHQR     -> Serial QR used to determine shifts and
*                         eigenvalues
*           VLARFG     -> Determine the Householder transforms
*
*       This ScaLAPACK, this routine calls:
*           PVLACONSB  -> To determine where to start each iteration
*           VLAMSH     -> Sends multiple shifts through a small
*                         submatrix to see how the consecutive
*                         subdiagonals change (if PVLACONSB indicates
*                         we can start a run in the middle)
*           PVLAWIL    -> Given the shift, get the transformation
*           PVLACP3    -> Parallel array to local replicated array copy
*                         & back.
*           VLAREF     -> Row/column reflector applier.  Core routine
*                         here.
*           PVLASMSUB  -> Finds negligible subdiagonal elements.
*
...
*       9.) To optimize this for your architecture, try tuning VLAREF.
...
*     .. Parameters ..
     ...
     +                RSRC_ = 7,  CSRC_ = 8,  LLD_ = 9 )
      TYPE(real64x2)               RONE
      ! Converted to assignments below: PARAMETER          ( RONE = real64x2(limbs=[1.0D0, 0.0_8]) )
      TYPE(cmplx64x2)            ZERO, ONE
      ! Converted to assignments below: PARAMETER          ( ZERO = cmplx64x2(real64x2(limbs=[0.0D0, 0.0_8]),real64x2(limbs=[0.0D0, 0.0_8])),                    ONE = cmplx64x2(real64x2(limbs=[1.0D0, 0.0_8]),real64x2(limbs=[0.0D0, 0.0_8])) )
      TYPE(real64x2)               CONST
...
*     .. Local Scalars ..
     ...
     $                   NUM, NZ, RIGHT, ROTN, UP, VECSIDX
      TYPE(real64x2)               CS, OVFL, S, SMLNUM, ULP, UNFL
      TYPE(cmplx64x2)            CDUM, H10, H11, H22, H33, H43H34, H44, 
     +SN, SUM,
...
*     .. Local Arrays ..
     ...
     $                   KP2ROW( IBLK ), KROW( IBLK )
      TYPE(cmplx64x2)            S1( 2*IBLK, 2*IBLK ), SMALLA( 6, 6, 
     +IBLK ),
     $                   VCSAVE( 3 )
*     ..
*     .. External Functions ..
      ...
      EXTERNAL           ILCM, NUMROC, PTLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, IGAMN2D, IGEBR2D, IGEBS2D,
     $                   INFOG1L, INFOG2L, PTLABAD, PXERBLA, PVLACONSB,
     $                   PVLACP3, PVLASMSUB, PVLAWIL, PVROT, VCOPY,
     $                   VGEBR2D, VGEBS2D, VGERV2D, VGESD2D, VGSUM2D,
     $                   VLAHQR2, VLAMSH, VLANV2, VLAREF, VLARFG
*     ..
*     .. Intrinsic Functions ..
*
*     ..
...
*     .. Statement Function definitions ..
      CABS1( CDUM ) = ABS( real64x2( CDUM ) ) + ABS( AIMAG( CDUM ) )
*     ..
*     .. Executable Statements ..
*
      RONE = real64x2(limbs=[1.0D0, 0.0_8])
      ZERO = cmplx64x2(real64x2(limbs=[0.0D0, 0.0_8]),
     +real64x2(limbs=[0.0D0, 0.0_8]))
      ONE = cmplx64x2(real64x2(limbs=[1.0D0, 0.0_8]),
     +real64x2(limbs=[0.0D0, 0.0_8]))
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PVLAHQR', -INFO )
         RETURN
      END IF
*
...
*
         CALL PVLASMSUB( A, DESCA, I, L, K, SMLNUM, WORK( IRBUF+1 ),
     $                   LWORK-IRBUF )
         L = K
*
         IF( L.GT.ILO ) THEN
...
*
         ...
         JBLK = MAX( JBLK, 1 )
*
         CALL PVLACP3( 2*JBLK, I-2*JBLK+1, A, DESCA, S1, 2*IBLK, -1, -1,
     $                 0 )
...
*
   ...
   20       CONTINUE
         ...
         ELSE
            CALL VLAHQR2( .FALSE., .FALSE., 2*JBLK, 1, 2*JBLK, S1,
...
*           PVLACONSB is the routine that does this.
*
         CALL PVLACONSB( A, DESCA, I, L, M, H44, H33, H43H34,
...
*
     ...
     $                    IROW1, ICOL1, ITMP1, ITMP2 )
            ...
            CALL PVLACP3( II, M-1, A, DESCA, WORK( IRBUF+1 ), II, ITMP1,
     $                    ITMP2, 0 )
            IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) THEN
*
*              Find a new NBULGE based on the bulges we have.
*
               CALL VLAMSH( S1, 2*IBLK, NBULGE, JBLK, WORK( IRBUF+1 ),
     $                      II, II, ULP )
               IF( NUM.GT.1 ) THEN
...
*
   ...
   31    CONTINUE
         ITMP1 = ISTARTROW
         ITMP2 = ISTARTCOL
         CALL PVLAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33, H43H34,
     $                 VCSAVE )
         V1SAVE = VCSAVE( 1 )
         V2SAVE = VCSAVE( 2 )
         V3SAVE = VCSAVE( 3 )
...
*
   ...
   40       CONTINUE
     ...
     $                     S1( 2*JBLK-2*IBULGE, 2*JBLK-2*IBULGE-1 )
                  ITMP1 = ISTARTROW
                  ITMP2 = ISTARTCOL
                  CALL PVLAWIL( ITMP1, ITMP2, M, A, DESCA, H44, H33,
     $                          H43H34, VCSAVE )
                  V1SAVE = VCSAVE( 1 )
                  V2SAVE = VCSAVE( 2 )
                  V3SAVE = VCSAVE( 3 )
...
*
                     ITMP1 = ICURROW( KI )
                     ITMP2 = ICURCOL( KI )
                     CALL PVLACP3( MIN( 6, N-K+2 ), K-1, A, DESCA,
...
*
     ...
     $                             MYCOL, IROW1, ICOL1, ITMP1, ITMP2 )
                     CALL PVLACP3( MIN( 6, N-K+3 ), K-2, A, DESCA,
     $                             SMALLA( 1, 1, KI ), 6, ITMP1, ITMP2,
     $                             0 )
                  END IF
               END IF
*
*
*           VLAHQR used to have a single row application and a single
...
*
     ...
     $             ( ISTART.LT.MIN( I-1, ISTOP+1 ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL VCOPY( NR, SMALLA( 2, 1, KI ), 1, VCSAVE, 1 )
                  ELSE
                     VCSAVE( 1 ) = V1SAVE
                     VCSAVE( 2 ) = V2SAVE
                     VCSAVE( 3 ) = V3SAVE
                  END IF
                  CALL VLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1, T1COPY )
                  IF( K.GT.M ) THEN
     ...
     $                  SMALLA( 4, 1, KI ) = ZERO
                  ELSE IF( M.GT.L ) THEN
*
*                 Following differs in comparison to ptlahqr.
*
...
*
     ...
     $             ( ISTART.LE.MIN( I, ISTOP ) ) ) THEN
                  ...
                  NR = MIN( 3, I-K+1 )
                  IF( K.GT.M ) THEN
                     CALL VCOPY( NR, SMALLA( 3, 2, KI ), 1, VCSAVE, 1 )
                  ELSE
                     VCSAVE( 1 ) = V1SAVE
                     VCSAVE( 2 ) = V2SAVE
                     VCSAVE( 3 ) = V3SAVE
...
*
                  ...
                  ICOL1 = KCOL( KI )
*
*                 The ELSE part of this IF needs updated VCSAVE, this
*                 was not necessary in PTLAHQR.
*
                  IF( ISTART.GT.M ) THEN
                     ...
                     NR = MIN( 3, I-ISTART+1 )
                     CALL VLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1,
     $                            T1COPY )
                  ...
                  ELSE
*
*                    If NPCOL.NE.1 THEN we need updated VCSAVE.
*
                     NR = MIN( 3, I-ISTART+1 )
                     IF( NPCOL.EQ.1 ) THEN
                        VCSAVE( 1 ) = V1SAVE
                     ...
                     ELSE
*
*                    Get updated VCSAVE from RIGHT
*
                        CALL VGERV2D( CONTXT, 3, 1, VCSAVE, 3, MYROW,
     $                                RIGHT )
                     END IF
                     CALL VLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1,
     $                            T1COPY )
...
*
                     ...
                     END IF
*
*                    Must send uptodate copy of VCSAVE to left.
*
                     IF( NPCOL.GT.1 .AND. ISTART.LE.M .AND.
     $                   MOD( K-1, HBL ).EQ.0 ) THEN
                        CALL VGESD2D( CONTXT, 3, 1, VCSAVE, 3, MYROW,
     $                                LEFT )
                     END IF
                     CALL VLARFG( NR, VCSAVE( 1 ), VCSAVE( 2 ), 1,
     $                            T1COPY )
...
*
     ...
     $             ( ISTART.LE.ISTOP ) ) THEN
                  IF( MYROW.NE.ICURROW( KI ) ) THEN
                     CALL VGEBR2D( CONTXT, 'COL', ' ',
     $                             3*( ISTOP-ISTART+1 ), 1,
     $                             WORK( VECSIDX+( ISTART-1 )*3+1 ),
     $                             3*( ISTOP-ISTART+1 ), ICURROW( KI ),
     $                             MYCOL )
                  ELSE
                     CALL VGEBS2D( CONTXT, 'COL', ' ',
     $                             3*( ISTOP-ISTART+1 ), 1,
     $                             WORK( VECSIDX+( ISTART-1 )*3+1 ),
     $                             3*( ISTOP-ISTART+1 ) )
                  END IF
...
*
                        T3 = T1*V3
                        CALL VLAREF( 'Col', A, LDA, .FALSE., Z, LDZ,
     ...
     $                      HBL ).LT.HBL-2 ) ) THEN
                           T3 = T1*V3
                           CALL VLAREF( 'Row', A, LDA, .FALSE., Z, LDZ,
...
*
     ...
     $                          ITMP1, ITMP2 )
                  ITMP2 = LOCALI2
                  II = KROW( KI )
                  CALL VLAREF( 'Row', A, LDA, WANTZ, Z, LDZ, .TRUE., II,
...
*
     ...
     $                      ( NPROW.EQ.1 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL VLAREF( 'Row', A, LDA, WANTZ, Z, LDZ,
     $                                  .FALSE., IROW1, IROW1, ISTART,
     ...
     $                      ( NPROW.GT.1 ) ) THEN
                           IF( IROW1.NE.IROW2 ) THEN
                              CALL VGESD2D( CONTXT, 2, ICOL2-ICOL1+1,
     $                                      A( ( ICOL1-1 )*LDA+IROW1 ),
     $                                      LDA, DOWN, MYCOL )
                              IF( SKIP .AND. ( ISTART.EQ.ISTOP ) ) THEN
                                 CALL VGERV2D( CONTXT, 2, ICOL2-ICOL1+1,
     $                                         A( ( ICOL1-1 )*LDA+
     $                                         IROW1 ), LDA, DOWN,
     $                                         MYCOL )
                              END IF
...
*
     ...
     $                          IROW2 )
                  ...
                  ELSE
                     ITMP2 = -1
                  END IF
                  CALL VLAREF( 'Col', A, LDA, WANTZ, Z, LDZ, .TRUE.,
...
*
     ...
     $                   ( NPCOL.GT.1 ) ) THEN
                        IF( ICOL1.NE.ICOL2 ) THEN
                           CALL VGESD2D( CONTXT, IROW2-IROW1+1, 2,
     $                                   A( ( ICOL1-1 )*LDA+IROW1 ),
     $                                   LDA, MYROW, RIGHT )
                           IF( ( ISTART.EQ.ISTOP ) .AND. SKIP ) THEN
                              CALL VGERV2D( CONTXT, IROW2-IROW1+1, 2,
     $                                      A( ( ICOL1-1 )*LDA+IROW1 ),
     $                                      LDA, MYROW, RIGHT )
                           END IF
                        ELSE IF( SKIP ) THEN
...
*
  ...
  460                      CONTINUE
                        ELSE
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL VGERV2D( CONTXT, 1, LIHIH-LILOH+1,
     $                                      WORK( IRBUF+1 ), 1, UP,
     $                                      MYCOL )
     ...
     $                              LDA+ITMP1 ) - SUM*V2
  470                         CONTINUE
                              CALL VGESD2D( CONTXT, 1, LIHIH-LILOH+1,
     $                                      WORK( IRBUF+1 ), 1, UP,
     $                                      MYCOL )
                           END IF
                        END IF
...
*                       LOCAL A(LILOZ:LIHIZ,KCOL:KCOL+2)
  ...
  480                      CONTINUE
                        ELSE
                           ITMP1 = KCOL( KI )
                           IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                              CALL VGERV2D( CONTXT, LIHIH-LILOH+1, 1,
     $                                      WORK( ICBUF+1 ),
     ...
     $                              SUM*CONJG( V2 )
  490                         CONTINUE
                              CALL VGESD2D( CONTXT, LIHIH-LILOH+1, 1,
     $                                      WORK( ICBUF+1 ),
     $                                      LIHIH-LILOH+1, MYROW, LEFT )
                           END IF
                        END IF
...
*                             IF WE ACTUALLY OWN COLUMN K
                              IF( MOD( K-1, HBL ).EQ.HBL-1 ) THEN
                                 CALL VGERV2D( CONTXT, LIHIZ-LILOZ+1, 1,
     $                                         WORK( IZBUF+1 ), LDZ,
     $                                         MYROW, LEFT )
                                 ITMP1 = ( ITMP1-1 )*LDZ
     ...
     $                                             SUM*CONJG( V2 )
  510                            CONTINUE
                                 CALL VGESD2D( CONTXT, LIHIZ-LILOZ+1, 1,
     $                                         WORK( IZBUF+1 ), LDZ,
     $                                         MYROW, LEFT )
                              END IF
                           END IF
...
*
         CALL PVLACP3( 2, I-1, A, DESCA, S1, 2*IBLK, -1, -1, 0 )
         CALL VLANV2( S1( 1, 1 ), S1( 1, 2 ), S1( 2, 1 ), S1( 2, 2 ),
     $                W( I-1 ), W( I ), CS, SN )
         CALL PVLACP3( 2, I-1, A, DESCA, S1, 2*IBLK, 0, 0, 1 )
*
         IF( NODE.NE.0 ) THEN
...
*
            IF( I2.GT.I ) THEN
               CALL PVROT( I2-I, A, I-1, I+1, DESCA, N, A, I, I+1,
     $                     DESCA, N, CS, SN )
            END IF
            CALL PVROT( I-I1-1, A, I1, I-1, DESCA, 1, A, I1, I, DESCA,
     $                  1, CS, CONJG( SN ) )
         END IF
         IF( WANTZ ) THEN
*
...
*
            CALL PVROT( NZ, Z, ILOZ, I-1, DESCZ, 1, Z, ILOZ, I, DESCZ,
     $                  1, CS, CONJG( SN ) )
         END IF
*
      ELSE
...
*
         ...
         IF( JBLK.LE.2*IBLK ) THEN
            CALL PVLACP3( I-L+1, L, A, DESCA, S1, 2*IBLK, 0, 0, 0 )
            CALL VLAHQR2( .FALSE., .FALSE., JBLK, 1, JBLK, S1, 2*IBLK,
...
*
  570 CONTINUE
      CALL VGSUM2D( CONTXT, 'All', ' ', N, 1, W, N, -1, -1 )
      RETURN
*
*     END OF PVLAHQR
*
