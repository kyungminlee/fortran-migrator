      SUBROUTINE PTLAHQR( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,
     $                    ILOZ, IHIZ, Z, DESCZ, WORK, LWORK, IWORK,
     $                    ILWORK, INFO )
      USE multifloats, only: abs, max, min, mod, real64x2, sign, sqrt,
     +dd_half, dd_one, dd_zero, operator(+), operator(-), operator(*),
...
*  =======
*
*  PTLAHQR is an auxiliary routine used to find the Schur decomposition
...
*          ILO = 1). PTLAHQR works primarily with the Hessenberg
...
*          = 0: successful exit
*          > 0: PTLAHQR failed to compute all the eigenvalues ILO to IHI
...
*  Subroutines:
*       This routine calls:
*           PTLACONSB   -> To determine where to start each iteration
...
*                        back.
*           TLAREF   -> Row/column reflector applier.  Core routine
*                        here.
...
*     .. Local Arrays ..
     ...
     $                   KP2ROW( IBLK ), KROW( IBLK ), LOCALK2( IBLK )
      TYPE(real64x2)   S1( 2*IBLK, 2*IBLK ), SMALLA( 6, 6, IBLK ),
     $                   VCSAVE( 3 )
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*     .. External Subroutines ..
     ...
     $                   PTLABAD, PTLACONSB, PTLACP3, PTLASMSUB,
     $                   PTLAWIL, PXERBLA, TLANV2
*     ..
*     .. Intrinsic Functions ..
*     ..
...
*
     ...
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PTLAHQR', -INFO )
         RETURN
      END IF
*
...
*
         ...
         JBLK = MAX( JBLK, 1 )
*
         CALL PTLACP3( 2*JBLK, I-2*JBLK+1, A, DESCA, S1, 2*IBLK, -1, -1,
     $                 0 )
...
*
   ...
   20       CONTINUE
         ...
         ELSE
            CALL TLAHQR( .FALSE., .FALSE., 2*JBLK, 1, 2*JBLK, S1,
...
*
c         CALL PTLACONSB( A, DESCA, I, L, M, H44, H33, H43H34,
...
*
         ...
         IF( JBLK.LE.2*IBLK ) THEN
            CALL PTLACP3( I-L+1, L, A, DESCA, S1, 2*IBLK, 0, 0, 0 )
            CALL TLAHQR( .FALSE., .FALSE., JBLK, 1, JBLK, S1, 2*IBLK,
...
*
  450 CONTINUE
      CALL TGSUM2D( CONTXT, 'All', ' ', N, 1, WR, N, -1, -1 )
      CALL TGSUM2D( CONTXT, 'All', ' ', N, 1, WI, N, -1, -1 )
      RETURN
*
*     END OF PTLAHQR
*
