      INTEGER          FUNCTION PJLAENV_EP( ICTXT, ISPEC, NAME, OPTS,
     $                 N1, N2, N3, N4 )
      IMPLICIT NONE
*
...
*          < 0:  if PJLAENV_EP = -k, the k-th argument had an illegal
*          value.
*
...
*
      C1 = SUBNAM( 2: 2 )
*     Recognize the migrated extended-precision letters in addition to
*     S/D/C/Z: real targets emit precision letter E (kind10), Q (kind16),
*     M (multifloats); complex targets emit Y (kind10), X (kind16), W
*     (multifloats). Callers like PESYEVX pass a migrated routine name
*     (e.g. 'PESYTTRD') to PJLAENV_EP; the upstream PJLAENV's strict
*     S/D/C/Z gate would early-return uninitialized for those, leading
*     to garbage block-size advice and downstream LWORK overflow.
...
*
      IF( GLOBAL ) THEN
         IDUMM = 0
         CALL IGAMX2D( ICTXT, 'All', ' ', 1, 1, PJLAENV_EP, 1, IDUMM,
     $                 IDUMM, -1, -1, IDUMM )
      END IF
*
*
