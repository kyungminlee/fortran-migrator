...
*     .. External Functions ..
      ...
      EXTERNAL           ICEIL, PDLAMCH, INDXG2P, LSAME, NUMROC, PDLANSY
*     ..
*     .. Intrinsic Functions ..
...
*           PDPOCON / PDPORFS calls (see comment at line 311 of the
*           upstream prologue documenting the intended formula).
*           Upstream sets LWMIN = 3*DESCA(LLD_), which is insufficient
*           and causes PDPOCON to abort with INFO=-10.
*           PDPOCON LWMIN  = 2*NPMOD + 2*NQMOD +
*                            MAX(2, MAX(NB*ICEIL(NPROW-1,NPCOL),
*                                       NQMOD + NB*ICEIL(NPCOL-1,NPROW)))
*           PDPORFS LWMIN  = 3*NPMOD
*           NPMOD/NQMOD are the unadjusted NUMROCs (PDPOCON's
