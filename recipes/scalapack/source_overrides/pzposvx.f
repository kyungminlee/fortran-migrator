      SUBROUTINE PZPOSVX( FACT, UPLO, N, NRHS, A, IA, JA, DESCA, AF,
...
*     .. Local Scalars ..
     ...
     $                   IXROW, J, JJA, JJB, JJX, LDB, LDX, LRWMIN,
...
*     .. External Subroutines ..
     ...
     $                   PXERBLA, PZPOCON, PZPOEQU,
     $                   PZPORFS, PZPOTRF, PZPOTRS,
     $                   PZLACPY, PZLAQSY
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      ...
      EXTERNAL           ICEIL, PDLAMCH, INDXG2P, LSAME, NUMROC, PZLANHE
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $         NP = NP-IROFFA
            ...
            IF( MYCOL.EQ.IACOL )
     $         NQ = NQ-ICOFFA
*           Upstream bug fix: LWMIN/LRWMIN must accommodate the internal
*           PZPOCON / PZPORFS calls (see comment at line 311 of the
*           upstream prologue documenting the intended formula).
*           Upstream sets LWMIN = 3*DESCA(LLD_) and LRWMIN = MAX(2*NQ,
*           NP), both insufficient — PZPOCON aborts with INFO=-10.
*           PZPOCON LWMIN  = 2*NPMOD + MAX(2, MAX(NB*ICEIL(NPROW-1,
*                                                          NPCOL),
*                                          NQMOD + NB*ICEIL(NPCOL-1,
*                                                           NPROW)))
*           PZPOCON LRWMIN = 2*NQMOD
*           PZPORFS LWMIN  = 2*NPMOD,   LRWMIN = NPMOD
*           PZLANHE('1') needs RWORK = 2*NQMOD + NPMOD + LDW (LDW=0 on
*           square grids); upstream's LRWMIN = MAX(2*NQ, NP) is too
*           small here too — pzlanhe scribbles past the buffer and
*           triggers free()-time heap corruption a few iterations later.
*           NPMOD/NQMOD are the unadjusted NUMROCs (PZPOCON's own
*           definition).
     ...
     $              2*NPMOD )
*           LDW term for non-square grids: MB_A*CEIL(CEIL(NPMOD/MB_A) /
*           (LCM/NPROW)). Conservatively use NPMOD as an upper bound
*           (it's always >= the LDW term).
            LRWMIN = MAX( 2*NQMOD + NPMOD + NPMOD,
     $                    2*NQMOD,
     $                    NPMOD,
     $                    NQ )
            NOFACT = LSAME( FACT, 'N' )
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPOSVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PZPOEQU( N, A, IA, JA, DESCA, SR, SC, SCOND, AMAX,
     $                 INFEQU )
*
         IF( INFEQU.EQ.0 ) THEN
*
...
*
            CALL PZLAQSY( UPLO, N, A, IA, JA, DESCA, SR, SC, SCOND,
     $                    AMAX, EQUED )
            RCEQU = LSAME( EQUED, 'Y' )
         END IF
      END IF
...
*
         CALL PZLACPY( 'Full', N, N, A, IA, JA, DESCA, AF, IAF, JAF,
     $                 DESCAF )
         CALL PZPOTRF( UPLO, N, AF, IAF, JAF, DESCAF, INFO )
*
...
*
      ANORM = PZLANHE( '1', UPLO, N, A, IA, JA, DESCA, RWORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PZPOCON( UPLO, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,
...
*
      CALL PZLACPY( 'Full', N, NRHS, B, IB, JB, DESCB, X, IX, JX,
     $              DESCX )
      CALL PZPOTRS( UPLO, N, NRHS, AF, IAF, JAF, DESCAF, X, IX, JX,
     $              DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PZPORFS( UPLO, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,
