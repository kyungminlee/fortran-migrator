      SUBROUTINE PDSYNTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,
     $                     LWORK, INFO )
      IMPLICIT NONE
*
...
*
*
*  PDSYNTRD reduces a real symmetric matrix sub( A ) to symmetric
...
*            ICTXT = DESCA( CTXT_ )
*            ANB = PJLAENV_EP( ICTXT, 3, 'PDSYTTRD', 'L', 0, 0, 0, 0 )
*            SQNPC = INT( SQRT( DBLE( NPROW * NPCOL ) ) )
*            NPS = MAX( NUMROC( N, 1, 0, 0, SQNPC ), 2*ANB )
*
*            NUMROC is a ScaLAPACK tool functions;
*            PJLAENV_EP is a ScaLAPACK envionmental inquiry function
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            INDXG2L, INDXG2P, NUMROC, PJLAENV_EP
      EXTERNAL           LSAME, INDXG2L, INDXG2P, NUMROC, PJLAENV_EP
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $           NPCOL ) )
            ...
            ANB = PJLAENV_EP( ICTXT, 3, 'PDSYTTRD', 'L', 0, 0, 0, 0 )
            MINSZ = PJLAENV_EP( ICTXT, 5, 'PDSYTTRD', 'L', 0, 0, 0, 0 )
            SQNPC = INT( SQRT( DBLE( NPROW*NPCOL ) ) )
            NPS = MAX( NUMROC( N, 1, 0, 0, SQNPC ), 2*ANB )
...
*
*
*     Use the serial, LAPACK, code:  DTRD on small matrices if we
...
*
         CALL PDTRMR2D( UPLO, 'N', N, N, A, IA, JA, DESCA, WORK( INDB ),
     $                  1, 1, DESCB, ICTXT )
*
*
...
*
            IF( NPROWB.EQ.1 ) THEN
               CALL DSYTRD( UPLO, N, WORK( INDB ), NPS, WORK( INDD ),
     $                      WORK( INDE ), WORK( INDTAU ), WORK( INDW ),
     $                      LLWORK, INFO )
            ELSE
*
               CALL PDSYTTRD( 'L', N, WORK( INDB ), 1, 1, DESCB,
     $                        WORK( INDD ), WORK( INDE ),
...
*           way that PDSYNTRD expects it.
*
         CALL PDLAMR1D( N-1, WORK( INDE ), 1, 1, DESCB, E, 1, JA,
     $                  DESCA )
*
         CALL PDLAMR1D( N, WORK( INDD ), 1, 1, DESCB, D, 1, JA, DESCA )
*
         CALL PDLAMR1D( N, WORK( INDTAU ), 1, 1, DESCB, TAU, 1, JA,
     $                  DESCA )
*
         CALL PDTRMR2D( UPLO, 'N', N, N, WORK( INDB ), 1, 1, DESCB, A,
     $                  IA, JA, DESCA, ICTXT )
*
         IF( MYROWB.GE.0 )
...
*           the matrix
*
               CALL PDLATRD( UPLO, K+JB-1, JB, A, IA, JA, DESCA, D, E,
...
*
               CALL PDSYR2K( UPLO, 'No transpose', K-1, JB, -ONE, A, IA,
...
*
               CALL PDSYR2K( UPLO, 'No transpose', N-K-NB+1, NB, -ONE,
...
*        Use unblocked code to reduce the last or only block
*
            CALL PDSYTD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, D, E, TAU,
     $                    WORK, LWORK, IINFO )
         END IF
*
