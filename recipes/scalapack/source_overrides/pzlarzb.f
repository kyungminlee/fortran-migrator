...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZLARZB', -INFO )
...
*        M+ICOFFV as PBZTRAN's N argument while sizing WORK( IPV ) / LV
*        from MPC20 = local rows of L. Whenever M > L (the typical case
*        — M = K+L from PZUNMRZ's caller), PBZTRAN's LDC contract
...
*        value") *without* writing WORK( IPV ), and the outer ZGEMM at
...
*        correct fix is to conjugate-transpose the K x (L+ICOFFV) block
*        instead of the K x (M+ICOFFV) block. The MPC20-sized IPV
*        buffer then matches PBZTRAN's NP requirement under the
*        alignment hypothesis IROFFC2 = ICOFFV (NB_V = MB_C). Documented
*        in doc/UPSTREAM_BUGS.md.
*
...
*
         IF( MYROW.EQ.IVROW ) THEN
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL ZLAMOV( 'All', K, NQV, V( IOFFV ), LDV,
...
*
     ...
     $                 L+ICOFFV, DESCV( NB_ ), WORK( IPW ), LW, ZERO,
...
*
         IF( MPC2.GT.0 ) THEN
            CALL ZGEMM( 'Transpose', 'No transpose', NQC2, K, MPC2,
     $                  ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,
     $                  WORK( IPW ), LW )
         ELSE
            CALL ZLASET( 'All', NQC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   10       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBZMATADD( ICTXT, 'Transpose', NQC2, IINXT-IIBEG+1,
     ...
     $                         WORK( IPW+ITOP ), LW )
         ...
         END IF
*
         CALL ZGSUM2D( ICTXT, 'Columnwise', ' ', NQC2, K, WORK( IPW ),
     $                 LW, IVROW, MYCOL )
*
...
*
               CALL ZTRBS2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     $                       K, K, T, MBV )
            ELSE
               CALL ZTRBR2D( ICTXT, 'Rowwise', ' ', 'Lower', 'Non unit',
     $                       K, K, T, MBV, MYROW, IVCOL )
            END IF
            CALL ZTRMM( 'Right', 'Lower', TRANST, 'Non unit', NQC2, K,
     $                  ONE, T, MBV, WORK( IPW ), LW )
*
            CALL ZGEBS2D( ICTXT, 'Columnwise', ' ', NQC2, K,
     $                    WORK( IPW ), LW )
         ELSE
            CALL ZGEBR2D( ICTXT, 'Columnwise', ' ', NQC2, K,
...
*
   20       CONTINUE
            IF( IIBEG.LE.IINXT ) THEN
               CALL PBZMATADD( ICTXT, 'Transpose', IINXT-IIBEG+1, NQC2,
...
*
         DO 30 J = 1, K
            CALL ZLACGV( MPC2, WORK( IPV+(J-1)*LV ), 1 )
   30    CONTINUE
         CALL ZGEMM( 'No transpose', 'Transpose', MPC2, NQC2, K, -ONE,
...
*
         ...
         IF( MYROW.EQ.IVROW ) THEN
            CALL ZGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC2,
     $                    V( IOFFV ), LDV )
            IF( MYCOL.EQ.IVCOL )
     ...
     $                    WORK( IPV ), LV, IVROW, MYCOL )
            IF( MYCOL.EQ.IVCOL )
     $         CALL ZTRBR2D( ICTXT, 'Columnwise', COLBTOP, 'Lower',
...
*
         IF( NQC2.GT.0 ) THEN
            CALL ZGEMM( 'No Transpose', 'Transpose', MPC2, K, NQC2,
     $                 ONE, C( IOFFC2 ), LDC, WORK( IPV ), LV, ZERO,
     $                 WORK( IPW ), LW )
         ELSE
            CALL ZLASET( 'All', MPC2, K, ZERO, ZERO, WORK( IPW ), LW )
         END IF
*
...
*
   40       CONTINUE
            IF( JJBEG.LE.JJNXT ) THEN
               CALL PBZMATADD( ICTXT, 'No transpose', MPC2,
     $                         JJNXT-JJBEG+1, ONE,
     ...
     $                         WORK( IPW+ILEFT*LW ), LW )
         ...
         END IF
*
         CALL ZGSUM2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),
     $                 LW, MYROW, IVCOL )
*
*        WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
         IF( MYCOL.EQ.IVCOL ) THEN
            DO 50 J = 1, K
               CALL ZLACGV( K-J+1, T( J+(J-1)*MBV ), 1 )
   50       CONTINUE
            CALL ZTRMM( 'Right', 'Lower', TRANS, 'Non unit', MPC2, K,
     $                  ONE, T, MBV, WORK( IPW ), LW )
            CALL ZGEBS2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),
     $                    LW )
            DO 60 J = 1, K
               CALL ZLACGV( K-J+1, T( J+(J-1)*MBV ), 1 )
   60       CONTINUE
         ELSE
            CALL ZGEBR2D( ICTXT, 'Rowwise', ' ', MPC2, K, WORK( IPW ),
     $                    LW, MYROW, IVCOL )
         END IF
*
*        C1 = C1 - WORK( IPW )
...
*
         DO 80 J = 1, NQC2
            CALL ZLACGV( K, WORK( IPV+(J-1)*LV ), 1 )
   80    CONTINUE
         IF( IOFFC2.GT.0 )
     $      CALL ZGEMM( 'No transpose', 'No transpose', MPC2, NQC2, K,
