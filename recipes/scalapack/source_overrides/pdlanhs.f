      DOUBLE PRECISION   FUNCTION PDLANHS( NORM, N, A, IA, JA, DESCA,
     $                                     WORK )
      IMPLICIT NONE
*
...
*  =======
*
*  PDLANHS returns the value of the one norm, or the Frobenius norm,
...
*
   ...
   20          CONTINUE
            ...
            END IF
*
*           Bug fix (vs upstream ScaLAPACK 2.2.3): the NPROW=1 path
*           failed to advance II after the first column block, so the
*           inner-loop bound II+LL-JJ+1 collapsed once JJ moved past
*           the first MB. M-norm passed by luck (max element typically
*           lay in the kept range); 1/F/I norms underestimated by
*           10-20%. Mirror the II = II + JB update that the NPROW>1
*           branch already performs after its first-block code.
            II = II + JB
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 440 LL = JJ, JJ+JB-1
                  CALL DLASSQ( MIN( II+LL-JJ+1, IIA+NP-1 )-IIA+1,
  ...
  470             CONTINUE
               ELSE
                  DO 480 LL = JJ, JJ + JB -1
                     CALL DLASSQ( MIN( II-1, IIA+NP-1 )-IIA+1,
     $                            A( IIA+IOFFA ), 1, SCALE, SUM )
                     IOFFA = IOFFA + LDA
  480             CONTINUE
                  IF( MYROW.EQ.INXTROW .AND. II.LE.IIA+NP-1 )
