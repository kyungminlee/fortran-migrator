...
*        from procrow ICROW2 with offset IROFFC2), since PBDTRNV transposes
*        V into a column at ICROW2 and DGEMV/DGER below act on sub(C2).
...
*        PDORMR3 may violate as it iterates IV across blocks.
...
*        ICCOL2 with offset ICOFFC2), since the col-V SIDE='R' PBDTRNV
*        calls transpose V into a row at ICCOL2 and the downstream
*        DGEMV/DGER act on sub(C2).  In the row-V SIDE='R' branch this
...
*
               IPW = MPV+1
               CALL PBDTRNV( ICTXT, 'Rowwise', 'Transpose', L,
...
*
               IPW = NQV+1
               CALL PBDTRNV( ICTXT, 'Columnwise', 'Transpose', L,
