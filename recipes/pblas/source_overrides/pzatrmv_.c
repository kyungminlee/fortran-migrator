...
*  =======
*
*  PZATRMV  performs one of the matrix-vector operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PZATRMV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PZATRMV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PZATRMV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PZATRMV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  9, &info );
      PB_Cchkvec( ctxt, "PZATRMV", "X", *N, 4, Xi, Xj, Xd, *INCX, 13, &info );
      PB_Cchkvec( ctxt, "PZATRMV", "Y", *N, 4, Yi, Yj, Yd, *INCY, 19, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PZATRMV", info ); return; }
#endif
/*
...
*/
   ...
   utyp = PB_Cdtypeset(); usiz = type->usiz;
/*
      ...
      if( upper )
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Akp > 0 && Anq0 > 0 )
               {
                  zagemv_( TRANS, &Akp, &Anq0, ((char *) ALPHA),
      ...
      else
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                  zagemv_( TRANS, &Amp0, &Anq0, ((char *) ALPHA),
         ...
         else
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                   ...
                   * (uplo=L, trans=T or C) only — every other
                   * (uplo,trans) combination uses the user-supplied
                   * ALPHA. The off-diagonal contribution skipped alpha
                   * scaling. Same bug as pdatrmv_.c L,T; see the
                   * comment in that file. Fixed by swapping
                   * `one` -> `((char *) ALPHA)`. */
                  zagemv_( TRANS, &Amp0, &Anq0, ((char *) ALPHA),
                           Mptr( Aptr, Akp, Akq,  Ald, size ), &Ald,
