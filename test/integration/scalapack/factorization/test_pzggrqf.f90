program test_pzggrqf
    ...
    use target_scalapack,  only: target_name, target_eps, target_pzggrqf
    implicit none
    ...
    call grid_init()
    call report_init('pzggrqf', target_name, my_rank)

    do i = 1, size(m_v)
        m = m_v(i); p = p_v(i); n = n_v(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 27201 + 31*i)
        call gen_distrib_matrix_z(p, n, mb, nb, B_loc, B_glob, seed = 27211 + 31*i)

        locm_a = numroc_local(m, mb, my_row, 0, my_nprow)
        locn_b = numroc_local(n, nb, my_col, 0, my_npcol)
        call local_desc(desca, m, n, mb, nb)
        call local_desc(descb, p, n, mb, nb)
        ...
        call gather_matrix_z(p, n, mb, nb, B_loc, B_got)
        ...
        if (my_rank == 0) then
            ...
            call zggrqf(m, p, n, A_ref, m, taua_ref, B_ref, p, taub_ref, &
