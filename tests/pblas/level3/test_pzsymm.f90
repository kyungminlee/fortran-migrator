program test_pzsymm
    ...
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, m, mb, mb, A_loc, A_glob, seed = 14201 + 47 * i)
        call gen_distrib_matrix_z(m, n, mb, nb, B_loc, B_glob, seed = 14211 + 47 * i)
        call gen_distrib_matrix_z(m, n, mb, nb, C_loc, C0,     seed = 14221 + 47 * i)
        ...
        call target_pzsymm('L', 'U', m, n, alpha, &
        ...
        if (my_rank == 0) then
            allocate(C_ref(m, n))
            C_ref = C0
            call zsymm('L', 'U', m, n, alpha, A_glob, m, B_glob, m, &
                       beta, C_ref, m)
