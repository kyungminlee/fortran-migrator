program test_pzgemm
    ...
    integer, parameter :: ms(*) = [24, 64, 96]
    integer, parameter :: ns(*) = [32, 48, 80]
    integer, parameter :: ks(*) = [16, 40, 72]
    ...
    do i = 1, size(ms)
        ...
        call gen_distrib_matrix_z(m, k, mb, nb, A_loc, A_glob, seed = 12101 + 43 * i)
        call gen_distrib_matrix_z(k, n, mb, nb, B_loc, B_glob, seed = 12111 + 43 * i)
        ...
        call target_pzgemm('N', 'N', m, n, k, alpha, &
        ...
        if (my_rank == 0) then
            allocate(C_ref(m, n))
            C_ref = C0
            call zgemm('N', 'N', m, n, k, alpha, A_glob, m, B_glob, k, &
                       beta, C_ref, m)
