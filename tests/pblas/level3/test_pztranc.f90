program test_pztranc
    ! pztranc: C := beta*C + alpha*A^H (conjugate transpose).
    ...
    use target_pblas,  only: target_name, target_eps, target_pztranc
    implicit none
    ...
    call grid_init()
    call report_init('pztranc', target_name, my_rank)

    alpha = cmplx(0.7_ep,  0.2_ep, kind=ep)
    beta  = cmplx(0.3_ep, -0.1_ep, kind=ep)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(n, m, mb, nb, A_loc, A_glob, seed = 17401 + 29*i)
        call gen_distrib_matrix_z(m, n, mb, nb, C_loc, C0,    seed = 17411 + 29*i)
        ...
        call target_pztranc(m, n, alpha, A_loc, 1, 1, desca, &
                            beta, C_loc, 1, 1, descc)
        call gather_matrix_z(m, n, mb, nb, C_loc, C_got)

        if (my_rank == 0) then
            allocate(C_ref(m, n))
            C_ref = beta * C0 + alpha * conjg(transpose(A_glob))
