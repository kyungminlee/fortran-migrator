program test_pztrmm
    ...
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, m, mb, mb, A_loc, A_glob, seed = 14601 + 59 * i)
        call gen_distrib_matrix_z(m, n, mb, nb, B_loc, B0,     seed = 14611 + 59 * i)
        ...
        call target_pztrmm('L', 'U', 'N', 'N', m, n, alpha, &
        ...
        if (my_rank == 0) then
            allocate(B_ref(m, n))
            B_ref = B0
            call ztrmm('L', 'U', 'N', 'N', m, n, alpha, A_glob, m, B_ref, m)
