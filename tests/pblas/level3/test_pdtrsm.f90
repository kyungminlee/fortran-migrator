program test_pdtrsm
    ...
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix(m, m, mb, mb, A_loc, A_glob, seed = 11001 + 41 * i)
        call gen_distrib_matrix(m, n, mb, nb, B_loc, B0,     seed = 11011 + 41 * i)

        ! Diagonally-dominant A for well-conditioned solve.
        bump = real(m, ep)
        do j = 1, m
        ...
        call target_pdtrsm('L', 'U', 'N', 'N', m, n, alpha, &
        ...
        if (my_rank == 0) then
            allocate(B_ref(m, n))
            B_ref = B0
            call dtrsm('L', 'U', 'N', 'N', m, n, alpha, A_glob, m, B_ref, m)
            err = max_rel_err_mat(B_got, B_ref)
            tol = 64.0_ep * real(m, ep) * target_eps
