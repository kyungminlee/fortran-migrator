program test_pzhemm
    ...
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        ! Side='L': A is m×m Hermitian, B is m×n, C is m×n.
        call gen_distrib_matrix_z(m, m, mb, mb, A_loc, A_glob, seed = 14101 + 47 * i)
        call gen_distrib_matrix_z(m, n, mb, nb, B_loc, B_glob, seed = 14111 + 47 * i)
        call gen_distrib_matrix_z(m, n, mb, nb, C_loc, C0,     seed = 14121 + 47 * i)
        ...
        call target_pzhemm('L', 'U', m, n, alpha, &
        ...
        if (my_rank == 0) then
            allocate(C_ref(m, n))
            C_ref = C0
            call zhemm('L', 'U', m, n, alpha, A_glob, m, B_glob, m, &
                       beta, C_ref, m)
