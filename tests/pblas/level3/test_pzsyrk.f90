program test_pzsyrk
    ...
    do i = 1, size(ns)
        n = ns(i); k = ks(i)
        call gen_distrib_matrix_z(n, k, mb, mb, A_loc, A_glob, seed = 14301 + 53 * i)
        call gen_distrib_matrix_z(n, n, mb, mb, C_loc, C0,     seed = 14311 + 53 * i)
        ...
        call target_pzsyrk('U', 'N', n, k, alpha, &
        ...
        if (my_rank == 0) then
            allocate(C_ref(n, n))
            C_ref = C0
            call zsyrk('U', 'N', n, k, alpha, A_glob, n, beta, C_ref, n)
