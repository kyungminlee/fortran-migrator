program test_pdsymm
    ...
    do is = 1, size(sides)
        ...
        do i = 1, size(ms)
            m = ms(i); n = ns(i)
            ! SIDE='L': A is m×m; SIDE='R': A is n×n. B and C are always m×n.
            if (side == 'L') then
                ka = m
            else
                ka = n
            ...
            call gen_distrib_matrix(ka, ka, mb, mb, A_loc, A_glob, &
                                    seed = 8101 + 29 * i + 211 * is)
            call gen_distrib_matrix(m, n, mb, nb, B_loc, B_glob, &
                                    seed = 8111 + 29 * i + 211 * is)
            ...
            if (my_rank == 0) then
                allocate(C_ref(m, n))
                C_ref = C0
                call dsymm(side, uplo, m, n, alpha, A_glob, ka, B_glob, m, &
                           beta, C_ref, m)
