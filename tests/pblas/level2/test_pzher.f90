program test_pzher
    ...
    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix_z(n, n, mb, mb, A_loc, A_glob, seed = 5701 + 19 * i)
        call gen_distrib_vector_z(n, mb, x_loc, x_glob, seed = 5711 + 19 * i)
        ...
        call target_pzher('U', n, alpha, x_loc, 1, 1, descx, 1, &
                          A_loc, 1, 1, desca)
        ...
        if (my_rank == 0) then
            allocate(A_ref(n, n))
            A_ref = A_glob
            call zher('U', n, alpha, x_glob, 1, A_ref, n)
