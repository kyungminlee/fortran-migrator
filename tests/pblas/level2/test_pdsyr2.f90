program test_pdsyr2
    ...
    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix(n, n, mb, mb, A_loc, A_glob, seed = 5301 + 19 * i)
        call gen_distrib_vector(n, mb, x_loc, x_glob, seed = 5311 + 19 * i)
        call gen_distrib_vector(n, mb, y_loc, y_glob, seed = 5321 + 19 * i)
        ...
        call target_pdsyr2('U', n, alpha, x_loc, 1, 1, descx, 1, &
        ...
        if (my_rank == 0) then
            allocate(A_ref(n, n))
            A_ref = A_glob
            call dsyr2('U', n, alpha, x_glob, 1, y_glob, 1, A_ref, n)
