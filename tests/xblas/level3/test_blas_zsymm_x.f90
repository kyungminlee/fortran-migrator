program test_blas_zsymm_x
    use prec_kinds,     only: ep
    use prec_report,    only: report_init, report_case, report_finalize
    use compare,        only: max_rel_err_mat_z
    use test_data,      only: gen_matrix_complex
    use target_xblas,   only: target_name, target_eps, target_blas_zsymm_x, &
                              blas_left_side, blas_upper
    use ref_quad_xblas, only: ref_blas_zsymm_x
    implicit none
    integer, parameter :: m = 32, n = 24
    complex(ep), allocatable :: a(:,:), b(:,:), c_ref(:,:), c_got(:,:)
    complex(ep) :: alpha, beta
    real(ep) :: err, tol
    call report_init('blas_zsymm_x', target_name)
    ...
    call ref_blas_zsymm_x(blas_left_side, blas_upper, m, n, alpha, a, m, &
                          b, m, beta, c_ref, m)
    call target_blas_zsymm_x(blas_left_side, blas_upper, m, n, alpha, a, m, &
                             b, m, beta, c_got, m)
