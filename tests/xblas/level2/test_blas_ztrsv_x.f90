program test_blas_ztrsv_x
    use prec_kinds,     only: ep
    use prec_report,    only: report_init, report_case, report_finalize
    use compare,        only: max_rel_err_vec_z
    use test_data,      only: gen_matrix_complex, gen_vector_complex
    use target_xblas,   only: target_name, target_eps, target_blas_ztrsv_x, &
                              blas_upper, blas_no_trans, blas_non_unit_diag
    use ref_quad_xblas, only: ref_blas_ztrsv_x
    implicit none
    ...
    real(ep) :: err, tol
    integer :: i
    call report_init('blas_ztrsv_x', target_name)
    call gen_matrix_complex(n, n, t, seed = 11)
    do i = 1, n
        t(i, i) = t(i, i) + cmplx(real(2 * n, ep), 0.0_ep, ep)
    end do
    call gen_vector_complex(n, x, seed = 12)
    allocate(x_ref(n), x_got(n)); x_ref = x; x_got = x
    alpha = (1.7_ep, -0.4_ep)
    call ref_blas_ztrsv_x(blas_upper, blas_no_trans, blas_non_unit_diag, &
                          n, alpha, t, n, x_ref, 1)
    call target_blas_ztrsv_x(blas_upper, blas_no_trans, blas_non_unit_diag, &
                             n, alpha, t, n, x_got, 1)
