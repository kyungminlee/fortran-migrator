program test_blas_zhemv2_x
    use prec_kinds,     only: ep
    use prec_report,    only: report_init, report_case, report_finalize
    use compare,        only: max_rel_err_vec_z
    use test_data,      only: gen_matrix_complex, gen_vector_complex
    use target_xblas,   only: target_name, target_eps, target_blas_zhemv2_x, &
                              blas_upper
    use ref_quad_xblas, only: ref_blas_zhemv2_x
    implicit none
    ...
    real(ep) :: err, tol
    integer :: i
    call report_init('blas_zhemv2_x', target_name)
    ...
    call ref_blas_zhemv2_x(blas_upper, n, alpha, a, n, head_x, tail_x, 1, &
                           beta, y_ref, 1)
    call target_blas_zhemv2_x(blas_upper, n, alpha, a, n, head_x, tail_x, 1, &
                              beta, y_got, 1)
