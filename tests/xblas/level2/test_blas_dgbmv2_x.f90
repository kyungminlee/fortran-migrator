program test_blas_dgbmv2_x
    use prec_kinds,     only: ep
    use prec_report,    only: report_init, report_case, report_finalize
    use compare,        only: max_rel_err_vec
    use test_data,      only: gen_matrix_quad, gen_vector_quad
    use target_xblas,   only: target_name, target_eps, target_blas_dgbmv2_x, &
                              blas_no_trans
    use ref_quad_xblas, only: ref_blas_dgbmv2_x
    implicit none
    ...
    call report_init('blas_dgbmv2_x', target_name)
    ...
    call ref_blas_dgbmv2_x(blas_no_trans, m, n, kl, ku, alpha, a, lda, &
                           head_x, tail_x, 1, beta, y_ref, 1)
    call target_blas_dgbmv2_x(blas_no_trans, m, n, kl, ku, alpha, a, lda, &
