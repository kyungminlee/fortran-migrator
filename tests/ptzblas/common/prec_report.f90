! Module name is ptzblas_prec_report (not prec_report) to avoid colliding
! with tests/blas/common/prec_report and tests/pblas/common/prec_report —
! all targets write their .mod files into the same
! ${PROJECT_BINARY_DIR}/fmod directory, so the module names must differ.
!
! PTZBLAS is serial Fortran (no MPI / no BLACS). The API still takes a
! `rank` argument so test programs can use the same call shape as the
! pblas variant — for serial tests, just pass 0 unconditionally.
