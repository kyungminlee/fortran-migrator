program test_zasymv
    ...
    use ptzblas_ref_quad_aux, only: ref_zasymv
    use test_data,            only: gen_matrix_complex, gen_vector_complex, gen_vector_quad
    use target_ptzblas,       only: target_name, target_eps, target_zasymv
    implicit none
    ...
    call report_init('zasymv', target_name, 0)
    do i = 1, size(uplo_set)
        uplo = uplo_set(i)
        do k = 1, size(alpha_set)
            call gen_matrix_complex(n, n, A,    seed = 4400 + 3 * i + 11 * k)
            call gen_vector_complex(n, x,        seed = 4500 + 5 * i + 13 * k)
            call gen_vector_quad   (n, y_got,    seed = 4600 + 7 * i + 17 * k)
            allocate(y_ref(n)); y_ref = y_got
            alpha = alpha_set(k); beta = -0.4_ep
            call target_zasymv(uplo, n, alpha, A, n, x, 1, beta, y_got, 1)
            call ref_zasymv(uplo, n, alpha, A, x, beta, y_ref)
