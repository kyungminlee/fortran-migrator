program test_pdsyevr
    ! Symmetric eigensolver via Relatively Robust Representations.
    ! Same shape as test_pdsyev but with RANGE='A' (all eigenvalues).
    ...
    call grid_init()
    call report_init('pdsyevr', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix(n, n, mb, nb, A_loc, A_glob, seed = 21601 + 31*i)
        allocate(A_sym(n, n))
        ...
        call target_pdsyevr('V', 'A', 'U', n, A_loc, 1, 1, desca, &
                            0.0_ep, 0.0_ep, 0, 0, m, nz, w_got, &
                            Z_loc, 1, 1, descz, wopt, -1, iwopt, -1, info)
        lwork  = max(1, int(wopt(1)))
        liwork = max(1, iwopt(1))
        allocate(work(lwork), iwork(liwork))
        call target_pdsyevr('V', 'A', 'U', n, A_loc, 1, 1, desca, &
                            0.0_ep, 0.0_ep, 0, 0, m, nz, w_got, &
                            Z_loc, 1, 1, descz, work, lwork, iwork, liwork, info)
        call gather_matrix(n, n, mb, nb, Z_loc, Z_glob)

        if (my_rank == 0) then
            allocate(A_ref(n, n), w_ref(n), work_ref(max(1, 64 * n)))
            A_ref = A_sym
            call dsyev('V', 'U', n, A_ref, n, w_ref, &
            ...
            call report_case(trim(label), err, tol)
            deallocate(AZ, ZD, A_ref, w_ref, work_ref)
        end if
