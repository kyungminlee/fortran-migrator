program test_pzheevd
    ...
    use target_scalapack, only: target_name, target_eps, target_pzheevd
    implicit none

    integer, parameter :: ns(*) = [32, 64, 96]
    character(len=1), parameter :: uplos(*) = ['U', 'L', 'U']
    integer, parameter :: mb = 8, nb = 8
    integer :: i, n, info, info_ref, lwork, lrwork, liwork, j
    ...
    real(ep), allocatable :: w_got(:), w_ref(:), rwork(:), rwork_ref(:)
    integer,  allocatable :: iwork(:), iwork_ref(:)
    complex(ep), allocatable :: AZ(:,:), ZD(:,:)
    real(ep) :: err, tol, anorm, rnorm
    character(len=1) :: jobz

    call grid_init()
    call report_init('pzheevd', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i); jobz = 'V'
        call gen_distrib_matrix_z(n, n, mb, nb, A_loc, A_glob, seed = 13101 + 31*i)
        ...
        Z_loc = (0.0_ep, 0.0_ep)
        allocate(w_got(n), work(1), rwork(1), iwork(1))
        call target_pzheevd(jobz, uplos(i), n, A_loc, 1, 1, desca, w_got, &
                            Z_loc, 1, 1, descz, work, -1, rwork, -1, &
                            iwork, -1, info)
        ...
        call target_pzheevd(jobz, uplos(i), n, A_loc, 1, 1, desca, w_got, &
                            Z_loc, 1, 1, descz, work, lwork, rwork, lrwork, &
                            iwork, liwork, info)
        ...
        if (my_rank == 0) then
            ...
            call zheevd(jobz, uplos(i), n, A_ref, n, w_ref, &
                        ...
                        iwork_ref, size(iwork_ref), info_ref)
            ...
            AZ = matmul(A_herm, Z_glob)
            do j = 1, n
                ZD(:, j) = Z_glob(:, j) * w_got(j)
            end do
            anorm = maxval(abs(A_herm))
            ...
            deallocate(AZ, ZD, A_ref, w_ref, work_ref, rwork_ref, iwork_ref)
        end if
        if (allocated(Z_glob)) deallocate(Z_glob)
        deallocate(A_loc, Z_loc, A_glob, A_herm, w_got, work, rwork, iwork)
    end do
