program test_pdtrsen
    ! Reorder the real Schur form via pdtrsen and compare the eigenvalue
    ! spectrum against LAPACK dtrsen. JOB='N' (no condition numbers — the
    ! upstream SCASY-based S/SEP path at pdtrsen.f:552-577 is commented
    ! out anyway, so WANTS/WANTSP would return uninitialised). Multiple
    ! sizes back-to-back guard against the LQUERY-IWORK heap corruption
    ! fixed in target_pdtrsen.
    ...
    real(ep), allocatable :: T_loc(:,:), T_glob(:,:), T_got(:,:), T_ref(:,:)
    ...
    real(ep) :: err, tol, S_got, SEP_got, S_ref, SEP_ref
    character(len=48) :: label

    call grid_init()
    call report_init('pdtrsen', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix(n, n, mb, nb, T_loc, T_glob, seed = 41101 + 31*i)

        ! Force upper-triangular Schur form (all real eigenvalues, 1x1 blocks).
        do jg = 1, n
            do ig = jg + 1, n
                T_glob(ig, jg) = 0.0_ep
            end do
        ...
        ! PARA constraints (mirrors test_pdtrord; see pdtrsen.f:435-446).
        para = [1, 2, 5, 50, mb, 1]

        allocate(WR(n), WI(n))
        allocate(work(1), iwork(1))
        call target_pdtrsen('N', 'N', select_v, para, n, T_loc, 1, 1, descT, &
                            Q_loc, 1, 1, descQ, WR, WI, m_got, S_got, SEP_got, &
                            work, -1, iwork, -1, info)
        ...
        if (my_rank == 0) then
            ...
            call dtrsen('N', 'N', select_log, n, T_ref, n, Q_ref, n, &
                        WR_ref, WI_ref, m_ref, S_ref, SEP_ref, &
                        work_ref, size(work_ref), iwork_ref, size(iwork_ref), info_ref)
            ! Compare the sorted eigenvalue spectrum (trailing-block ordering
            ! may differ between pdtrsen and dtrsen, same as test_pdtrord).
            block
                integer :: a, b
                real(ep) :: tmp
