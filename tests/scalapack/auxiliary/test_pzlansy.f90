program test_pzlansy
    ...
    call grid_init()
    call report_init('pzlansy', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        do k = 1, size(uplos)
            do j = 1, size(norms)
                got = target_pzlansy(norms(j), uplos(k), n, A_loc, 1, 1, desca, work)
                if (my_rank == 0) then
                    refv = zlansy(norms(j), uplos(k), n, A_sym, n, work_ref)
