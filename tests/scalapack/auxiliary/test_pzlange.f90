program test_pzlange
    ...
    use target_scalapack, only: target_name, target_eps, target_pzlange
    implicit none
    ...
    call grid_init()
    call report_init('pzlange', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 15701 + 31*i)
        ...
        do j = 1, size(norms)
            got = target_pzlange(norms(j), m, n, A_loc, 1, 1, desca, work)
            if (my_rank == 0) then
                refv = zlange(norms(j), m, n, A_glob, m, work_ref)
