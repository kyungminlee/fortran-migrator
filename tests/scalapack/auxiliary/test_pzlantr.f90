program test_pzlantr
    ...
    use target_scalapack, only: target_name, target_eps, target_pzlantr
    implicit none
    ...
    call grid_init()
    call report_init('pzlantr', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 16001 + 31*i)
        ...
        do l = 1, size(diags)
            do k = 1, size(uplos)
                do j = 1, size(norms)
                    got = target_pzlantr(norms(j), uplos(k), diags(l), m, n, &
                                         A_loc, 1, 1, desca, work)
                    if (my_rank == 0) then
                        refv = zlantr(norms(j), uplos(k), diags(l), m, n, &
                                      A_glob, m, work_ref)
