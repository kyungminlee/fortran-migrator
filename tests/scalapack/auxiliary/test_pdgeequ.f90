program test_pdgeequ
    ! Row/column equilibration scaling. Outputs R, C, ROWCND, COLCND, AMAX.
    use prec_kinds,       only: ep
    use compare,          only: rel_err_scalar, max_rel_err_vec
    use pblas_prec_report, only: report_init, report_case, report_finalize
    use ref_quad_lapack,  only: dgeequ
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_context, &
                                my_nprow, my_npcol, my_row, my_col, &
                                numroc_local, descinit_local
    use pblas_distrib,    only: gen_distrib_matrix, gather_vector, &
                                gather_vector_row
    use target_scalapack, only: target_name, target_eps, target_pdgeequ
    implicit none
    ...
    call grid_init()
    call report_init('pdgeequ', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix(n, n, mb, nb, A_loc, A_glob, seed = 21201 + 31*i)
        ...
        call target_pdgeequ(n, n, A_loc, 1, 1, desca, R_loc, C_loc, &
        ...
        if (my_rank == 0) then
            ...
            call dgeequ(n, n, A_glob, n, R_ref, C_ref, &
