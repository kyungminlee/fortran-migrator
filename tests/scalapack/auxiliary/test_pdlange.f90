program test_pdlange
    ...
    use target_scalapack, only: target_name, target_eps, target_pdlange
    implicit none
    ...
    call grid_init()
    call report_init('pdlange', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix(m, n, mb, nb, A_loc, A_glob, seed = 9901 + 31*i)
        ...
        allocate(work_ref(max(1, max(m, n))))

        do j = 1, size(norms)
            got = target_pdlange(norms(j), m, n, A_loc, 1, 1, desca, work)
            if (my_rank == 0) then
                refv = dlange(norms(j), m, n, A_glob, m, work_ref)
                err = rel_err_scalar(got, refv)
                tol = 32.0_ep * real(max(m, n), ep) * target_eps
                write(label, '(a,a1,a,i0,a,i0)') 'norm=', norms(j), &
                    ',m=', m, ',n=', n
