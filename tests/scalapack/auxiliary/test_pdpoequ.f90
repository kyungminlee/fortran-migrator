program test_pdpoequ
    ! Symmetric/Hermitian-PD equilibration scaling. Outputs (SR=SC), SCOND, AMAX.
    ...
    integer :: desca(9)
    real(ep), allocatable :: A_loc(:,:), A_glob(:,:), M_glob(:,:), dummy(:,:)
    ...
    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix(n, n, mb, nb, dummy, M_glob, seed = 22001 + 31*i)
        deallocate(dummy)
        allocate(A_glob(n, n))
        ...
        call target_pdpoequ(n, A_loc, 1, 1, desca, SR_loc, SC_loc, &
                            scond, amax, info)
        ...
        if (my_rank == 0) then
            allocate(S_ref(n))
            call dpoequ(n, A_glob, n, S_ref, scond_ref, amax_ref, info_ref)
