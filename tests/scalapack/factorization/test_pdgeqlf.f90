program test_pdgeqlf
    ...
    use target_scalapack, only: target_name, target_eps, target_pdgeqlf
    implicit none
    ...
    call grid_init()
    call report_init('pdgeqlf', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix(m, n, mb, nb, A_loc, A_glob, seed = 13501 + 31*i)
