program test_pdormrq
    ! Apply Q (from LQ factorization) to C; compare to dgerqf+dormrq.
    ...
    call grid_init()
    call report_init('pdormrq', target_name, my_rank)
    ...
    do ic = 1, cases
        ...
        call gen_distrib_matrix(mA, nA, mb, nb, A_loc, A_glob, seed = 14301 + 17*ic)
        call gen_distrib_matrix(mC, nC, mb, nb, C_loc, C_glob, seed = 14311 + 17*ic)
        ...
        call target_pdgerqf(mA, nA, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(work(1)))
        deallocate(work); allocate(work(lwork))
        call target_pdgerqf(mA, nA, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pdormrq(sides(ic), transes(ic), mC, nC, k, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            A_ref = A_glob
            C_ref = C_glob
            call dgerqf(mA, nA, A_ref, mA, tau_ref, work_ref, size(work_ref), info_ref)
            call dormrq(sides(ic), transes(ic), mC, nC, k, A_ref, mA, tau_ref, &
