program test_pzung2l
    ...
    call grid_init()
    call report_init('pzung2l', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i); k = n
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 13901 + 31*i)
        ...
        call target_pzgeqlf(m, n, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzgeqlf(m, n, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pzung2l(m, n, k, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzung2l(m, n, k, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        call gather_matrix_z(m, n, mb, nb, A_loc, Q_got)

        if (my_rank == 0) then
            allocate(A_ref(m, n), tau_ref(min(m, n)), work_ref(max(1, n * 64)))
            A_ref = A_glob
            call zgeqlf(m, n, A_ref, m, tau_ref, work_ref, size(work_ref), info_ref)
            call zungql(m, n, k, A_ref, m, tau_ref, work_ref, size(work_ref), info_ref)
