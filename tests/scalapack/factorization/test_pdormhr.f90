program test_pdormhr
    ! Apply Q from gehrd to a matrix C; compare to dgehrd+dormhr.
    ...
    call grid_init()
    call report_init('pdormhr', target_name, my_rank)
    ...
    do ic = 1, cases
        ...
        call target_pdgehrd(n, ilo, ihi, A_loc, 1, 1, desca, tau, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pdormhr(sides(ic), transes(ic), mC, nC, ilo, ihi, &
        ...
        if (my_rank == 0) then
            ...
            call dormhr(sides(ic), transes(ic), mC, nC, ilo, ihi, &
