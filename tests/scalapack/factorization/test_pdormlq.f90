program test_pdormlq
    ! Apply Q (from LQ factorization) to C; compare to dgelqf+dormlq.
    ...
    call grid_init()
    call report_init('pdormlq', target_name, my_rank)

    do ic = 1, cases
        ! For LQ: A is m_A-by-n_A with m_A <= n_A.
        ! Q has m_A orthonormal rows. SIDE='L' applies Q to m_C-by-n_C C
        ! where m_C must equal n_A; SIDE='R' applies to right where n_C = n_A.
        mA = 32
        if (sides(ic) == 'L') then
            mC = 64; nC = 48
            nA = mC
        ...
        call gen_distrib_matrix(mA, nA, mb, nb, A_loc, A_glob, seed = 10201 + 17*ic)
        call gen_distrib_matrix(mC, nC, mb, nb, C_loc, C_glob, seed = 10211 + 17*ic)
        ...
        call target_pdgelqf(mA, nA, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(work(1)))
        deallocate(work); allocate(work(lwork))
        call target_pdgelqf(mA, nA, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pdormlq(sides(ic), transes(ic), mC, nC, k, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            A_ref = A_glob
            C_ref = C_glob
            call dgelqf(mA, nA, A_ref, mA, tau_ref, work_ref, size(work_ref), info_ref)
            call dormlq(sides(ic), transes(ic), mC, nC, k, A_ref, mA, tau_ref, &
