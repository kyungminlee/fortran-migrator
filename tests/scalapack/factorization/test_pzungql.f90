program test_pzungql
    ...
    call grid_init()
    call report_init('pzungql', target_name, my_rank)
    ...
    do i = 1, size(ms)
        ...
        allocate(work(1))
        call target_pzungql(m, n, k, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzungql(m, n, k, A_loc, 1, 1, desca, tau_got, work, lwork, info)
