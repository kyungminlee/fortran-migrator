program test_pzgeqr2
    use prec_kinds,       only: ep
    use compare,          only: max_rel_err_mat_z
    use pblas_prec_report, only: report_init, report_case, report_finalize
    use ref_quad_lapack,  only: zgeqr2
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_context, &
                                my_nprow, my_npcol, my_row, my_col, &
                                numroc_local, descinit_local
    use pblas_distrib,    only: gen_distrib_matrix_z, gather_matrix_z
    use target_scalapack, only: target_name, target_eps, target_pzgeqr2
    implicit none
    ...
    call grid_init()
    call report_init('pzgeqr2', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 9701 + 31*i)
        ...
        call target_pzgeqr2(m, n, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzgeqr2(m, n, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        call gather_matrix_z(m, n, mb, nb, A_loc, A_got)

        if (my_rank == 0) then
            allocate(A_ref(m, n), tau_ref(min(m, n)), work_ref(max(1, n)))
            A_ref = A_glob
            call zgeqr2(m, n, A_ref, m, tau_ref, work_ref, info_ref)
