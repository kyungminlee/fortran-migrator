program test_pdorgqr
    ...
    call grid_init()
    call report_init('pdorgqr', target_name, my_rank)
    ...
    do i = 1, size(ms)
        ...
        ! Generate Q.
        allocate(work(1))
        call target_pdorgqr(m, n, k, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(work(1)))
        deallocate(work); allocate(work(lwork))
        call target_pdorgqr(m, n, k, A_loc, 1, 1, desca, tau_got, work, lwork, info)
