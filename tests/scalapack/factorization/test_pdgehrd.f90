program test_pdgehrd
    ...
    use target_scalapack, only: target_name, target_eps, target_pdgehrd
    implicit none
    ...
    call grid_init()
    call report_init('pdgehrd', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i); ilo = 1; ihi = n
        call gen_distrib_matrix(n, n, mb, nb, A_loc, A_glob, seed = 14501 + 31*i)
        ...
        call target_pdgehrd(n, ilo, ihi, A_loc, 1, 1, desca, tau_got, &
                            work, -1, info)
