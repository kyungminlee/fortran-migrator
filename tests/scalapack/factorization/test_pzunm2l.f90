program test_pzunm2l
    use prec_kinds,       only: ep
    use compare,          only: max_rel_err_mat_z
    use pblas_prec_report, only: report_init, report_case, report_finalize
    use ref_quad_lapack,  only: zgeqlf, zunmql
    ...
    call grid_init()
    call report_init('pzunm2l', target_name, my_rank)
    ...
    do ic = 1, cases
        ...
        call gen_distrib_matrix_z(mA, nA, mb, nb, A_loc, A_glob, seed = 14001 + 17*ic)
        call gen_distrib_matrix_z(mC, nC, mb, nb, C_loc, C_glob, seed = 14011 + 17*ic)
        ...
        call target_pzgeqlf(mA, nA, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzgeqlf(mA, nA, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pzunm2l(sides(ic), transes(ic), mC, nC, k, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            A_ref = A_glob
            C_ref = C_glob
            call zgeqlf(mA, nA, A_ref, mA, tau_ref, work_ref, size(work_ref), info_ref)
            call zunmql(sides(ic), transes(ic), mC, nC, k, A_ref, mA, tau_ref, &
