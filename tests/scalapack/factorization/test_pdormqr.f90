program test_pdormqr
    ! Apply Q (from a pdgeqrf factorization) to a random matrix C, and
    ! compare to dormqr applied to the dgeqrf reference factorization.
    ! Cycles SIDE/TRANS so all four combinations are exercised.
    ...
    call grid_init()
    call report_init('pdormqr', target_name, my_rank)

    do ic = 1, cases
        ! For SIDE='L', op(Q) is m_C-by-m_C → A is m_C-by-k.
        ! For SIDE='R', op(Q) is n_C-by-n_C → A is n_C-by-k.
        mC = 64; nC = 48
        if (sides(ic) == 'L') then
            mA = mC
        else
        ...
        call target_pdgeqrf(mA, nA, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(work(1)))
        deallocate(work); allocate(work(lwork))
        call target_pdgeqrf(mA, nA, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        ! Apply Q to C.
        allocate(work(1))
        call target_pdormqr(sides(ic), transes(ic), mC, nC, k, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            A_ref = A_glob
            C_ref = C_glob
            call dgeqrf(mA, nA, A_ref, mA, tau_ref, work_ref, size(work_ref), info_ref)
            call dormqr(sides(ic), transes(ic), mC, nC, k, A_ref, mA, tau_ref, &
