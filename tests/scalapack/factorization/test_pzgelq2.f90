program test_pzgelq2
    ...
    use target_scalapack, only: target_name, target_eps, target_pzgelq2
    implicit none
    ...
    call grid_init()
    call report_init('pzgelq2', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 50301 + 31*i)
        ...
        call target_pzgelq2(m, n, A_loc, 1, 1, desca, tau_got, work, -1, info)
        lwork = max(1, int(real(work(1), ep)))
        deallocate(work); allocate(work(lwork))
        call target_pzgelq2(m, n, A_loc, 1, 1, desca, tau_got, work, lwork, info)
