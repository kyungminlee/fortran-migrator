program test_pzgeqlf
    ...
    use target_scalapack, only: target_name, target_eps, target_pzgeqlf
    implicit none
    ...
    call grid_init()
    call report_init('pzgeqlf', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 13801 + 31*i)
