program test_pdggqrf
    ! Generalized QR factorization: A=Q*R (n×m), B=Q*T*Z (n×p).
    ! Compare gathered factors against dggqrf.
    ...
    call grid_init()
    call report_init('pdggqrf', target_name, my_rank)
    ...
    do i = 1, size(n_v)
        ...
        call gather_matrix(n, m, mb, nb, A_loc, A_got)
        call gather_matrix(n, p, mb, nb, B_loc, B_got)
        ...
        if (my_rank == 0) then
            ...
            call dggqrf(n, m, p, A_ref, n, taua_ref, B_ref, n, taub_ref, &
