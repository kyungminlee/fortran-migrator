program test_pzunmrq
    ...
    call grid_init()
    call report_init('pzunmrq', target_name, my_rank)
    ...
    do ic = 1, cases
        ...
        call gen_distrib_matrix_z(mA, nA, mb, nb, A_loc, A_glob, seed = 14401 + 17*ic)
        ...
        call target_pzgerqf(mA, nA, A_loc, 1, 1, desca, tau_got, work, lwork, info)
        deallocate(work)

        allocate(work(1))
        call target_pzunmrq(sides(ic), transes(ic), mC, nC, k, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            A_ref = A_glob
            C_ref = C_glob
            call zgerqf(mA, nA, A_ref, mA, tau_ref, work_ref, size(work_ref), info_ref)
            call zunmrq(sides(ic), transes(ic), mC, nC, k, A_ref, mA, tau_ref, &
