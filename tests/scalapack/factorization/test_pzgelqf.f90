program test_pzgelqf
    ...
    use target_scalapack, only: target_name, target_eps, target_pzgelqf
    implicit none
    ...
    call grid_init()
    call report_init('pzgelqf', target_name, my_rank)

    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_distrib_matrix_z(m, n, mb, nb, A_loc, A_glob, seed = 10301 + 31*i)
