program test_pzgbsv
    ! Complex general banded LU solve — Z mirror of pdgbsv. Local A
    ! storage: LLD_A = 2*bwl+2*bwu+1; element A(i,j) at packed row
    ! bwl+2*bwu+1+i-j of column j.
    ...
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_nproc, &
                                ...
                                descinit_1d
    use target_scalapack, only: target_name, target_eps, target_pzgbsv
    use mpi
    implicit none
    ...
    call grid_init()
    call report_init('pzgbsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        call target_pzgbsv(n, bwl, bwu, nrhs, A_loc, 1, desca, ipiv_loc, &
