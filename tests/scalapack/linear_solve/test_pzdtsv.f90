program test_pzdtsv
    ! Complex 1D-distributed tridiagonal solver — Z-mirror of test_pddtsv.
    ...
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_nproc, &
                                ...
                                descinit_1d
    use target_scalapack, only: target_name, target_eps, target_pzdtsv
    use mpi
    implicit none
    ...
    call grid_init()
    call report_init('pzdtsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        call target_pzdtsv(n, nrhs, dl_loc, d_loc, du_loc, 1, desca, &
