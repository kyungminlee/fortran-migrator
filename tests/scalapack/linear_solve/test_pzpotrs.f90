program test_pzpotrs
    ! Z-mirror of test_pdpotrs.
    use prec_kinds,       only: ep
    use compare,          only: max_rel_err_mat_z
    use pblas_prec_report, only: report_init, report_case, report_finalize
    use ref_quad_lapack,  only: zpotrf, zpotrs
    ...
    call grid_init()
    call report_init('pzpotrs', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix_z(n, n, mb, nb, dummy_loc, M_glob, seed = 21901 + 31*i)
        deallocate(dummy_loc)
        call gen_distrib_matrix_z(n, nrhs, mb, nb, B_loc, B_glob, seed = 21911 + 31*i)
        ...
        call target_pzpotrs('U', n, nrhs, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            B_ref = B_glob
            call zpotrf('U', n, A_glob, n, info_ref)
            call zpotrs('U', n, nrhs, A_glob, n, B_ref, n, info_ref)
        ...
        deallocate(A_loc, B_loc, A_glob, B_glob, M_glob)
    end do

    call report_finalize()
    call grid_exit()
end program test_pzpotrs
