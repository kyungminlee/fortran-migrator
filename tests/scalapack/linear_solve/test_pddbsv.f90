program test_pddbsv
    ! Diagonally-dominant banded solver. A is in 1D-distributed band
    ! format: LLD_A = BWL+BWU+1, packed so that A(i,j) lives at
    ! local row BWU+1+i-j of column j (1-based).
    ...
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_nproc, &
                                ...
                                descinit_1d
    use target_scalapack, only: target_name, target_eps, target_pddbsv
    use mpi
    implicit none
    ...
    integer :: i, n, nb, info, info_ref, lwork, ig, jg, jl, lldB, mxb
    integer :: bytes_per_elem, ierr, src_rank, peer
    integer, parameter :: tag = 7681
    ...
    call grid_init()
    call report_init('pddbsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        mxb = max(bwl, bwu)
        lwork = nb * (bwl + bwu) + 6 * mxb * mxb &
              + max(mxb * nrhs, mxb * mxb) + 16
        allocate(work(max(1, lwork)))

        call target_pddbsv(n, bwl, bwu, nrhs, A_loc, 1, desca, &
        ...
        deallocate(A_loc, B_loc, work, A_full, B_glob)
    end do

    call report_finalize()
    call grid_exit()
end program test_pddbsv
