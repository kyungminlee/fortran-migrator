program test_pzpttrf
    ! Complex PD tridiagonal factor (pzpttrf) + solve (pzpttrs) split.
    ...
    call grid_init()
    call report_init('pzpttrf', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        if (my_rank == 0) then
            ...
            d_ref = d_glob
            e_ref(1:n-1) = e_glob(1:n-1); e_ref(n) = (0.0_ep, 0.0_ep)
            B_ref = B_glob
            call zpttrf(n, d_ref, e_ref, info_ref)
            call zpttrs('L', n, nrhs, d_ref, e_ref, B_ref, n, info_ref)
