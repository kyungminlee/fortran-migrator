program test_pzpbsv
    ! Hermitian-PD banded solve. UPLO='U'; LLD_A = BW+1.
    ...
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_nproc, &
                                ...
                                descinit_1d
    use target_scalapack, only: target_name, target_eps, target_pzpbsv
    use mpi
    implicit none
    ...
    integer, parameter :: tag = 7771
    integer :: desca(9), descb(9)
    complex(ep), allocatable :: A_full(:,:), B_glob(:,:), M(:,:)
    complex(ep), allocatable :: A_loc(:,:), B_loc(:,:)
    complex(ep), allocatable :: B_got(:,:), B_ref(:,:), buf(:,:), A_ref(:,:)
    complex(ep), allocatable :: work(:)
    real(ep) :: err, tol
    character(len=48) :: label

    call grid_init()
    call report_init('pzpbsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        call target_pzpbsv('U', n, bw, nrhs, A_loc, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            block
                ...
                do j = 1, n
                    do k = 1, j - 1
                        A_ref(j, k) = conjg(A_ref(k, j))
                    end do
                end do
            end block
            B_ref = B_glob
            call zposv('U', n, nrhs, A_ref, n, B_ref, n, info_ref)
