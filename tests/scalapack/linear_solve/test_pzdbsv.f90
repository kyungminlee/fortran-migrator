program test_pzdbsv
    ! Complex diagonally-dominant banded solver — Z mirror of pddbsv.
    ...
    use pblas_grid,       only: grid_init, grid_exit, my_rank, my_nproc, &
                                ...
                                descinit_1d
    use target_scalapack, only: target_name, target_eps, target_pzdbsv
    use mpi
    implicit none
    ...
    call grid_init()
    call report_init('pzdbsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        call target_pzdbsv(n, bwl, bwu, nrhs, A_loc, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            B_ref = B_glob
            call zgesv(n, nrhs, A_full, n, ipiv, B_ref, n, info_ref)
