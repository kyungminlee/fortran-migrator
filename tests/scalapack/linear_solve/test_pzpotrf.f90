program test_pzpotrf
    ...
    call grid_init()
    call report_init('pzpotrf', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix_z(n, n, mb, nb, dummy_loc, M_glob, seed = 13401 + 31*i)
        deallocate(dummy_loc)

        ! Global Hermitian-PD A = M^H*M + n*I.
        allocate(A_glob(n, n))
        ...
        if (my_rank == 0) then
            allocate(A_ref(n, n))
            A_ref = A_glob
            call zpotrf('U', n, A_ref, n, info_ref)
