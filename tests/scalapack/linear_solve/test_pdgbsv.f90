program test_pdgbsv
    ! General banded LU solve (with pivoting). Local A storage:
    ! LLD_A = 2*bwl+2*bwu+1; element A(i,j) at packed row
    ! bwl+bwu+1+i-j of column j. Rows 1..bwl+bwu unused on input.
    ...
    call grid_init()
    call report_init('pdgbsv', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        if (my_rank == 0) then
            ...
            B_ref = B_glob
            call dgesv(n, nrhs, A_full, n, ipiv_ref, B_ref, n, info_ref)
        ...
        deallocate(A_loc, B_loc, work, ipiv_loc, A_full, B_glob)
    end do

    call report_finalize()
    call grid_exit()
end program test_pdgbsv
