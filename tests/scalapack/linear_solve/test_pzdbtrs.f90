program test_pzdbtrs
    ! Complex diagonally-dominant banded factor (pzdbtrf) + solve (pzdbtrs).
    ...
    call grid_init()
    call report_init('pzdbtrs', target_name, my_rank)
    ...
    do i = 1, size(ns)
        ...
        ! pxdbtrs/pxdbtrsv WORK_SIZE_MIN = max(bwl,bwu)*nrhs, but the
        ! local-update XLAMOV at pxdbtrsv.f:1500 writes BWU rows x NRHS
