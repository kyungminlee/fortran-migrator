program test_pdpbtrsv
    ! Smoke test: factor SPD banded with pdpbtrf, then call pdpbtrsv
    ! twice (UPLO='U'/'L'). Same caveat as pdpttrsv — the trsv halves
    ! don't compose to the full pdpbtrs solve.
    ...
    use target_scalapack,  only: target_name, target_eps, &
                                 target_pdpbtrf, target_pdpbtrsv
    implicit none
    ...
    real(ep), allocatable :: A_full(:,:), M(:,:), B_glob(:,:)
    ...
    do i = 1, size(ns)
        ...
        lldB = nb
        allocate(B_loc(lldB, nrhs)); B_loc = 0.0_ep
        do ig = 1, n
            block
                integer :: blk, owner
        ...
        if (my_rank == 0) then
            ...
            write(label, '(a,i0,a,i0,a,i0,a,i0,a,i0)') 'n=', n, ',bw=', bw, &
                ',nrhs=', nrhs, ',info_N=', info_u, ',info_T=', info_t
