program test_pdpttrs
    ! PD tridiagonal factor (pdpttrf) + solve (pdpttrs) split.
    ...
    call grid_init()
    call report_init('pdpttrs', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        nb = (n + my_nproc - 1) / my_nproc
        call gen_vector_quad(n,     d_glob, seed = 18501 + 31*i)
        call gen_vector_quad(n,     e_glob, seed = 18511 + 31*i)
        call gen_matrix_quad(n, nrhs, B_glob, seed = 18521 + 31*i)
