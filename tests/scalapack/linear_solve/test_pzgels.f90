program test_pzgels
    ...
    call grid_init()
    call report_init('pzgels', target_name, my_rank)
    ...
    do ic = 1, cases
        ...
        call gather_matrix_z(m_big, nrhs, mb, nb, B_loc, B_got)

        if (my_rank == 0) then
            allocate(A_ref(m, n), B_ref(m_big, nrhs), work_ref(max(1, max(m, n) * 64)))
            A_ref = A_glob
            B_ref = B_glob
            call zgels(transes(ic), m, n, nrhs, A_ref, m, B_ref, m_big, &
