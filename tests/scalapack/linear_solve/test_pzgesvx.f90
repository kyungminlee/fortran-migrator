program test_pzgesvx
    ! Expert complex general solver. FACT='N'/EQUED='N'/TRANS='N'.
    ...
    complex(ep), allocatable :: A_ref(:,:), B_ref(:,:)
    real(ep), allocatable :: R_vec(:), C_vec(:), ferr(:), berr(:)
    complex(ep), allocatable :: work(:)
    real(ep), allocatable :: rwork(:)
    integer,  allocatable :: ipiv(:), ipiv_ref(:)
    real(ep) :: err, tol, rcond
    character(len=1) :: equed
    character(len=48) :: label

    call grid_init()
    call report_init('pzgesvx', target_name, my_rank)

    do i = 1, size(ns)
        n = ns(i)
        call gen_distrib_matrix_z(n, n,    mb, nb, A_loc, A_glob, seed = 29301 + 31*i)
        call gen_distrib_matrix_z(n, nrhs, mb, nb, B_loc, B_glob, seed = 29311 + 31*i)
        ...
        call target_pzgesvx('N', 'N', n, nrhs, A_loc, 1, 1, desca, &
        ...
        if (my_rank == 0) then
            ...
            call zgesv(n, nrhs, A_ref, n, ipiv_ref, B_ref, n, info_ref)
        ...
        deallocate(A_loc, A_glob, B_loc, B_glob, AF_loc, X_loc, &
                   ipiv, R_vec, C_vec, ferr, berr, work, rwork)
    end do

    call report_finalize()
    call grid_exit()
end program test_pzgesvx
