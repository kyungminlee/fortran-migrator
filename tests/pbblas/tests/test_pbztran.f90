! Test PBZTRAN — distributed conjugate-transpose between column-block
! and row-block distributions. Same scaffolding as test_pbdtran but
! with TRANS = 'C' so the operation is C := A**H + beta*C.
...
contains
    ...
    subroutine run_case_R()
        ! ADIST='R': mirror of run_case_R in test_pbdtran.f90.
        integer, parameter :: m = nb
        integer, parameter :: n = 8
        integer, parameter :: iarow = 0, iacol = 0, icrow = 0, iccol = 0
        integer :: nq_loc, np_loc, lda, ldc, lwork
        integer :: bytes_per_elem, ig, jg, locm_C, locn_C, src_rank, peer
        integer, parameter :: tag_C = 7411
        complex(ep), allocatable :: A_glob(:,:), A_loc(:,:), C_loc(:,:), C0_loc(:,:)
        complex(ep), allocatable :: C_glob_got(:,:), C_glob_ref(:,:), C0_glob(:,:)
        complex(ep), allocatable :: buf2d(:,:)
        complex(ep) :: beta
        ...
        call gen_matrix_complex(m, n, A_glob, seed = 4701)
        ...
        call gen_matrix_complex(n, m, C0_glob, seed = 4801)
