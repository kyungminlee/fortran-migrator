! Test PBZTRNV — replicated input/output paths (IXCOL=-1, IYROW=-1).
!
! Closes the IXCOL=-1 / IYROW=-1 coverage gap previously listed in
! tests/pbblas/TODO.md. Mirror of test_pbdtrnv_replicated for the
! complex variant with TRANS='C' (conjugate transpose). See that
! file for full commentary on the two replicated paths.
program test_pbztrnv_replicated
    ...
    integer, parameter :: tag_Y0 = 7621, tag_YN = 7622
    complex(ep), allocatable :: X_glob(:), X_loc(:), Y_loc(:)
    complex(ep), allocatable :: Y_glob_got(:), Y_glob_lastrow(:), Y_glob_ref(:)
    complex(ep), allocatable :: buf1d(:)
    complex(ep) :: beta
    real(ep)    :: err, err_rep, tol
    character(len=64) :: label
    integer :: pc, locn, jl, last_row

    call grid_init()
    call report_init('pbztrnv_replicated', target_name, my_rank)
    ...
    call gen_vector_complex(n, X_glob, seed = 9401)
    ...
    if (my_rank == 0) then
        ...
        err_rep = max_rel_err_vec_z(Y_glob_lastrow, Y_glob_got)
    ...
    call grid_exit()
end program test_pbztrnv_replicated
