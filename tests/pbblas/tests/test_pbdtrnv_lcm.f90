! Test PBDTRNV — non-square grid exercising LCMQ > 1.
!
! Closes the "larger / non-square grids where LCMP > 1 or LCMQ > 1"
! gap in tests/pbblas/TODO.md (XDIST='C' half — see test_pbztrnv_lcm
! for the LCMP > 1 mirror).
!
! Uses grid_init_shape(nprow=4, npcol=1) so LCM(4,1)=4, LCMP=1, LCMQ=4.
! In XDIST='C' the upstream WORK-size formula CEIL(Nqb,LCMQ)*NB and
! the I=0..LCM-1 dispatch loop in pbdtrnv.f only do non-trivial work
! when LCMQ > 1; on a 1×1 / 2×2 grid LCMQ=1 and the loop collapses.
!
! Requires connected MPI — the sandbox's plain mpiexec spawns
! unconnected worlds where blacs_pinfo reports my_nproc=1 and the
! grid_init_shape mismatch check skips. Use the linux-impi preset
! (Intel MPI with FI_PROVIDER=tcp + I_MPI_HYDRA_BOOTSTRAP=fork).
program test_pbdtrnv_lcm
    ...
    ! 4×1 grid: NPROW=4, NPCOL=1 ⇒ LCMP=1, LCMQ=4.
    call grid_init_shape(4, 1)
    call report_init('pbdtrnv_lcm', target_name, my_rank)
    ...
    call gen_vector_quad(n, X_glob, seed = 9701)
