...
program test_pbzvecadd
    ...
    do k = 1, size(modes)
        do i = 1, size(cases)
            n = cases(i)
            call gen_vector_complex(n, X,  seed = 911 + 13 * i + 100 * my_rank + 1000 * k)
            call gen_vector_complex(n, Y0, seed = 1011 + 13 * i + 100 * my_rank + 1000 * k)
            allocate(Y_got(n), Y_ref(n))
            Y_got = Y0; Y_ref = Y0

            call target_pbzvecadd(my_context, modes(k)(1:1), n, alpha, X, 1, &
                                  beta, Y_got, 1)

            if (modes(k)(1:1) == 'C' .or. modes(k)(1:1) == 'c') then
                Y_ref = alpha * conjg(X) + beta * Y_ref
            else
