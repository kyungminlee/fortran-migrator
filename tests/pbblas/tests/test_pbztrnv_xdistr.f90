! Test PBZTRNV — XDIST='R' (row-vector input → column-vector output).
!
! Mirror of test_pbztrnv.f90 with row/column distribution roles swapped.
! Closes the row-vector half of the pbztrnv coverage gap noted in
! tests/pbblas/TODO.md. TRANS='C' so reference is conjg(X_glob).
program test_pbztrnv_xdistr
    ...
    call grid_init()
    call report_init('pbztrnv_xdistr', target_name, my_rank)
    ...
    call gen_vector_complex(n, X_glob, seed = 9211)
    ...
    call grid_exit()
end program test_pbztrnv_xdistr
