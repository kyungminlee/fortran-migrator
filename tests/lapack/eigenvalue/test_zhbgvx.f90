! zhbgvx: complex Hermitian banded gen-eig expert driver.
program test_zhbgvx
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_hpd_matrix_quad, pack_herm_band_quad
    use target_lapack,   only: target_name, target_eps, target_zhbgvx
    use ref_quad_lapack, only: zhbgvx
    implicit none
    ...
    complex(ep), allocatable :: AB_r(:,:), BB_r(:,:), Q_r(:,:), Z_r(:,:), work(:)
    complex(ep), allocatable :: AB_g(:,:), BB_g(:,:), Q_g(:,:), Z_g(:,:)
    real(ep), allocatable :: w_r(:), w_g(:), rwork(:)
    integer, allocatable :: iwork(:), ifail_r(:), ifail_g(:)
    real(ep) :: vl, vu, abstol, err, tol
    character(len=48) :: label

    call report_init('zhbgvx', target_name)
    do i = 1, size(ns)
        n = ns(i); ka = kas(i); kb = kbs(i)
        ldab = ka + 1; ldbb = kb + 1
        call gen_hpd_matrix_quad(n, A0, seed = 22101 + 47 * i)
        call gen_hpd_matrix_quad(n, B0, seed = 22111 + 47 * i)
        ...
        allocate(work(n), rwork(7*n), iwork(5*n))
        call zhbgvx('N', 'A', 'U', n, ka, kb, AB_r, ldab, BB_r, ldbb, &
                    Q_r, n, vl, vu, 1, n, abstol, m_r, w_r, Z_r, n, &
                    work, rwork, iwork, ifail_r, info)
        deallocate(work, rwork, iwork)
        call target_zhbgvx('N', 'A', 'U', n, ka, kb, AB_g, ldab, BB_g, ldbb, &
