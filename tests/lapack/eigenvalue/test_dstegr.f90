! dstegr: MRRR tridiagonal eigensolver, eigenvalues only.
program test_dstegr
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use target_lapack,   only: target_name, target_eps, target_dstegr
    use ref_quad_lapack, only: dstegr
    implicit none
    ...
    call report_init('dstegr', target_name)
    do i = 1, size(ns)
        n = ns(i)
        ...
        call dstegr('N', 'A', n, D_r, E_r, 0.0_ep, 0.0_ep, 0, 0, 0.0_ep, &
        ...
        call target_dstegr('N', 'A', n, D_g, E_g, 0.0_ep, 0.0_ep, 0, 0, 0.0_ep, &
