program test_dspev
    ...
    use target_lapack,   only: target_name, target_eps, target_dspev
    use ref_quad_lapack, only: dspev
    implicit none
    ...
    call report_init('dspev', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_symmetric_matrix_quad(n, A0, seed = 53001 + 47 * i)
        do jz = 1, size(jobzs)
            allocate(AP_ref(np), AP_got(np))
            allocate(w_ref(n), w_got(n), z_ref(n, n), z_got(n, n))
            allocate(work(max(1, 3*n)))
            call pack_sym_packed_quad('U', n, A0, AP_ref)
            AP_got = AP_ref
            call dspev(jobzs(jz), 'U', n, AP_ref, w_ref, z_ref, n, work, info)
            call target_dspev(jobzs(jz), 'U', n, AP_got, w_got, z_got, n, info)
            err = max_rel_err_vec(w_got, w_ref)
            tol = 16.0_ep * real(n, ep)**2 * target_eps
            write(label, '(a,a,a,i0)') 'jobz=', jobzs(jz), ',n=', n
            call report_case(trim(label), err, tol)
            deallocate(AP_ref, AP_got, w_ref, w_got, z_ref, z_got, work)
        end do
    end do
    call report_finalize()
end program test_dspev
