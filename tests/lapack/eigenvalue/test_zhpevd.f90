program test_zhpevd
    ...
    use target_lapack,   only: target_name, target_eps, target_zhpevd
    use ref_quad_lapack, only: zhpevd
    implicit none

    integer, parameter :: ns(*) = [16, 32, 64]
    character(len=1), parameter :: jobzs(2) = ['N', 'V']
    integer :: i, n, info, jz, np, lwork, lrwork, liwork, iwopt(1)
    ...
    call report_init('zhpevd', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_hermitian_matrix_quad(n, A0, seed = 57001 + 47 * i)
        do jz = 1, size(jobzs)
            allocate(AP_ref(np), AP_got(np))
            allocate(w_ref(n), w_got(n), z_ref(n, n), z_got(n, n))
            call pack_herm_packed_quad('U', n, A0, AP_ref)
            AP_got = AP_ref
            call zhpevd(jobzs(jz), 'U', n, AP_ref, w_ref, z_ref, n, &
            ...
            call target_zhpevd(jobzs(jz), 'U', n, AP_got, w_got, z_got, n, info)
