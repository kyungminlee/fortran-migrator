! zgeesx: complex Schur with sensitivity. SORT='N', SENSE='N'.
program test_zgeesx
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zgeesx
    use ref_quad_lapack, only: zgeesx
    implicit none
    ...
    complex(ep) :: wopt(1)
    real(ep) :: rce_r, rcv_r, rce_g, rcv_g, err, tol
    character(len=48) :: label

    call report_init('zgeesx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_complex(n, n, A, seed = 103001 + 47 * i)
        allocate(Aref(n,n), Agot(n,n), W_r(n), W_g(n), VS_r(n,n), VS_g(n,n))
        allocate(rwork(n), bwork(n), mods_r(n), mods_g(n))
        Aref = A; Agot = A
        call zgeesx('N', 'N', sel_all_c, 'N', n, Aref, n, sdim_r, W_r, &
                    VS_r, n, rce_r, rcv_r, wopt, -1, rwork, bwork, info)
        lwork = max(1, int(real(wopt(1), ep)))
        allocate(work(lwork))
        call zgeesx('N', 'N', sel_all_c, 'N', n, Aref, n, sdim_r, W_r, &
                    VS_r, n, rce_r, rcv_r, work, lwork, rwork, bwork, info)
        deallocate(work)
        call target_zgeesx('N', 'N', sel_all_c, 'N', n, Agot, n, sdim_g, &
                           W_g, VS_g, n, rce_g, rcv_g, info)
        do j = 1, n
            mods_r(j) = abs(W_r(j))
            mods_g(j) = abs(W_g(j))
        end do
