! dstein: tridiagonal eigenvectors via inverse iteration.
program test_dstein
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use target_lapack,   only: target_name, target_eps, target_dstein
    use ref_quad_lapack, only: dstebz, dstein
    implicit none

    integer, parameter :: ns(*) = [10, 32]
    integer :: i, n, m, nsplit, info, j
    real(ep), allocatable :: D(:), E(:), W(:), Z_r(:,:), Z_g(:,:), work(:)
    integer, allocatable :: iblock(:), isplit(:), iwork(:), ifail_r(:), ifail_g(:)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('dstein', target_name)
    do i = 1, size(ns)
        n = ns(i)
        ...
        allocate(work(5*n))
        call dstein(n, D, E, m, W, iblock, isplit, Z_r, n, work, iwork, ifail_r, info)
        deallocate(work)
        call target_dstein(n, D, E, m, W, iblock, isplit, Z_g, n, ifail_g, info)
        err = max_rel_err_mat(Z_g, Z_r)
