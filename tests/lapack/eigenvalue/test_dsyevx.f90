program test_dsyevx
    ...
    use target_lapack,   only: target_name, target_eps, target_dsyevx
    use ref_quad_lapack, only: dsyevx
    implicit none
    ...
    call report_init('dsyevx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 41001 + 47 * i)
        abstol = 0.0_ep; vl = 0.0_ep; vu = 0.0_ep
        do jz = 1, size(jobzs)
            allocate(A_ref(n,n), A_got(n,n), w_ref(n), w_got(n))
            allocate(z_ref(n,n), z_got(n,n), ifail(n), iwork(5*n))
            A_ref = A0; A_got = A0
            call dsyevx(jobzs(jz), 'A', 'U', n, A_ref, n, vl, vu, 1, n, &
            ...
            deallocate(work)
            call target_dsyevx(jobzs(jz), 'A', 'U', n, A_got, n, vl, vu, 1, n, &
            ...
            deallocate(A_ref, A_got, w_ref, w_got, z_ref, z_got, ifail, iwork)
        end do
    end do
