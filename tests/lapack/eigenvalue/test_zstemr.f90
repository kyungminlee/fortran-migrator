! zstemr: MRRR tridiagonal eigensolver (complex Z).
program test_zstemr
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use target_lapack,   only: target_name, target_eps, target_zstemr
    use ref_quad_lapack, only: zstemr
    implicit none
    ...
    call report_init('zstemr', target_name)
    do i = 1, size(ns)
        n = ns(i); nzc = 0
        ...
        call zstemr('N', 'A', n, D_r, E_r, 0.0_ep, 0.0_ep, 0, 0, &
        ...
        call target_zstemr('N', 'A', n, D_g, E_g, 0.0_ep, 0.0_ep, 0, 0, &
