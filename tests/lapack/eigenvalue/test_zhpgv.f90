program test_zhpgv
    ...
    use target_lapack,   only: target_name, target_eps, target_zhpgv
    use ref_quad_lapack, only: zhpgv
    implicit none
    ...
    call report_init('zhpgv', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_hermitian_matrix_quad(n, A, seed = 250021 + 47 * i)
        call gen_hpd_matrix_quad(n, B, seed = 250031 + 47 * i)
        ...
        call zhpgv(1, 'N', 'U', n, AP_ref, BP_ref, W_ref, Z, 1, work, rwork, info)
        call target_zhpgv(1, 'N', 'U', n, AP_got, BP_got, W_got, Z, 1, info)
        err = max_rel_err_vec(W_got, W_ref)
        tol = 16.0_ep * real(n, ep)**2 * target_eps
        write(label, '(a,i0)') 'n=', n
        call report_case(trim(label), err, tol)
        deallocate(A, B, work, rwork, AP_ref, BP_ref, AP_got, BP_got, W_ref, W_got, Z)
    end do
    call report_finalize()
end program test_zhpgv
