program test_zgesdd
    ...
    use target_lapack,   only: target_name, target_eps, target_zgesdd
    use ref_quad_lapack, only: zgesdd
    implicit none
    ...
    complex(ep), allocatable :: work(:), US(:,:), USVt(:,:)
    ...
    call report_init('zgesdd', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); kmn = min(m, n)
        call gen_matrix_complex(m, n, A0, seed = 30001 + 47 * i)
        lrwork = max(1, 5 * kmn * kmn + 7 * kmn)
        allocate(A_ref(m,n), A_got(m,n), s_ref(kmn), s_got(kmn))
        allocate(U_ref(m,m), U_got(m,m), VT_ref(n,n), VT_got(n,n))
        allocate(rwork(lrwork), iwork(8*kmn))
        A_ref = A0; A_got = A0
        call zgesdd('A', m, n, A_ref, m, s_ref, U_ref, m, VT_ref, n, &
                    wopt, -1, rwork, iwork, info)
        lwork = max(1, int(real(wopt(1), ep)))
        allocate(work(lwork))
        call zgesdd('A', m, n, A_ref, m, s_ref, U_ref, m, VT_ref, n, &
                    work, lwork, rwork, iwork, info)
        deallocate(work)
        call target_zgesdd('A', m, n, A_got, m, s_got, U_got, m, VT_got, n, info)
        ...
        deallocate(A_ref, A_got, s_ref, s_got, U_ref, U_got, VT_ref, VT_got, rwork, iwork, US, USVt)
    end do
    call report_finalize()
end program test_zgesdd
