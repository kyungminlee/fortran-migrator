! zlansp: norm of a complex symmetric packed matrix.
program test_zlansp
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: rel_err_scalar
    use test_data,       only: gen_complex_symmetric_quad, pack_herm_packed_quad
    use target_lapack,   only: target_name, target_eps, target_zlansp
    use ref_quad_lapack, only: zlansp
    implicit none
    ...
    call report_init('zlansp', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A, seed = 18851 + 47 * i)
        allocate(AP(n*(n+1)/2))
        do u = 1, size(uplos)
            call pack_herm_packed_quad(uplos(u), n, A, AP)
            allocate(work(max(1, n)))
            do k = 1, size(norms)
                ref_val = zlansp(norms(k), uplos(u), n, AP, work)
                got_val = target_zlansp(norms(k), uplos(u), n, AP)
