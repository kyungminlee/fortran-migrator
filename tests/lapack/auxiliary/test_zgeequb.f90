program test_zgeequb
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec, rel_err_scalar
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zgeequb
    use ref_quad_lapack, only: zgeequb
    implicit none
    ...
    call report_init('zgeequb', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_complex(n, n, A, seed = 210101 + 47 * i)
        allocate(R_ref(n), C_ref(n), R_got(n), C_got(n))
        call zgeequb(n, n, A, n, R_ref, C_ref, rcnd_ref, ccnd_ref, am_ref, info)
        call target_zgeequb(n, n, A, n, R_got, C_got, rcnd_got, ccnd_got, am_got, info)
