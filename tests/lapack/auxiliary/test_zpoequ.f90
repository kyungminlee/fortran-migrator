program test_zpoequ
    ...
    use target_lapack,   only: target_name, target_eps, target_zpoequ
    ...
    call report_init('zpoequ', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hpd_matrix_quad(n, A, seed = 200121 + 47 * i)
        allocate(S_ref(n), S_got(n))
        call zpoequ(n, A, n, S_ref, scnd_ref, am_ref, info)
        call target_zpoequ(n, A, n, S_got, scnd_got, am_got, info)
