! zlanhb: norm of a complex Hermitian banded matrix.
program test_zlanhb
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: rel_err_scalar
    use test_data,       only: gen_hermitian_matrix_quad, pack_herm_band_quad
    use target_lapack,   only: target_name, target_eps, target_zlanhb
    use ref_quad_lapack, only: zlanhb
    implicit none
    ...
    call report_init('zlanhb', target_name)
    do i = 1, size(ns)
        n = ns(i); kd = kds(i); ldab = kd + 1
        call gen_hermitian_matrix_quad(n, A, seed = 18701 + 47 * i)
        allocate(AB(ldab, n))
        do u = 1, size(uplos)
            call pack_herm_band_quad(uplos(u), n, kd, A, AB)
            allocate(work(max(1, n)))
            do k = 1, size(norms)
                ref_val = zlanhb(norms(k), uplos(u), n, kd, AB, ldab, work)
                got_val = target_zlanhb(norms(k), uplos(u), n, kd, AB, ldab)
