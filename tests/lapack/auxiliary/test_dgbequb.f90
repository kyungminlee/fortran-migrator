program test_dgbequb
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec, rel_err_scalar
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dgbequb
    use ref_quad_lapack, only: dgbequb
    implicit none
    ...
    real(ep), allocatable :: A(:,:), AB(:,:), R_ref(:), C_ref(:), R_got(:), C_got(:)
    real(ep) :: rcnd_ref, ccnd_ref, am_ref, rcnd_got, ccnd_got, am_got
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('dgbequb', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = kl + ku + 1
        call gen_matrix_quad(n, n, A, seed = 210011 + 47 * i)
        allocate(AB(ldab, n))
        AB = 0.0_ep
        do j = 1, n
        ...
        call dgbequb(n, n, kl, ku, AB, ldab, R_ref, C_ref, rcnd_ref, ccnd_ref, am_ref, info)
        call target_dgbequb(n, n, kl, ku, AB, ldab, R_got, C_got, rcnd_got, ccnd_got, am_got, info)
