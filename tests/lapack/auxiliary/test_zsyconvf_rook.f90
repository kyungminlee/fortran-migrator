program test_zsyconvf_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf_rook, target_zsyconvf_rook
    use ref_quad_lapack, only: zsytrf_rook, zsyconvf_rook
    implicit none
    ...
    call report_init('zsyconvf_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 72501 + 19 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call zsyconvf_rook(uplos(ju), 'C', n, A_ref, n, e_ref, ipiv_ref, info)
            call target_zsyconvf_rook(uplos(ju), 'C', n, A_got, n, e_got, ipiv_got, info)
...
end program test_zsyconvf_rook
