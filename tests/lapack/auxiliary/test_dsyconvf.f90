program test_dsyconvf
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf, target_dsyconvf
    use ref_quad_lapack, only: dsytrf, dsyconvf
    implicit none
    ...
    call report_init('dsyconvf', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 72101 + 19 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call dsyconvf(uplos(ju), 'C', n, A_ref, n, e_ref, ipiv_ref, info)
            call target_dsyconvf(uplos(ju), 'C', n, A_got, n, e_got, ipiv_got, info)
