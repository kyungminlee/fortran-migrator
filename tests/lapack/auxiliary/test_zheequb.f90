program test_zheequb
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec, rel_err_scalar
    use test_data,       only: gen_hpd_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_zheequb
    use ref_quad_lapack, only: zheequb
    implicit none
    ...
    call report_init('zheequb', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hpd_matrix_quad(n, A, seed = 210141 + 47 * i)
        allocate(S_ref(n), S_got(n), work(2*n))
        call zheequb('U', n, A, n, S_ref, scnd_ref, am_ref, work, info)
        call target_zheequb('U', n, A, n, S_got, scnd_got, am_got, info)
