program test_dggbal
    ...
    use target_lapack,   only: target_name, target_eps, target_dggbal
    use ref_quad_lapack, only: dggbal
    implicit none
    ...
    call report_init('dggbal', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_quad(n, n, A0, seed = 310001 + 47 * i)
        call gen_matrix_quad(n, n, B0, seed = 310011 + 47 * i)
        ...
        call dggbal('B', n, A_ref, n, B_ref, n, ilo_ref, ihi_ref, ls_ref, rs_ref, work, info)
        call target_dggbal('B', n, A_got, n, B_got, n, ilo_got, ihi_got, ls_got, rs_got, info)
