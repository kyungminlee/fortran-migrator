program test_dgbequ
    ...
    use target_lapack,   only: target_name, target_eps, target_dgbequ
    use ref_quad_lapack, only: dgbequ
    implicit none
    ...
    call report_init('dgbequ', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = kl + ku + 1
        call gen_matrix_quad(n, n, A, seed = 200011 + 47 * i)
        allocate(AB(ldab, n))
        AB = 0.0_ep
        do j = 1, n
        ...
        call dgbequ(n, n, kl, ku, AB, ldab, R_ref, C_ref, rcnd_ref, ccnd_ref, am_ref, info)
        call target_dgbequ(n, n, kl, ku, AB, ldab, R_got, C_got, rcnd_got, ccnd_got, am_got, info)
