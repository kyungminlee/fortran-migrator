program test_zsyconv
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf, target_zsyconv
    use ref_quad_lapack, only: zsytrf, zsyconv
    implicit none
    ...
    call report_init('zsyconv', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 72301 + 19 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call zsyconv(uplos(ju), 'C', n, A_ref, n, ipiv_ref, e_ref, info)
            call target_zsyconv(uplos(ju), 'C', n, A_got, n, ipiv_got, e_got, info)
