program test_zhetrd
    ...
    use target_lapack,   only: target_name, target_eps, target_zhetrd
    ...
    call report_init('zhetrd', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A0, seed = 220011 + 47 * i)
        allocate(A_ref(n,n), A_got(n,n))
        allocate(D_ref(n), E_ref(n-1), tau_ref(n-1))
        allocate(D_got(n), E_got(n-1), tau_got(n-1))
        A_ref = A0; A_got = A0
        call zhetrd('U', n, A_ref, n, D_ref, E_ref, tau_ref, wopt, -1, info)
        lwork = max(1, int(real(wopt(1), ep)))
        allocate(work(lwork))
        call zhetrd('U', n, A_ref, n, D_ref, E_ref, tau_ref, work, lwork, info)
        call target_zhetrd('U', n, A_got, n, D_got, E_got, tau_got, info)
