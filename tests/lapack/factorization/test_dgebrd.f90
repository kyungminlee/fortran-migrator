program test_dgebrd
    ...
    use target_lapack,   only: target_name, target_eps, target_dgebrd
    ...
    real(ep), allocatable :: D_ref(:), E_ref(:), tauq_ref(:), taup_ref(:)
    real(ep), allocatable :: D_got(:), E_got(:), tauq_got(:), taup_got(:)
    real(ep) :: wopt(1), err, tol
    character(len=48) :: label

    call report_init('dgebrd', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); mn = min(m, n)
        call gen_matrix_quad(m, n, A0, seed = 240001 + 47 * i)
        ...
        A_ref = A0; A_got = A0
        call dgebrd(m, n, A_ref, m, D_ref, E_ref, tauq_ref, taup_ref, wopt, -1, info)
        lwork = max(1, int(wopt(1)))
        allocate(work(lwork))
        call dgebrd(m, n, A_ref, m, D_ref, E_ref, tauq_ref, taup_ref, work, lwork, info)
        call target_dgebrd(m, n, A_got, m, D_got, E_got, tauq_got, taup_got, info)
