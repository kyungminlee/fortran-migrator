program test_zsytrf
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf
    use ref_quad_lapack, only: zsytrf
    implicit none
    ...
    call report_init('zsytrf', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 60001 + 47 * i)
