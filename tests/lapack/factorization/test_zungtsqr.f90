! zungtsqr: form Q from a TSQR factorization (complex).
program test_zungtsqr
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zungtsqr
    use ref_quad_lapack, only: zlatsqr, zungtsqr
    implicit none
    ...
    call report_init('zungtsqr', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); mb = mbs(i); nb = nbs(i)
        call gen_matrix_complex(m, n, A0, seed = 23251 + 71 * i)
        ...
        deallocate(work)
        A_got = A_ref
        call zungtsqr(m, n, mb, nb, A_ref, m, T_arr, nb, wopt, -1, info)
        lwork = max(1, int(real(wopt(1), ep))); allocate(work(lwork))
        call zungtsqr(m, n, mb, nb, A_ref, m, T_arr, nb, work, lwork, info)
        deallocate(work)
        call target_zungtsqr(m, n, mb, nb, A_got, m, T_arr, nb, info)
