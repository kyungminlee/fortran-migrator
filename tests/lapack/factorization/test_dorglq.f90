program test_dorglq
    ...
    use target_lapack,   only: target_name, target_eps, target_dgelqf, target_dorglq
    ...
    call report_init('dorglq', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n, A0, seed = 39001 + 47 * i)
        ...
        ! Form Q (m rows, since m <= n).
        call dorglq(m, n, m, A_ref, m, tau_ref, wopt, -1, info)
        lwork = max(1, int(wopt(1)))
        allocate(work(lwork))
        call dorglq(m, n, m, A_ref, m, tau_ref, work, lwork, info)
        deallocate(work)
        call target_dorglq(m, n, m, A_got, m, tau_got, info)
