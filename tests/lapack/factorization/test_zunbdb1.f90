! zunbdb1: complex unbdb variant. Q ≤ MIN(P, M-P, M-Q).
program test_zunbdb1
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zunbdb1
    use ref_quad_lapack, only: zgeqrf, zungqr, zunbdb1
    implicit none
    ...
    call report_init('zunbdb1', target_name)
    do i = 1, size(ms)
        m = ms(i); p = m/2; q = m/3
        call gen_matrix_complex(m, m, A, seed = 24351 + 79 * i)
        allocate(tau(m))
        ...
        allocate(theta_r(q), theta_g(q), phi_r(max(1,q-1)), phi_g(max(1,q-1)), &
                 tp1(p), tp2(m-p), tq1(q))
        call zunbdb1(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, wopt, -1, info)
        lwork = max(1, int(real(wopt(1), ep))); allocate(work(lwork))
        call zunbdb1(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, work, lwork, info)
        deallocate(work)
        call target_zunbdb1(m, p, q, X11g, p, X21g, m-p, theta_g, phi_g, &
                            tp1, tp2, tq1, info)
