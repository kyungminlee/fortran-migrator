program test_zhetrf_rk
    ...
    use target_lapack,   only: target_name, target_eps, target_zhetrf_rk
    use ref_quad_lapack, only: zhetrf_rk
    implicit none
    ...
    call report_init('zhetrf_rk', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A0, seed = 49001 + 47 * i)
