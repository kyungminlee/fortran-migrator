program test_zsptrf
    ...
    use target_lapack,   only: target_name, target_eps, target_zsptrf
    use ref_quad_lapack, only: zsptrf
    implicit none
    ...
    call report_init('zsptrf', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_complex_symmetric_quad(n, A0, seed = 84001 + 47 * i)
