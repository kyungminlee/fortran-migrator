program test_zhetrf_aa_2stage
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z, max_rel_err_vec_z
    use test_data,       only: gen_hermitian_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_zhetrf_aa_2stage
    use ref_quad_lapack, only: zhetrf_aa_2stage
    implicit none
    ...
    call report_init('zhetrf_aa_2stage', target_name)
    do i = 1, size(ns)
        n = ns(i)
...
end program test_zhetrf_aa_2stage
