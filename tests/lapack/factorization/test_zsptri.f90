program test_zsptri
    ...
    use target_lapack,   only: target_name, target_eps, target_zsptrf, target_zsptri
    use ref_quad_lapack, only: zsptrf, zsptri
    implicit none
    ...
    call report_init('zsptri', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_complex_symmetric_quad(n, A0, seed = 87001 + 47 * i)
        ...
        call zsptri('U', n, AP_ref, ipiv_ref, work, info)
        call target_zsptri('U', n, AP_got, ipiv_got, info)
