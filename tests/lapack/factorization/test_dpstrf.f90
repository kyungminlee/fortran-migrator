! dpstrf: pivoted Cholesky of an SPD matrix.
program test_dpstrf
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_spd_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dpstrf
    use ref_quad_lapack, only: dpstrf
    implicit none
    ...
    call report_init('dpstrf', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_spd_matrix_quad(n, A0, seed = 21301 + 67 * i)
        do u = 1, size(uplos)
            allocate(A_ref(n, n), A_got(n, n), piv_ref(n), piv_got(n), work(2*n))
            A_ref = A0; A_got = A0
            call dpstrf(uplos(u), n, A_ref, n, piv_ref, rank_ref, -1.0_ep, work, info_ref)
            call target_dpstrf(uplos(u), n, A_got, n, piv_got, rank_got, &
                               -1.0_ep, info_got)
