! dorbdb2: variant of orbdb for the case P ≤ min(M-P, Q, M-Q).
program test_dorbdb2
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dorbdb2
    use ref_quad_lapack, only: dgeqrf, dorgqr, dorbdb2
    implicit none
    ...
    call report_init('dorbdb2', target_name)
    do i = 1, size(ms)
        m = ms(i); p = m/4; q = m/2  ! P ≤ M-P, P ≤ Q, P ≤ M-Q
        call gen_matrix_quad(m, m, A, seed = 24401 + 79 * i)
        allocate(tau(m))
        ...
        allocate(theta_r(q), theta_g(q), phi_r(max(1,q-1)), phi_g(max(1,q-1)), &
                 tp1(p), tp2(m-p), tq1(q))
        call dorbdb2(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, wopt, -1, info)
        lwork = max(1, int(wopt(1))); allocate(work(lwork))
        call dorbdb2(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, work, lwork, info)
        deallocate(work)
        call target_dorbdb2(m, p, q, X11g, p, X21g, m-p, theta_g, phi_g, &
                            tp1, tp2, tq1, info)
        ! Only theta(1:p) is filled by orbdb2 (loop runs 1..P).
        err = max_rel_err_vec(theta_g(1:p), theta_r(1:p))
