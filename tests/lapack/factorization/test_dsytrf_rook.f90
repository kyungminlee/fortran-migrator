program test_dsytrf_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf_rook
    use ref_quad_lapack, only: dsytrf_rook
    implicit none
    ...
    call report_init('dsytrf_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 16101 + 47 * i)
