program test_zhetrf
    ...
    use target_lapack,   only: target_name, target_eps, target_zhetrf
    use ref_quad_lapack, only: zhetrf
    implicit none
    ...
    call report_init('zhetrf', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A0, seed = 44001 + 47 * i)
        do j = 1, n
