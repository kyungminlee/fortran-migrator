! zunbdb4: complex unbdb variant. M-Q ≤ MIN(P, M-P, Q).
program test_zunbdb4
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zunbdb4
    use ref_quad_lapack, only: zgeqrf, zungqr, zunbdb4
    implicit none
    ...
    complex(ep), allocatable :: tp1(:), tp2(:), tq1(:), phantom(:)
    complex(ep) :: wopt(1)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('zunbdb4', target_name)
    do i = 1, size(ms)
        m = ms(i); p = m/2; q = 3*m/4
        call gen_matrix_complex(m, m, A, seed = 24651 + 79 * i)
        allocate(tau(m))
        ...
        call zunbdb4(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, phantom, wopt, -1, info)
        lwork = max(1, int(real(wopt(1), ep))); allocate(work(lwork))
        call zunbdb4(m, p, q, X11r, p, X21r, m-p, theta_r, phi_r, &
                     tp1, tp2, tq1, phantom, work, lwork, info)
        deallocate(work)
        call target_zunbdb4(m, p, q, X11g, p, X21g, m-p, theta_g, phi_g, &
                            tp1, tp2, tq1, phantom, info)
        ! Only theta(1:m-q) is filled by unbdb4 (loop runs 1..M-Q).
