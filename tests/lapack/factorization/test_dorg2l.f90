! dorg2l: unblocked generation of Q from QL.
program test_dorg2l
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_dgeqlf, target_dorg2l
    use ref_quad_lapack, only: dgeqlf, dorg2l
    implicit none
    ...
    call report_init('dorg2l', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n, A0, seed = 19601 + 67 * i)
        ...
        call dorg2l(m, n, n, A_ref, m, tau_ref, work, info)
        call target_dorg2l(m, n, n, A_got, m, tau_got, info)
