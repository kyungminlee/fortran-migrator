program test_zsytri_3x
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf_rk, target_zsytri_3x
    use ref_quad_lapack, only: zsytrf_rk, zsytri_3x
    implicit none
    ...
    call report_init('zsytri_3x', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 59301 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call zsytri_3x(uplos(ju), n, A_ref, n, e_ref, ipiv_ref, work2, nb, info)
            deallocate(work2)
            call target_zsytri_3x(uplos(ju), n, A_got, n, e_got, ipiv_got, nb, info)
