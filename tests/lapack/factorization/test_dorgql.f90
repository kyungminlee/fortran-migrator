! Build reflectors via dgeqlf, then form Q via dorgql.
program test_dorgql
    ...
    call report_init('dorgql', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n, A0, seed = 61001 + 61 * i)
        ...
        call dorgql(m, n, n, A_ref, m, tau_ref, wopt, -1, info)
        lwork = max(1, int(wopt(1)))
        allocate(work(lwork))
        call dorgql(m, n, n, A_ref, m, tau_ref, work, lwork, info)
        deallocate(work)
        call target_dorgql(m, n, n, A_got, m, tau_got, info)
