program test_zhetf2_rk
    ...
    use target_lapack,   only: target_name, target_eps, target_zhetf2_rk
    use ref_quad_lapack, only: zhetf2_rk
    implicit none
    ...
    call report_init('zhetf2_rk', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A0, seed = 49101 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            A_ref = A0; A_got = A0
            call zhetf2_rk(uplos(ju), n, A_ref, n, e_ref, ipiv_ref, info)
            call target_zhetf2_rk(uplos(ju), n, A_got, n, e_got, ipiv_got, info)
