program test_dsptrd
    ...
    use target_lapack,   only: target_name, target_eps, target_dsptrd
    use ref_quad_lapack, only: dsptrd
    implicit none

    integer, parameter :: ns(*) = [16, 32, 64]
    integer :: i, n, np, info
    real(ep), allocatable :: A(:,:), AP_ref(:), AP_got(:)
    real(ep), allocatable :: D_ref(:), E_ref(:), tau_ref(:)
    real(ep), allocatable :: D_got(:), E_got(:), tau_got(:)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('dsptrd', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_symmetric_matrix_quad(n, A, seed = 220021 + 47 * i)
        ...
        call dsptrd('U', n, AP_ref, D_ref, E_ref, tau_ref, info)
        call target_dsptrd('U', n, AP_got, D_got, E_got, tau_got, info)
