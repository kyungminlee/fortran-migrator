! zsytri2: blocked inversion of a complex symmetric matrix from sytrf.
program test_zsytri2
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z
    use test_data,       only: gen_complex_symmetric_quad
    use target_lapack,   only: target_name, target_eps, target_zsytrf, target_zsytri2
    use ref_quad_lapack, only: zsytrf, zsytri2
    implicit none
    ...
    call report_init('zsytri2', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 26051 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call zsytri2(uplos(ju), n, A_ref, n, ipiv_ref, wopt, -1, info)
            lwork = max(1, int(real(wopt(1), ep)))
            allocate(work(lwork))
            call zsytri2(uplos(ju), n, A_ref, n, ipiv_ref, work, lwork, info)
            deallocate(work)
            call target_zsytri2(uplos(ju), n, A_got, n, ipiv_got, info)
