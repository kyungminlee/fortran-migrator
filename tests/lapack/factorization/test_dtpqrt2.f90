! dtpqrt2: unblocked pentagonal QR factorization of [A; B].
program test_dtpqrt2
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dtpqrt2
    use ref_quad_lapack, only: dtpqrt2
    implicit none
    ...
    call report_init('dtpqrt2', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); l = ls(i)
        call gen_matrix_quad(n, n, A0, seed = 22301 + 79 * i)
        do j = 1, n-1
            A0(j+1:n, j) = 0.0_ep
        end do
        call gen_matrix_quad(m, n, B0, seed = 22311 + 79 * i)
        allocate(A_ref(n, n), A_got(n, n), B_ref(m, n), B_got(m, n), &
                 T_ref(n, n), T_got(n, n))
        A_ref = A0; A_got = A0; B_ref = B0; B_got = B0
        T_ref = 0.0_ep; T_got = 0.0_ep
        call dtpqrt2(m, n, l, A_ref, n, B_ref, m, T_ref, n, info)
        call target_dtpqrt2(m, n, l, A_got, n, B_got, m, T_got, n, info)
