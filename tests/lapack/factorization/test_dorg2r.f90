! dorg2r: unblocked generation of Q from QR (matches dorgqr modulo blocking).
program test_dorg2r
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_dgeqrf, target_dorg2r
    use ref_quad_lapack, only: dgeqrf, dorg2r
    implicit none
    ...
    call report_init('dorg2r', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n, A0, seed = 19501 + 61 * i)
        ...
        call dorg2r(m, n, n, A_ref, m, tau_ref, work, info)
        call target_dorg2r(m, n, n, A_got, m, tau_got, info)
