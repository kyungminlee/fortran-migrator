...
program test_zgelqt
    ...
    use target_lapack,   only: target_name, target_eps, target_zgelqt
    use ref_quad_lapack, only: zgelqt
    implicit none
    ...
    call report_init('zgelqt', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n, A0, seed = 380031 + 47 * i)
