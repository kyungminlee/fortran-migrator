program test_dsytri_3x
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf_rk, target_dsytri_3x
    use ref_quad_lapack, only: dsytrf_rk, dsytri_3x
    implicit none
    ...
    real(ep), allocatable :: A0(:,:), A_ref(:,:), A_got(:,:), e_ref(:), e_got(:), work(:), work2(:,:)
    integer,  allocatable :: ipiv_ref(:), ipiv_got(:)
    real(ep) :: wopt(1), err, tol
    character(len=48) :: label

    call report_init('dsytri_3x', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 19301 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call dsytri_3x(uplos(ju), n, A_ref, n, e_ref, ipiv_ref, work2, nb, info)
            deallocate(work2)
            call target_dsytri_3x(uplos(ju), n, A_got, n, e_got, ipiv_got, nb, info)
