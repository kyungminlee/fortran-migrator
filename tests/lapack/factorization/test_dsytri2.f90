! dsytri2: blocked inversion of a real symmetric matrix from sytrf.
program test_dsytri2
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_symmetric_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dsytrf, target_dsytri2
    use ref_quad_lapack, only: dsytrf, dsytri2
    implicit none
    ...
    call report_init('dsytri2', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 26001 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            call dsytri2(uplos(ju), n, A_ref, n, ipiv_ref, wopt, -1, info)
            lwork = max(1, int(wopt(1)))
            allocate(work(lwork))
            call dsytri2(uplos(ju), n, A_ref, n, ipiv_ref, work, lwork, info)
            deallocate(work)
            call target_dsytri2(uplos(ju), n, A_got, n, ipiv_got, info)
