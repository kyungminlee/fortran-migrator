program test_zpbtrf
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z
    use test_data,       only: gen_hpd_matrix_quad, pack_herm_band_quad
    use target_lapack,   only: target_name, target_eps, target_zpbtrf
    use ref_quad_lapack, only: zpbtrf
    implicit none

    integer, parameter :: ns(*)  = [16, 32, 64]
    integer, parameter :: kds(*) = [2, 4, 8]
    integer :: i, n, kd, ldab, info
    complex(ep), allocatable :: A0(:,:), AB_ref(:,:), AB_got(:,:)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('zpbtrf', target_name)
    do i = 1, size(ns)
        n = ns(i); kd = kds(i); ldab = kd + 1
        call gen_hpd_matrix_quad(n, A0, seed = 95001 + 47 * i)
        allocate(AB_ref(ldab, n), AB_got(ldab, n))
        call pack_herm_band_quad('U', n, kd, A0, AB_ref); AB_got = AB_ref
        call zpbtrf('U', n, kd, AB_ref, ldab, info)
        call target_zpbtrf('U', n, kd, AB_got, ldab, info)
        err = max_rel_err_mat_z(AB_got, AB_ref)
        tol = 16.0_ep * real(n, ep)**2 * target_eps
        write(label, '(a,i0,a,i0)') 'n=', n, ',kd=', kd
        call report_case(trim(label), err, tol)
        deallocate(AB_ref, AB_got)
    end do
    call report_finalize()
end program test_zpbtrf
