program test_dgeqlf
    ...
    use target_lapack,   only: target_name, target_eps, target_dgeqlf
    ...
    call report_init('dgeqlf', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n, A0, seed = 60001 + 59 * i)
