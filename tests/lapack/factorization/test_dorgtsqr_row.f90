! dorgtsqr_row: form Q from a TSQR factorization (row variant).
program test_dorgtsqr_row
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dorgtsqr_row
    use ref_quad_lapack, only: dlatsqr, dorgtsqr_row
    implicit none
    ...
    call report_init('dorgtsqr_row', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); mb = mbs(i); nb = nbs(i)
        call gen_matrix_quad(m, n, A0, seed = 23301 + 79 * i)
        ...
        deallocate(work)
        A_got = A_ref
        call dorgtsqr_row(m, n, mb, nb, A_ref, m, T_arr, nb, wopt, -1, info)
        lwork = max(1, int(wopt(1))); allocate(work(lwork))
        call dorgtsqr_row(m, n, mb, nb, A_ref, m, T_arr, nb, work, lwork, info)
        deallocate(work)
        call target_dorgtsqr_row(m, n, mb, nb, A_got, m, T_arr, nb, info)
