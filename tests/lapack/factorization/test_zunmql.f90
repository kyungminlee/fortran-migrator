program test_zunmql
    ...
    use target_lapack,   only: target_name, target_eps, target_zgeqlf, target_zunmql
    ...
    call report_init('zunmql', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n,    A0, seed = 65001 + 47 * i)
        call gen_matrix_complex(m, nrhs, C0, seed = 65011 + 47 * i)
        ...
        do jt = 1, size(transes)
            ...
            C_ref = C0; C_got = C0
            call zunmql('L', transes(jt), m, nrhs, n, A_ref, m, tau_ref, &
                        C_ref, m, wopt, -1, info)
            lwork = max(1, int(real(wopt(1), ep)))
            allocate(work(lwork))
            call zunmql('L', transes(jt), m, nrhs, n, A_ref, m, tau_ref, &
            ...
            deallocate(work)
            call target_zunmql('L', transes(jt), m, nrhs, n, A_got, m, tau_got, &
                               C_got, m, info)
