program test_dpptrf
    ...
    use target_lapack,   only: target_name, target_eps, target_dpptrf
    use ref_quad_lapack, only: dpptrf
    implicit none
    ...
    call report_init('dpptrf', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_spd_matrix_quad(n, A0, seed = 76001 + 47 * i)
