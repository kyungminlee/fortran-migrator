! dormhr: apply Q from Hessenberg reduction.
program test_dormhr
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dgehrd, target_dormhr
    ...
    call report_init('dormhr', target_name)
    do i = 1, size(ns)
        n = ns(i); ilo = 1; ihi = n
        call gen_matrix_quad(n, n,    A0, seed = 20001 + 73 * i)
        call gen_matrix_quad(n, nrhs, C0, seed = 20011 + 73 * i)
        allocate(A_ref(n,n), A_got(n,n), tau_ref(n-1), tau_got(n-1))
        A_ref = A0; A_got = A0
        call dgehrd(n, ilo, ihi, A_ref, n, tau_ref, wopt, -1, info)
        lwork = max(1, int(wopt(1))); allocate(work(lwork))
        call dgehrd(n, ilo, ihi, A_ref, n, tau_ref, work, lwork, info)
        deallocate(work)
        call target_dgehrd(n, ilo, ihi, A_got, n, tau_got, info)
        do jt = 1, size(transes)
            allocate(C_ref(n,nrhs), C_got(n,nrhs))
            C_ref = C0; C_got = C0
            call dormhr('L', transes(jt), n, nrhs, ilo, ihi, A_ref, n, tau_ref, &
                        C_ref, n, wopt, -1, info)
            lwork = max(1, int(wopt(1))); allocate(work(lwork))
            call dormhr('L', transes(jt), n, nrhs, ilo, ihi, A_ref, n, tau_ref, &
                        C_ref, n, work, lwork, info)
            deallocate(work)
            call target_dormhr('L', transes(jt), n, nrhs, ilo, ihi, A_got, n, tau_got, &
                               C_got, n, info)
