program test_zungqr
    ...
    call report_init('zungqr', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n, A0, seed = 8101 + 61 * i)
        ...
        call zungqr(m, n, n, A_ref, m, tau_ref, wopt, -1, info)
        lwork = max(1, int(real(wopt(1), ep)))
        allocate(work(lwork))
        call zungqr(m, n, n, A_ref, m, tau_ref, work, lwork, info)
        deallocate(work)
        call target_zungqr(m, n, n, A_got, m, tau_got, info)
