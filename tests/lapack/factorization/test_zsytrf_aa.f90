program test_zsytrf_aa
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf_aa
    use ref_quad_lapack, only: zsytrf_aa
    implicit none
    ...
    call report_init('zsytrf_aa', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 53001 + 47 * i)
