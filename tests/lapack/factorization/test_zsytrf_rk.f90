program test_zsytrf_rk
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf_rk
    use ref_quad_lapack, only: zsytrf_rk
    implicit none
    ...
    call report_init('zsytrf_rk', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 59001 + 47 * i)
