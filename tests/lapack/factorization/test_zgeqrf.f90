program test_zgeqrf
    ...
    use target_lapack,   only: target_name, target_eps, target_zgeqrf
    use ref_quad_lapack, only: zgeqrf
    implicit none

    integer, parameter :: ms(*) = [16, 48, 64]
    integer, parameter :: ns(*) = [8,  24, 48]
    integer :: i, m, n, info, lwork
    complex(ep), allocatable :: A0(:,:), A_ref(:,:), A_got(:,:)
    complex(ep), allocatable :: tau_ref(:), tau_got(:), work_ref(:)
    complex(ep) :: wopt(1)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('zgeqrf', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n, A0, seed = 9001 + 67 * i)
        ...
        call zgeqrf(m, n, A_ref, m, tau_ref, work_ref, lwork, info)

        call target_zgeqrf(m, n, A_got, m, tau_got, info)

        err = max(max_rel_err_mat_z(A_got, A_ref), &
                  max_rel_err_vec_z(tau_got, tau_ref))
