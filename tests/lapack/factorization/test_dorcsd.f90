! dorcsd: CS decomposition of a 2x2-partitioned orthogonal matrix.
! Builds the partition by orthonormalizing a random matrix (via dgeqrf+dorgqr).
program test_dorcsd
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dorcsd
    use ref_quad_lapack, only: dgeqrf, dorgqr, dorcsd
    implicit none
    ...
    call report_init('dorcsd', target_name)
    do i = 1, size(ms)
        m = ms(i); p = m/2; q = m/2
        call gen_matrix_quad(m, m, A, seed = 24101 + 73 * i)
        allocate(tau(m))
        ...
        deallocate(work)
        call target_dorcsd('Y', 'Y', 'Y', 'Y', 'N', 'D', m, p, q, &
