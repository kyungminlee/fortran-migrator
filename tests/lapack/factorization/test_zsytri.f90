program test_zsytri
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf, target_zsytri
    use ref_quad_lapack, only: zsytrf, zsytri
    implicit none
    ...
    call report_init('zsytri', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 60301 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            allocate(work(n))
            call zsytri(uplos(ju), n, A_ref, n, ipiv_ref, work, info)
            deallocate(work)
            call target_zsytri(uplos(ju), n, A_got, n, ipiv_got, info)
