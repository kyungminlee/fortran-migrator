program test_zhetri
    ...
    use target_lapack,   only: target_name, target_eps, target_zhetrf, target_zhetri
    use ref_quad_lapack, only: zhetrf, zhetri
    implicit none
    ...
    call report_init('zhetri', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A0, seed = 55001 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            allocate(work(n))
            call zhetri(uplos(ju), n, A_ref, n, ipiv_ref, work, info)
            deallocate(work)
            call target_zhetri(uplos(ju), n, A_got, n, ipiv_got, info)
