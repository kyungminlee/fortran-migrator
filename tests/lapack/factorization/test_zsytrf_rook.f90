program test_zsytrf_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf_rook
    use ref_quad_lapack, only: zsytrf_rook
    implicit none
    ...
    call report_init('zsytrf_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 61001 + 47 * i)
