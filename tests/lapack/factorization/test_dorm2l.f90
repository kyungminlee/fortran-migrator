! dorm2l: apply Q from QL (unblocked).
program test_dorm2l
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dgeqlf, target_dorm2l
    use ref_quad_lapack, only: dgeqlf, dorm2l
    implicit none
    ...
    call report_init('dorm2l', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_quad(m, n,    A0, seed = 19801 + 59 * i)
        call gen_matrix_quad(m, nrhs, C0, seed = 19811 + 59 * i)
        ...
        do jt = 1, size(transes)
            ...
            C_ref = C0; C_got = C0
            call dorm2l('L', transes(jt), m, nrhs, n, A_ref, m, tau_ref, C_ref, m, work, info)
            call target_dorm2l('L', transes(jt), m, nrhs, n, A_got, m, tau_got, C_got, m, info)
