program test_dsytri_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf_rook, target_dsytri_rook
    use ref_quad_lapack, only: dsytrf_rook, dsytri_rook
    implicit none
    ...
    call report_init('dsytri_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 16301 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            allocate(work(n))
            call dsytri_rook(uplos(ju), n, A_ref, n, ipiv_ref, work, info)
            deallocate(work)
            call target_dsytri_rook(uplos(ju), n, A_got, n, ipiv_got, info)
