! dgbtrf: banded LU factorization. Build a diagonally-dominant banded
! matrix, pack into LAPACK banded storage with row indexing
! AB(kl+ku+1+i-j, j) = A(i,j) for max(1,j-ku) <= i <= min(m,j+kl).
! Storage size is (2*kl+ku+1) * n; the leading kl rows are workspace
! that dgbtrf fills with the L multipliers.
program test_dgbtrf
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dgbtrf
    use ref_quad_lapack, only: dgbtrf
    implicit none

    integer, parameter :: ns(*) = [16, 32, 64]
    integer, parameter :: kl    = 2
    integer, parameter :: ku    = 3
    integer :: i, n, ldab, info, j, k
    real(ep), allocatable :: Adense(:,:), AB0(:,:), AB_ref(:,:), AB_got(:,:)
    integer,  allocatable :: ipiv_ref(:), ipiv_got(:)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('dgbtrf', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = 2*kl + ku + 1
        call gen_matrix_quad(n, n, Adense, seed = 60001 + 47 * i)
        do j = 1, n
            Adense(j, j) = Adense(j, j) + real(2*n, ep)
        end do
        allocate(AB0(ldab, n))
        AB0 = 0.0_ep
        do j = 1, n
            do k = max(1, j-ku), min(n, j+kl)
                AB0(kl + ku + 1 + k - j, j) = Adense(k, j)
            end do
        end do
        allocate(AB_ref(ldab, n), AB_got(ldab, n), ipiv_ref(n), ipiv_got(n))
        AB_ref = AB0; AB_got = AB0
        call dgbtrf(n, n, kl, ku, AB_ref, ldab, ipiv_ref, info)
        call target_dgbtrf(n, n, kl, ku, AB_got, ldab, ipiv_got, info)
        err = max_rel_err_mat(AB_got, AB_ref)
        tol = 16.0_ep * real(n, ep)**2 * target_eps
        write(label, '(a,i0)') 'n=', n
        call report_case(trim(label), err, tol)
        deallocate(Adense, AB0, AB_ref, AB_got, ipiv_ref, ipiv_got)
    end do
    call report_finalize()
end program test_dgbtrf
