! zung2r: unblocked generation of Q from QR (complex).
program test_zung2r
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_zgeqrf, target_zung2r
    use ref_quad_lapack, only: zgeqrf, zung2r
    implicit none
    ...
    call report_init('zung2r', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n, A0, seed = 19551 + 71 * i)
        ...
        call zung2r(m, n, n, A_ref, m, tau_ref, work, info)
        call target_zung2r(m, n, n, A_got, m, tau_got, info)
