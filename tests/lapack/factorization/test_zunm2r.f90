! zunm2r: apply Q from QR (unblocked variant, complex).
program test_zunm2r
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zgeqrf, target_zunm2r
    use ref_quad_lapack, only: zgeqrf, zunm2r
    implicit none
    ...
    call report_init('zunm2r', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i)
        call gen_matrix_complex(m, n,    A0, seed = 19751 + 53 * i)
        call gen_matrix_complex(m, nrhs, C0, seed = 19761 + 53 * i)
        ...
        do jt = 1, size(transes)
            ...
            C_ref = C0; C_got = C0
            call zunm2r('L', transes(jt), m, nrhs, n, A_ref, m, tau_ref, C_ref, m, work, info)
            call target_zunm2r('L', transes(jt), m, nrhs, n, A_got, m, tau_got, C_got, m, info)
