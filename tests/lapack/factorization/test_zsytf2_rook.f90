program test_zsytf2_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytf2_rook
    use ref_quad_lapack, only: zsytf2_rook
    implicit none
    ...
    call report_init('zsytf2_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 61101 + 47 * i)
        do j = 1, n; A0(j,j) = A0(j,j) + cmplx(real(n, ep), 0.0_ep, ep); end do
        do ju = 1, size(uplos)
            allocate(A_ref(n,n), A_got(n,n), ipiv_ref(n), ipiv_got(n))
            A_ref = A0; A_got = A0
            call zsytf2_rook(uplos(ju), n, A_ref, n, ipiv_ref, info)
            call target_zsytf2_rook(uplos(ju), n, A_got, n, ipiv_got, info)
