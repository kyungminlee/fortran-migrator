program test_dsytri
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf, target_dsytri
    use ref_quad_lapack, only: dsytrf, dsytri
    implicit none
    ...
    call report_init('dsytri', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 25001 + 47 * i)
        ...
        do ju = 1, size(uplos)
            ...
            allocate(work(n))
            call dsytri(uplos(ju), n, A_ref, n, ipiv_ref, work, info)
            deallocate(work)
            call target_dsytri(uplos(ju), n, A_got, n, ipiv_got, info)
