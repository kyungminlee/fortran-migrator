! dorbdb: simultaneous bidiagonalization of a partitioned orthogonal matrix.
program test_dorbdb
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dorbdb
    use ref_quad_lapack, only: dorbdb
    implicit none

    integer, parameter :: ms(*) = [12, 24]
    integer :: i, m, p, q, info, lwork
    real(ep), allocatable :: X11r(:,:), X12r(:,:), X21r(:,:), X22r(:,:)
    real(ep), allocatable :: X11g(:,:), X12g(:,:), X21g(:,:), X22g(:,:)
    real(ep), allocatable :: theta_r(:), theta_g(:), phi_r(:), phi_g(:)
    real(ep), allocatable :: tp1r(:), tp2r(:), tq1r(:), tq2r(:), work(:)
    real(ep), allocatable :: tp1g(:), tp2g(:), tq1g(:), tq2g(:)
    real(ep) :: wopt(1), err, tol
    character(len=48) :: label

    call report_init('dorbdb', target_name)
    do i = 1, size(ms)
        m = ms(i); p = m/2; q = m/2
        ...
        block
            ...
            integer :: ii, jj
            call gen_matrix_quad(m, m, A, seed = 24001 + 67 * i)
            do ii = 1, p
            ...
            do ii = 1, p; X11r(ii, ii) = X11r(ii, ii) + 1.0_ep; end do
            do ii = 1, m-p; X22r(ii, ii) = X22r(ii, ii) + 1.0_ep; end do
            deallocate(A)
        end block
        X11g = X11r; X12g = X12r; X21g = X21r; X22g = X22r
        call dorbdb('N', 'O', m, p, q, X11r, p, X12r, p, X21r, m-p, X22r, m-p, &
        ...
        deallocate(work)
        call target_dorbdb('N', 'O', m, p, q, X11g, p, X12g, p, X21g, m-p, X22g, m-p, &
