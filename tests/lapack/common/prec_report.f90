module prec_report
    ...
    ! Bit-exact mode for kind16: the migrated LAPACK routine and the
    ! quad-promoted Netlib reflapack_quad reference compute the same
    ! algorithm at REAL(KIND=16) — any non-zero divergence is a real
    ! migration bug, even one that lands inside the per-case rounding
    ! budget. (Distributed suites disable this; their reductions reorder.)
...
contains
    ...
    subroutine report_case(case_label, max_rel, tol)
        ...
        if (strict_exact) then
            ...
            ! against reflapack_quad. Quad arithmetic gives ~33
