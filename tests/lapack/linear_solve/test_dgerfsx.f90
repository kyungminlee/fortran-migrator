! dgerfsx: extra-precise refinement of LU solution. Pre-factor with
! getrf+getrs on both paths, then refine via rfsx; compare X / rcond.
program test_dgerfsx
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat, rel_err_scalar
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, &
                                target_dgetrf, target_dgetrs, target_dgerfsx
    use ref_quad_lapack, only: dgetrf, dgetrs, dgerfsx
    implicit none
    ...
    call report_init('dgerfsx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_quad(n, n, A0, seed = 410201 + 47 * i)
        do j = 1, n; A0(j, j) = A0(j, j) + real(2*n, ep); end do
        call gen_matrix_quad(n, nrhs, B0, seed = 410211 + 47 * i)
        ...
        X_ref = B0; X_got = B0
        call dgetrs('N', n, nrhs, A_ref, n, ipiv_ref, X_ref, n, info)
        call target_dgetrs('N', n, nrhs, A_got, n, ipiv_got, X_got, n, info)
        R = 0.0_ep; C = 0.0_ep
        call dgerfsx('N', 'N', n, nrhs, A0, n, A_ref, n, ipiv_ref,        &
                     R, C, B0, n, X_ref, n, rcond_ref, berr_ref,          &
                     n_err_bnds, ebn_ref, ebc_ref, 0, params, work, iwork, info)
        call target_dgerfsx('N', 'N', n, nrhs, A0, n, A_got, n, ipiv_got, &
