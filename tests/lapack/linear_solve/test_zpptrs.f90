program test_zpptrs
    ...
    use target_lapack,   only: target_name, target_eps, target_zpptrf, target_zpptrs
    use ref_quad_lapack, only: zpptrf, zpptrs
    implicit none
    ...
    call report_init('zpptrs', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_hpd_matrix_quad(n, A0, seed = 89001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 89011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call zpptrs('U', n, nrhs, AP_ref, B_ref, n, info)
        call target_zpptrs('U', n, nrhs, AP_got, B_got, n, info)
