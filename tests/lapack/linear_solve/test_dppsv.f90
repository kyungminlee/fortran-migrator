program test_dppsv
    ...
    use target_lapack,   only: target_name, target_eps, target_dppsv
    use ref_quad_lapack, only: dppsv
    implicit none
    ...
    call report_init('dppsv', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_spd_matrix_quad(n, A0, seed = 78001 + 47 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 78011 + 47 * i)
        allocate(AP_ref(np), AP_got(np), B_ref(n, nrhs), B_got(n, nrhs))
        call pack_sym_packed_quad('U', n, A0, AP_ref); AP_got = AP_ref
        B_ref = B0; B_got = B0
        call dppsv('U', n, nrhs, AP_ref, B_ref, n, info)
        call target_dppsv('U', n, nrhs, AP_got, B_got, n, info)
