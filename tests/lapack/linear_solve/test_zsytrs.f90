program test_zsytrs
    ...
    use target_lapack,   only: target_name, target_eps, target_zsytrf, target_zsytrs
    use ref_quad_lapack, only: zsytrf, zsytrs
    implicit none
    ...
    call report_init('zsytrs', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 60101 + 47 * i)
        do j = 1, n; A0(j,j) = A0(j,j) + cmplx(real(n, ep), 0.0_ep, ep); end do
        call gen_matrix_complex(n, nrhs, B0, seed = 60111 + 47 * i)
        do ju = 1, size(uplos)
            ...
            B_ref = B0; B_got = B0
            call zsytrs(uplos(ju), n, nrhs, A_ref, n, ipiv_ref, B_ref, n, info)
            call target_zsytrs(uplos(ju), n, nrhs, A_got, n, ipiv_got, B_got, n, info)
