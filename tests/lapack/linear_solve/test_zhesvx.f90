program test_zhesvx
    ...
    use target_lapack,   only: target_name, target_eps, target_zhesvx
    use ref_quad_lapack, only: zhesvx
    implicit none
    ...
    call report_init('zhesvx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_hermitian_matrix_quad(n, A, seed = 420081 + 47 * i)
        do j = 1, n; A(j, j) = A(j, j) + cmplx(real(2*n, ep), 0.0_ep, ep); end do
        call gen_matrix_complex(n, nrhs, B0, seed = 420091 + 47 * i)
