program test_zsysv_rook
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z
    use test_data,       only: gen_complex_symmetric_quad, gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zsysv_rook
    use ref_quad_lapack, only: zsysv_rook
    implicit none
    ...
    call report_init('zsysv_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 61401 + 47 * i)
        do j = 1, n; A0(j,j) = A0(j,j) + cmplx(real(n, ep), 0.0_ep, ep); end do
        call gen_matrix_complex(n, nrhs, B0, seed = 61411 + 47 * i)
        do ju = 1, size(uplos)
            allocate(A_ref(n,n), A_got(n,n), B_ref(n,nrhs), B_got(n,nrhs))
            allocate(ipiv_ref(n), ipiv_got(n))
            A_ref = A0; A_got = A0; B_ref = B0; B_got = B0
            call zsysv_rook(uplos(ju), n, nrhs, A_ref, n, ipiv_ref, B_ref, n, wopt, -1, info)
            lwork = max(1, int(real(wopt(1), ep)))
            allocate(work(lwork))
            call zsysv_rook(uplos(ju), n, nrhs, A_ref, n, ipiv_ref, B_ref, n, work, lwork, info)
            deallocate(work)
            call target_zsysv_rook(uplos(ju), n, nrhs, A_got, n, ipiv_got, B_got, n, info)
