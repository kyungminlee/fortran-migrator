program test_dgbtrs
    ...
    use target_lapack,   only: target_name, target_eps, target_dgbtrf, target_dgbtrs
    use ref_quad_lapack, only: dgbtrf, dgbtrs
    implicit none
    ...
    call report_init('dgbtrs', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = 2*kl + ku + 1
        call gen_matrix_quad(n, n, Adense, seed = 61001 + 47 * i)
        do j = 1, n
            Adense(j, j) = Adense(j, j) + real(2*n, ep)
        end do
        call gen_matrix_quad(n, nrhs, B0, seed = 61011 + 47 * i)
        allocate(AB0(ldab, n))
        AB0 = 0.0_ep
        do j = 1, n
        ...
        B_ref = B0; B_got = B0
        call dgbtrs('N', n, kl, ku, nrhs, AB_ref, ldab, ipiv_ref, B_ref, n, info)
        call target_dgbtrs('N', n, kl, ku, nrhs, AB_got, ldab, ipiv_got, B_got, n, info)
