program test_dsptrs
    ...
    use target_lapack,   only: target_name, target_eps, target_dsptrf, target_dsptrs
    use ref_quad_lapack, only: dsptrf, dsptrs
    implicit none
    ...
    call report_init('dsptrs', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_symmetric_matrix_quad(n, A0, seed = 73001 + 47 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 73011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call dsptrs('U', n, nrhs, AP_ref, ipiv_ref, B_ref, n, info)
        call target_dsptrs('U', n, nrhs, AP_got, ipiv_got, B_got, n, info)
