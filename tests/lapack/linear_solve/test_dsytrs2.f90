! dsytrs2: blocked solve A*X=B given the sytrf factorization.
program test_dsytrs2
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat
    use test_data,       only: gen_symmetric_matrix_quad, gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dsytrf, target_dsytrs2
    use ref_quad_lapack, only: dsytrf, dsytrs2
    implicit none
    ...
    real(ep), allocatable :: B_ref(:,:), B_got(:,:), work(:)
    integer,  allocatable :: ipiv_ref(:), ipiv_got(:)
    real(ep) :: wopt(1), err, tol
    character(len=64) :: label

    call report_init('dsytrs2', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 26101 + 47 * i)
        do j = 1, n; A0(j,j) = A0(j,j) + real(n, ep); end do
        do ir = 1, size(nrhss)
            nrhs = nrhss(ir)
            call gen_matrix_quad(n, nrhs, B0, seed = 26111 + 47 * i + ir)
            do ju = 1, size(uplos)
                ...
                allocate(work(n))
                call dsytrs2(uplos(ju), n, nrhs, A_ref, n, ipiv_ref, B_ref, n, work, info)
                deallocate(work)
                call target_dsytrs2(uplos(ju), n, nrhs, A_got, n, ipiv_got, B_got, n, info)
