program test_zpbsvx
    ...
    use target_lapack,   only: target_name, target_eps, target_zpbsvx
    use ref_quad_lapack, only: zpbsvx
    implicit none
    ...
    call report_init('zpbsvx', target_name)
    do i = 1, size(ns)
        n = ns(i); kd = kds(i); ldab = kd + 1
        call gen_hpd_matrix_quad(n, A, seed = 410021 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 410031 + 47 * i)
        ...
        call pack_herm_band_quad('U', n, kd, A, AB_ref); AB_got = AB_ref
        B_ref = B0; B_got = B0
        equed_ref = 'N'; equed_got = 'N'
        call zpbsvx('E', 'U', n, kd, nrhs, AB_ref, ldab, AFB_ref, ldab, equed_ref, &
                    S_ref, B_ref, n, X_ref, n, rcond_ref, ferr_ref, berr_ref, work, rwork, info)
        call target_zpbsvx('E', 'U', n, kd, nrhs, AB_got, ldab, AFB_got, ldab, equed_got, &
