program test_zgesv
    ...
    use target_lapack,   only: target_name, target_eps, target_zgesv
    ...
    complex(ep), allocatable :: A_ref(:,:), B_ref(:,:), A_got(:,:), B_got(:,:)
    integer,     allocatable :: ipiv_ref(:), ipiv_got(:)
    real(ep) :: err, tol
    character(len=48) :: label

    call report_init('zgesv', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_complex(n, n,    A0, seed = 6001 + 53 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 6011 + 53 * i)
        ...
        call target_zgesv(n, nrhs, A_got, n, ipiv_got, B_got, n, info_got)
