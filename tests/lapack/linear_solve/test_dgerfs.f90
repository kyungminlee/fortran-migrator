! Iterative refinement of LU solution. Compare refined X.
program test_dgerfs
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_dgetrf, target_dgetrs, target_dgerfs
    use ref_quad_lapack, only: dgetrf, dgetrs, dgerfs
    implicit none
    ...
    call report_init('dgerfs', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_quad(n, n, A0, seed = 134001 + 47 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 134011 + 47 * i)
        ...
        call dgerfs('N', n, nrhs, A0, n, A_ref, n, ipiv_ref, B0, n, &
                    X_ref, n, ferr, berr, work, iwork, info)
        call target_dgerfs('N', n, nrhs, A0, n, A_got, n, ipiv_got, B0, n, &
