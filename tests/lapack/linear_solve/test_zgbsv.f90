program test_zgbsv
    ...
    use target_lapack,   only: target_name, target_eps, target_zgbsv
    use ref_quad_lapack, only: zgbsv
    implicit none
    ...
    call report_init('zgbsv', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = 2*kl + ku + 1
        call gen_matrix_complex(n, n, Adense, seed = 94001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 94011 + 47 * i)
        do j = 1, n
        ...
        B_ref = B0; B_got = B0
        call zgbsv(n, kl, ku, nrhs, AB_ref, ldab, ipiv_ref, B_ref, n, info)
        call target_zgbsv(n, kl, ku, nrhs, AB_got, ldab, ipiv_got, B_got, n, info)
