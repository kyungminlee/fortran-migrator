program test_zsptrs
    ...
    use target_lapack,   only: target_name, target_eps, target_zsptrf, target_zsptrs
    use ref_quad_lapack, only: zsptrf, zsptrs
    implicit none
    ...
    call report_init('zsptrs', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_complex_symmetric_quad(n, A0, seed = 85001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 85011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call zsptrs('U', n, nrhs, AP_ref, ipiv_ref, B_ref, n, info)
        call target_zsptrs('U', n, nrhs, AP_got, ipiv_got, B_got, n, info)
