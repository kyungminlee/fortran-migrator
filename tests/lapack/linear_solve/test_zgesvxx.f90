! zgesvxx: extra-precise expert solver, complex.
program test_zgesvxx
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat_z, rel_err_scalar
    use test_data,       only: gen_matrix_complex
    use target_lapack,   only: target_name, target_eps, target_zgesvxx
    use ref_quad_lapack, only: zgesvxx
    implicit none
    ...
    call report_init('zgesvxx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_complex(n, n, A0, seed = 410101 + 47 * i)
        do j = 1, n; A0(j, j) = A0(j, j) + cmplx(real(2*n, ep), 0.0_ep, ep); end do
        call gen_matrix_complex(n, nrhs, B0, seed = 410111 + 47 * i)
        ...
        call zgesvxx('E', 'N', n, nrhs, A_ref, n, AF_ref, n, ipiv_ref, equed_ref, &
                     R_ref, C_ref, B_ref, n, X_ref, n, rcond_ref, rpvgrw_ref,     &
                     berr_ref, n_err_bnds, ebn_ref, ebc_ref, 0, params,           &
                     work, rwork, info)
        call target_zgesvxx('E', 'N', n, nrhs, A_got, n, AF_got, n, ipiv_got, equed_got, &
