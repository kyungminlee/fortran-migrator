! zhesvxx: extra-precise expert solver for Hermitian systems.
program test_zhesvxx
    ...
    use target_lapack,   only: target_name, target_eps, target_zhesvxx
    use ref_quad_lapack, only: zhesvxx
    implicit none
    ...
    call report_init('zhesvxx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        ! HPD inputs guarantee non-singularity for the indefinite-symmetric driver
        ! while still hitting the Hermitian code path (Bunch–Kaufman).
        call gen_hpd_matrix_quad(n, A0, seed = 411601 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 411611 + 47 * i)
        ...
        allocate(ipiv_ref(n), ipiv_got(n), work(5*n), rwork(2*n), params(1))
        A_ref = A0; B_ref = B0; A_got = A0; B_got = B0
        equed_ref = 'N'; equed_got = 'N'
        call zhesvxx('E', 'U', n, nrhs, A_ref, n, AF_ref, n, ipiv_ref, equed_ref, &
                     S_ref, B_ref, n, X_ref, n, rcond_ref, rpvgrw_ref, berr_ref,   &
                     n_err_bnds, ebn_ref, ebc_ref, 0, params, work, rwork, info)
        call target_zhesvxx('E', 'U', n, nrhs, A_got, n, AF_got, n, ipiv_got, equed_got, &
