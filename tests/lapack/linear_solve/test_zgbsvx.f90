program test_zgbsvx
    ...
    use target_lapack,   only: target_name, target_eps, target_zgbsvx
    use ref_quad_lapack, only: zgbsvx
    implicit none
    ...
    call report_init('zgbsvx', target_name)
    do i = 1, size(ns)
        n = ns(i); ldab = kl + ku + 1; ldafb = 2*kl + ku + 1
        call gen_matrix_complex(n, n, A, seed = 420021 + 47 * i)
        do j = 1, n; A(j, j) = A(j, j) + cmplx(real(2*n, ep), 0.0_ep, ep); end do
        call gen_matrix_complex(n, nrhs, B0, seed = 420031 + 47 * i)
        ...
        allocate(ipiv_ref(n), ipiv_got(n), work(2*n), rwork(n))
        ...
        call zgbsvx('E', 'N', n, kl, ku, nrhs, AB_ref, ldab, AFB_ref, ldafb, ipiv_ref, &
                    equed_ref, R_ref, C_ref, B_ref, n, X_ref, n, rcond_ref, &
                    ferr_ref, berr_ref, work, rwork, info)
        call target_zgbsvx('E', 'N', n, kl, ku, nrhs, AB_got, ldab, AFB_got, ldafb, ipiv_got, &
