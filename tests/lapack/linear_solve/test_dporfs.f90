program test_dporfs
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_dpotrf, target_dpotrs, target_dporfs
    use ref_quad_lapack, only: dpotrf, dpotrs, dporfs
    implicit none
    ...
    call report_init('dporfs', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_spd_matrix_quad(n, A0, seed = 136001 + 47 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 136011 + 47 * i)
        ...
        call dporfs('U', n, nrhs, A0, n, A_ref, n, B0, n, &
                    X_ref, n, ferr, berr, work, iwork, info)
        call target_dporfs('U', n, nrhs, A0, n, A_got, n, B0, n, &
                           X_got, n, ferr, berr, info)
        err = max_rel_err_mat(X_got, X_ref)
        tol = 16.0_ep * real(n, ep)**2 * target_eps
        write(label, '(a,i0)') 'n=', n
        call report_case(trim(label), err, tol)
        deallocate(A_ref, A_got, X_ref, X_got, work, iwork, ferr, berr)
    end do
    call report_finalize()
end program test_dporfs
