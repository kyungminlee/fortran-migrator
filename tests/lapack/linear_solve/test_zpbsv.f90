program test_zpbsv
    ...
    use target_lapack,   only: target_name, target_eps, target_zpbsv
    use ref_quad_lapack, only: zpbsv
    implicit none
    ...
    call report_init('zpbsv', target_name)
    do i = 1, size(ns)
        n = ns(i); kd = kds(i); ldab = kd + 1
        call gen_hpd_matrix_quad(n, A0, seed = 97001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 97011 + 47 * i)
        allocate(AB_ref(ldab, n), AB_got(ldab, n), B_ref(n, nrhs), B_got(n, nrhs))
        call pack_herm_band_quad('U', n, kd, A0, AB_ref); AB_got = AB_ref
        B_ref = B0; B_got = B0
        call zpbsv('U', n, kd, nrhs, AB_ref, ldab, B_ref, n, info)
        call target_zpbsv('U', n, kd, nrhs, AB_got, ldab, B_got, n, info)
