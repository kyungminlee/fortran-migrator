! dgelsd: divide-and-conquer SVD-based least squares.
program test_dgelsd
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_mat, max_rel_err_vec
    use test_data,       only: gen_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_dgelsd
    use ref_quad_lapack, only: dgelsd
    implicit none
    ...
    call report_init('dgelsd', target_name)
    do i = 1, size(ms)
        m = ms(i); n = ns(i); ldb = max(m, n)
        call gen_matrix_quad(m, n,    A0, seed = 150001 + 47 * i)
        call gen_matrix_quad(ldb, nrhs, B0, seed = 150011 + 47 * i)
        allocate(A_r(m,n), B_r(ldb,nrhs), A_g(m,n), B_g(ldb,nrhs))
        allocate(S_r(min(m,n)), S_g(min(m,n)))
        A_r = A0; B_r = B0; A_g = A0; B_g = B0
        rcond = -1.0_ep
        call dgelsd(m, n, nrhs, A_r, m, B_r, ldb, S_r, rcond, rank_r, &
                    wopt, -1, iwopt, info)
        lwork = max(1, int(wopt(1))); allocate(work(lwork), iwork(max(1, iwopt(1))))
        call dgelsd(m, n, nrhs, A_r, m, B_r, ldb, S_r, rcond, rank_r, &
                    work, lwork, iwork, info)
        deallocate(work, iwork)
        call target_dgelsd(m, n, nrhs, A_g, m, B_g, ldb, S_g, rcond, rank_g, info)
