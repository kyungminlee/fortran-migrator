program test_zspsv
    ...
    use target_lapack,   only: target_name, target_eps, target_zspsv
    use ref_quad_lapack, only: zspsv
    implicit none
    ...
    call report_init('zspsv', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_complex_symmetric_quad(n, A0, seed = 86001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 86011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call zspsv('U', n, nrhs, AP_ref, ipiv_ref, B_ref, n, info)
        call target_zspsv('U', n, nrhs, AP_got, ipiv_got, B_got, n, info)
