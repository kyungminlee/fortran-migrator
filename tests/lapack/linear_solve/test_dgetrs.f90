! dgetrs test: run dgetrf+dgetrs on each side independently and
! compare the solution B. Tests the solve step plus dgetrf upstream.
program test_dgetrs
    ...
    call report_init('dgetrs', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_matrix_quad(n, n,    A0, seed = 3001 + 41 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 3011 + 41 * i)
        ...
        call dgetrs('N', n, nrhs, A_ref, n, ipiv_ref, B_ref, n, info)
        call target_dgetrf(n, n, A_got, n, ipiv_got, info)
        call target_dgetrs('N', n, nrhs, A_got, n, ipiv_got, B_got, n, info)
