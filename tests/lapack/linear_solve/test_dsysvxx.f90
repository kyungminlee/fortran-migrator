! dsysvxx: extra-precise expert solver for symmetric indefinite systems.
program test_dsysvxx
    ...
    use target_lapack,   only: target_name, target_eps, target_dsysvxx
    use ref_quad_lapack, only: dsysvxx
    implicit none
    ...
    call report_init('dsysvxx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 411201 + 47 * i)
        do j = 1, n; A0(j, j) = A0(j, j) + real(2*n, ep); end do
        call gen_matrix_quad(n, nrhs, B0, seed = 411211 + 47 * i)
        ...
        call dsysvxx('E', 'U', n, nrhs, A_ref, n, AF_ref, n, ipiv_ref, equed_ref, &
                     S_ref, B_ref, n, X_ref, n, rcond_ref, rpvgrw_ref, berr_ref,   &
                     n_err_bnds, ebn_ref, ebc_ref, 0, params, work, iwork, info)
        call target_dsysvxx('E', 'U', n, nrhs, A_got, n, AF_got, n, ipiv_got, equed_got, &
