! zsyrfsx: extra-precise refinement of complex symmetric solution.
program test_zsyrfsx
    ...
    use target_lapack,   only: target_name, target_eps, &
                                target_zsytrf, target_zsytrs, target_zsyrfsx
    use ref_quad_lapack, only: zsytrf, zsytrs, zsyrfsx
    implicit none
    ...
    call report_init('zsyrfsx', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_complex_symmetric_quad(n, A0, seed = 411501 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 411511 + 47 * i)
        ...
        call zsytrf('U', n, A_ref, n, ipiv_ref, work_f, lwork, info)
        deallocate(work_f)
        call target_zsytrf('U', n, A_got, n, ipiv_got, info)
        X_ref = B0; X_got = B0
        call zsytrs('U', n, nrhs, A_ref, n, ipiv_ref, X_ref, n, info)
        call target_zsytrs('U', n, nrhs, A_got, n, ipiv_got, X_got, n, info)
        S = 0.0_ep
        call zsyrfsx('U', 'N', n, nrhs, A0, n, A_ref, n, ipiv_ref, S,       &
                     B0, n, X_ref, n, rcond_ref, berr_ref,                  &
                     n_err_bnds, ebn_ref, ebc_ref, 0, params, work, rwork, info)
        call target_zsyrfsx('U', 'N', n, nrhs, A0, n, A_got, n, ipiv_got, S, &
