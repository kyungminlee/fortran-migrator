program test_dpptrs
    ...
    use target_lapack,   only: target_name, target_eps, target_dpptrf, target_dpptrs
    use ref_quad_lapack, only: dpptrf, dpptrs
    implicit none
    ...
    call report_init('dpptrs', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_spd_matrix_quad(n, A0, seed = 77001 + 47 * i)
        call gen_matrix_quad(n, nrhs, B0, seed = 77011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call dpptrs('U', n, nrhs, AP_ref, B_ref, n, info)
        call target_dpptrs('U', n, nrhs, AP_got, B_got, n, info)
