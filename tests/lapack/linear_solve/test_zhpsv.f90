program test_zhpsv
    ...
    use target_lapack,   only: target_name, target_eps, target_zhpsv
    use ref_quad_lapack, only: zhpsv
    implicit none
    ...
    call report_init('zhpsv', target_name)
    do i = 1, size(ns)
        n = ns(i); np = n*(n+1)/2
        call gen_hermitian_matrix_quad(n, A0, seed = 82001 + 47 * i)
        call gen_matrix_complex(n, nrhs, B0, seed = 82011 + 47 * i)
        ...
        B_ref = B0; B_got = B0
        call zhpsv('U', n, nrhs, AP_ref, ipiv_ref, B_ref, n, info)
        call target_zhpsv('U', n, nrhs, AP_got, ipiv_got, B_got, n, info)
