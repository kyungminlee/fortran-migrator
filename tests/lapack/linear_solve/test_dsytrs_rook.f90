program test_dsytrs_rook
    ...
    use target_lapack,   only: target_name, target_eps, target_dsytrf_rook, target_dsytrs_rook
    use ref_quad_lapack, only: dsytrf_rook, dsytrs_rook
    implicit none
    ...
    call report_init('dsytrs_rook', target_name)
    do i = 1, size(ns)
        n = ns(i)
        call gen_symmetric_matrix_quad(n, A0, seed = 17101 + 47 * i)
        do j = 1, n; A0(j,j) = A0(j,j) + real(n, ep); end do
        call gen_matrix_quad(n, nrhs, B0, seed = 17111 + 47 * i)
        do ju = 1, size(uplos)
            ...
            B_ref = B0; B_got = B0
            call dsytrs_rook(uplos(ju), n, nrhs, A_ref, n, ipiv_ref, B_ref, n, info)
            call target_dsytrs_rook(uplos(ju), n, nrhs, A_got, n, ipiv_got, B_got, n, info)
