! zpftrf: Cholesky factor of a Hermitian PD matrix in RFP storage (complex).
program test_zpftrf
    use prec_kinds,      only: ep
    use prec_report,     only: report_init, report_case, report_finalize
    use compare,         only: max_rel_err_vec_z
    use test_data,       only: gen_hpd_matrix_quad
    use target_lapack,   only: target_name, target_eps, target_zpftrf
    use ref_quad_lapack, only: ztrttf, zpftrf
    implicit none
    ...
    call report_init('zpftrf', target_name)
    do i = 1, size(ns)
        n = ns(i); nt = n*(n+1)/2
        call gen_hpd_matrix_quad(n, A, seed = 21051 + 97 * i)
        do t = 1, size(transrs)
            do u = 1, size(uplos)
                allocate(ARF0(nt), AR_ref(nt), AR_got(nt))
                call ztrttf(transrs(t), uplos(u), n, A, n, ARF0, info)
                AR_ref = ARF0; AR_got = ARF0
                call zpftrf(transrs(t), uplos(u), n, AR_ref, info)
                call target_zpftrf(transrs(t), uplos(u), n, AR_got, info)
