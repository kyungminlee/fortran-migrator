...
"""
    ...
    # Write target_config.cmake
    pmap = target_mode.prefix_map
    lib_prefix = pmap['R'].lower()
    needs_mf = target_mode.module_name is not None
    helpers_dir = str((proj_root / 'external' / 'lapack-3.12.1' / 'SRC').resolve())
    staged_list = ';'.join(staged)

    # For multifloats, only set MULTIFLOATS_DIR if a local copy exists.
    # Otherwise CMake will fetch from GitHub automatically.
    mf_dir_line = ''
    if needs_mf:
        local_mf = proj_root / 'external' / 'multifloats'
        if local_mf.is_dir():
            mf_dir_line = f'set(MULTIFLOATS_DIR "{local_mf.resolve()}")'
...
{mf_dir_line}
set(MF_HELPERS_DIR "{helpers_dir}")
