...
| Module | Purpose |
|---|---|
| `__main__.py` | CLI entry. Subcommands: `migrate`, `verify`, `stage`, `build`, `run`, `divergence-report`, `convergence-report`. `cmd_stage` (305 lines) orchestrates the per-library staging into a build-ready directory; `_generate_cmake` (233 lines) produces the top-level `CMakeLists.txt` template wired to the multifloats `FetchContent`, optional MPI, and per-target prefix selection. |
