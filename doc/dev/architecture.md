...
| `xblas.yaml` | blas | Extra-Precise BLAS from Netlib XBLAS 1.0.248. Auto-generated 402-entry skip list (`skip_files_manifest: xblas/skip_mixed_precision.txt`, written by `recipes/xblas/gen_skip_list.py`) excludes mixed-precision variants unreachable from LAPACK. Header patches inject quad/multifloats typedefs into `blas_enum.h` (the umbrella header `blas_extended.h` and every `*-f2c.c` bridge transitively include it). |
